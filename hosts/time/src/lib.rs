...
pub use snapshot::{
    ...
    time_input_base_uri, time_source_matches,
};
...
mod tests {
    ...
    fn snapshot_packet_uses_one_base_uri() {
        ...
        assert!(
            packet
                .updates
                .iter()
                .all(|u| u.source.base_uri() == "time://clock/clock")
        );
    }
    ...
    fn settings_accept_default() {
        assert_eq!(
            time_settings_from_config(&empty_settings()).unwrap(),
            TimeHostSettings { interval_ms: 100 }
        );
    }
    ...
    fn settings_accept_positive_interval() {
        ...
        assert_eq!(
            time_settings_from_config(&ConfigValue::Map(map)).unwrap(),
            TimeHostSettings { interval_ms: 250 }
        );
    }
    ...
    impl TimeBackend for FixedBackend {
        fn snapshot(&self) -> mech_core::MResult<TimeSnapshot> {
            Ok(snapshot())
        }
    }
    ...
    #[cfg(feature = "native")]
    fn long_interval_driver(
        runtime: &mech_runtime::MechRuntime,
    ) -> NativeTimeInputDriver<FixedBackend> {
    ...
    fn wait_for_pending_packets(runtime: &mech_runtime::MechRuntime, count: usize) {
        ...
        while runtime.pending_host_input_count().unwrap() < count {
            assert!(
                ...
                "timed out waiting for native time packet"
            );
    ...
    fn native_driver_stop_wakes_long_interval() {
        ...
        eprintln!(
            "native_driver_stop_wakes_long_interval elapsed_ms={}",
            elapsed.as_millis()
        );
        assert!(
