    ...
    B: TimeBackend,
{
    ...
    fn attach(&mut self, ingress: RuntimeIngress) -> MResult<()> {
        if self.is_live() {
            return Err(time_error(
                "TimeDriverAttach",
                "cannot attach browser time driver while live",
            ));
        }
        if self.ingress.is_some() {
            return Err(time_error(
                "TimeDriverAttach",
                "browser time driver is already attached",
            ));
        }
    ...
    fn start(&mut self) -> MResult<()> {
        ...
        let ingress = self.ingress.clone().ok_or_else(|| {
            time_error(
                "TimeDriverStart",
                "browser time driver must be attached before start",
            )
        })?;
        let window = web_sys::window()
            .ok_or_else(|| time_error("TimeDriverStart", "browser Window is unavailable"))?;
...
impl BrowserTimeHostFactory<BrowserTimeBackend> {
    pub fn new() -> MResult<Self> {
        Self::with_backend(BrowserTimeBackend)
    }
}
    ...
    B: TimeBackend,
{
    ...
    fn instantiate(
        ...
        Ok(RuntimeHostInstallation {
            ...
            input_drivers: vec![Box::new(BrowserTimeInputDriver::new(
                instance_name,
                self.backend.clone(),
                snapshot,
