...
pub fn time_settings_from_config(settings: &ConfigValue) -> MResult<TimeHostSettings> {
    let ConfigValue::Map(map) = settings else {
        return Err(time_error(
            "TimeHostConfig",
            "time host settings must be a map",
        ));
    };
        ...
        match key.as_str() {
            "interval-ms" => {
                let ConfigValue::Integer(raw) = value else {
                    return Err(time_error(
                        "TimeHostConfig",
                        "time host interval-ms must be an integer",
                    ));
                };
                if *raw <= 0 {
                    return Err(time_error(
                        "TimeHostConfig",
                        "time host interval-ms must be positive",
                    ));
                }
                if *raw > 60_000 {
                    return Err(time_error(
                        "TimeHostConfig",
                        "time host interval-ms must be at most 60000",
                    ));
                }
            ...
            other => {
                return Err(time_error(
                    "TimeHostConfig",
                    format!("unknown time host setting `{other}`"),
                ));
            }
        }
    }
