...
use crate::{SharedTimeSnapshot, TimeSnapshot, time_error, time_source_matches};
    ...
    pub fn publish(&self, snapshot: TimeSnapshot) -> MResult<()> {
        if !self.is_live() {
            return Err(time_error(
                "TimeDriverNotLive",
                "manual time driver is not live",
            ));
        }
        *self
            .snapshot
            .lock()
            .map_err(|_| time_error("TimeDriverNotLive", "time snapshot lock is poisoned"))? =
            snapshot;
        let ingress = self
            .ingress
            .lock()
            .map_err(|_| time_error("TimeDriverNotLive", "time ingress lock is poisoned"))?
            .clone()
            .ok_or_else(|| {
                time_error(
                    "TimeDriverNotLive",
                    "manual time driver has no ingress attachment",
                )
            })?;
...
impl RuntimeHostInputDriver for ManualTimeInputDriver {
    ...
    fn attach(&mut self, ingress: RuntimeIngress) -> MResult<()> {
        ...
        if guard.is_some() {
            return Err(time_error(
                "TimeDriverAttach",
                "time input driver is already attached",
            ));
        }
        *guard = Some(ingress);
        Ok(())
    ...
    fn start(&mut self) -> MResult<()> {
            ...
            .is_none()
        {
            return Err(time_error(
                "TimeDriverStart",
                "time input driver must be attached before start",
            ));
        }
