...
/// Answers one-shot `read()` requests for a lidar scan context by returning
/// the latest reduced snapshot value. The live push path is the input driver;
/// this provider serves pull-style reads.
#[derive(Debug)]
        ...
        let value = match path {
            ...
            other => {
                return Err(lidar_error(
                    ...
                    format!("unknown lidar scan path `{other}`"),
                ))
            }
        };
...
impl RuntimeResourceProvider for LidarResourceProvider {
    ...
    fn read(&self, request: RuntimeResourceReadRequest) -> MResult<Value> {
        if request.base_uri != self.base_uri() {
            return Err(lidar_error(
                "LidarResourceProvider",
                format!("unknown lidar resource `{}`", request.base_uri),
            ));
        }
        let snapshot = *self
            .snapshot
            .lock()
            .map_err(|_| lidar_error("LidarResourceProvider", "lidar snapshot lock is poisoned"))?;
