...
use mech_runtime::{
    ...
    materialize_host_manifest, parse_host_context_target, validate_host_manifest,
};
...
impl BrowserRuntimeInjectionConfig {
    ...
    pub fn from_document_and_runtime_with_profile(
        ...
        let injected_hosts_by_name: BTreeMap<String, HostInstanceConfig> = hosts
            .iter()
        ...
        if let Some(run) = &document.run {
            for grant in &run.grants {
                ...
                let spec = profile
                    ...
                    .expect("retained host provider must be registered");
    ...
    pub fn browser_config(&self) -> MResult<BrowserHostBrowserConfig> {
        browser_config_from_browser_hosts(
            self.hosts.iter().filter(|host| host.provider == "browser"),
        )
    }
...
impl MechErrorKind for InvalidBrowserHostConfigError {
    ...
    fn message(&self) -> String {
        format!(
            "Invalid browser host config field `{}`: {}",
...
impl BrowserHostConfig {
    ...
    pub fn into_browser_authority(&self) -> MResult<BrowserAuthority> {
        ...
        for entry in &self.browser.dom_manifest {
            let path = BrowserDomPath::new(&entry.path).map_err(|error| {
                invalid_error(
                    "browser.domManifest.path",
                    format!("invalid DOM path `{}`: {error}", entry.path),
                )
            })?;
            let selector = BrowserDomScope::new(&entry.selector).map_err(|error| {
                invalid_error(
            ...
            let operations = entry
                ...
                .iter()
                .map(|operation| {
                    BrowserOperation::parse(operation).ok_or_else(|| {
                        invalid_error(
            ...
            authority.bind_dom_path(BrowserDomManifestEntry::new(
                path, selector, property, operations,
            ));
        }
    ...
    pub fn to_browser_resource(&self) -> MResult<BrowserResource> {
        match self {
            Self::Dom { selector } => BrowserDomScope::new(selector)
                ...
                .map_err(|error| {
                    invalid_error(
                        "browser.grants.resource.selector",
                        format!("invalid DOM selector `{selector}`: {error}"),
                    )
                }),
            ...
            Self::Network { origin, methods } => BrowserNetworkScope::new(origin, methods.clone())
                ...
                .map_err(|error| {
                    invalid_error(
                        "browser.grants.resource.origin",
            ...
            Self::Storage {
                ...
                let backend = BrowserStorageBackend::parse(backend).ok_or_else(|| {
                    invalid_error(
        ...
        .ok_or_else(|| {
            ...
            invalid_error(
                ...
                format!(
                    ...
                    host.name, context_name, supported,
                ),
            )
        })?;
...
pub fn browser_config_from_settings(settings: &ConfigValue) -> MResult<BrowserHostBrowserConfig> {
    ...
    for key in map.keys() {
        match key.as_str() {
            ...
            other => {
                return invalid(
                    "browser.settings",
                    format!("unknown browser settings key `{other}`"),
                );
            }
        }
    }
    ...
    if let Some(value) = map.get("dom") {
        ...
        for item in dom {
            let ConfigValue::Map(item) = item else {
                return invalid("browser.settings.dom", "items must be maps");
            };
            reject_unknown_fields(
                item,
                &["path", "selector", "property", "attribute", "operations"],
                "browser.settings.dom",
            )?;
            ...
            BrowserDomPath::new(&path).map_err(|error| {
                invalid_error(
                    "browser.settings.dom.path",
                    format!("invalid DOM path `{path}`: {error}"),
                )
            })?;
            BrowserDomScope::new(&selector).map_err(|error| {
                invalid_error(
                    "browser.settings.dom.selector",
                    format!("invalid DOM selector `{selector}`: {error}"),
                )
            })?;
            BrowserDomProperty::parse_config_name(&property, attribute.as_deref()).map_err(
                |error| {
                    invalid_error(
                        "browser.settings.dom.property",
                        format!("invalid DOM property `{property}`: {error}"),
                    )
                },
            )?;
            dom_manifest.push(BrowserHostDomManifestEntry {
                path,
            ...
            grants.push(BrowserHostBrowserGrant {
                resource: BrowserHostResourceConfig::Dom { selector },
                allow: operations,
            });
        }
    }
    ...
    if let Some(value) = map.get("clipboard") {
        ...
        for item in clipboard {
            let ConfigValue::Map(item) = item else {
                return invalid("browser.settings.clipboard", "items must be maps");
            };
            ...
            grants.push(BrowserHostBrowserGrant {
                resource: BrowserHostResourceConfig::Clipboard,
                allow: operations,
            });
        }
    }
    ...
    if let Some(value) = map.get("network") {
        ...
        for item in network {
            let ConfigValue::Map(item) = item else {
                return invalid("browser.settings.network", "items must be maps");
            };
            reject_unknown_fields(
                item,
                &["origin", "operations", "methods"],
            ...
            let methods = optional_config_string_list(
                item.get("methods"),
                "browser.settings.network.methods",
            )?;
            let scope = BrowserNetworkScope::new(&origin, methods.clone()).map_err(|error| {
                invalid_error(
    ...
    if let Some(value) = map.get("storage") {
        ...
        for item in storage {
            let ConfigValue::Map(item) = item else {
                return invalid("browser.settings.storage", "items must be maps");
            };
            reject_unknown_fields(
                item,
                &["backend", "scope", "recursive", "operations"],
                "browser.settings.storage",
            )?;
            let backend_name =
                config_string(item.get("backend"), "browser.settings.storage.backend")?;
            let scope = config_string(item.get("scope"), "browser.settings.storage.scope")?;
            let recursive = config_bool(
                item.get("recursive"),
                "browser.settings.storage.recursive",
                false,
            )?;
            let operations = config_operations(
                item.get("operations"),
            ...
            let backend = BrowserStorageBackend::parse(&backend_name).ok_or_else(|| {
                invalid_error(
                    "browser.settings.storage.backend",
                    format!("unknown storage backend `{backend_name}`"),
                )
            })?;
            BrowserStorageScope::new(backend, &scope).map_err(|error| {
                invalid_error(
...
fn config_operations(
    ...
    for operation in &operations {
        let parsed = BrowserOperation::parse(operation).ok_or_else(|| {
            invalid_error(field, format!("unknown browser operation `{operation}`"))
        })?;
...
fn optional_config_string_list(
    value: Option<&ConfigValue>,
    field: &'static str,
) -> MResult<Option<Vec<String>>> {
    match value {
...
"##,
    ...
    fn enum_string_mappings_live_on_domain_types() {
        ...
        assert_eq!(
            BrowserOperation::parse("list"),
            Some(BrowserOperation::List)
        );
        assert_eq!(BrowserResourceKind::Dom.as_str(), "dom");
        assert_eq!(
            BrowserResourceKind::parse("storage"),
            Some(BrowserResourceKind::Storage)
        );
        assert_eq!(
            BrowserDomProperty::Attribute("href".to_string()).config_name(),
            "attribute"
        );
        assert_eq!(
            BrowserDomProperty::Attribute("href".to_string()).config_attribute(),
            Some("href")
        );
        assert_eq!(
    ...
    fn browser_host_config_projects_runtime_browser_grants_and_dom_manifest() {
        ...
        let runtime_config = RuntimeConfig::default()
            ...
            .unwrap();
        let host_config =
            BrowserHostConfig::from_document_and_runtime(&document, &runtime_config).unwrap();
        ...
        assert!(
            host_config
                ...
                .dom_manifest
                .iter()
                .any(|entry| entry.path == "button/_aria-label"
                    && entry.property == "attribute"
                    && entry.attribute.as_deref() == Some("aria-label"))
        );
    }
...
"##,
        ...
        assert!(
            config
                .dom_manifest
                .iter()
                .any(|entry| entry.selector == "#default")
        );
        assert!(
            config
                .dom_manifest
                .iter()
                .any(|entry| entry.selector == "#ui")
        );
    }
...
"##,
        ...
        .unwrap();
        let injected = BrowserRuntimeInjectionConfig::from_document_and_runtime(
            &document,
    ...
    fn browser_host_config_converts_back_to_runtime_and_authority() {
        ...
        assert!(
            authority
                .allows_dom("#counter", BrowserOperation::Write)
                .is_ok()
        );
    ...
    fn browser_settings_dom_only_parses() {
        let config = browser_config_from_settings(&cfg_map(vec![(
            ...
            cfg_list(vec![cfg_map(vec![
                ...
                (
                    "operations",
                    cfg_list(vec![cfg_string("read"), cfg_string("write")]),
                ),
            ])]),
        )]))
        .unwrap();
        ...
        assert!(
            matches!(config.grants[0].resource, BrowserHostResourceConfig::Dom { ref selector } if selector == "#input")
        );
        assert_eq!(
            config.grants[0].allow,
    ...
    fn browser_settings_dom_absent_does_not_drop_other_grants() {
        ...
        assert!(
            config
                .grants
                .iter()
                .any(|grant| matches!(grant.resource, BrowserHostResourceConfig::Clipboard))
        );
        assert!(
            config
                .grants
                .iter()
                .any(|grant| matches!(grant.resource, BrowserHostResourceConfig::Network { .. }))
        );
    }
    ...
    fn browser_settings_mixed_resources_parse() {
        ...
        assert!(
            config
                .grants
                .iter()
                .any(|grant| matches!(grant.resource, BrowserHostResourceConfig::Dom { .. }))
        );
        assert!(
            config
                .grants
    ...
    fn browser_settings_unknown_key_rejected() {
        let err = browser_config_from_settings(&cfg_map(vec![("cookies", cfg_list(vec![]))]))
            .unwrap_err();
        let error = format!("{err:?}");
        assert!(
            error.contains("unknown browser settings key `cookies`"),
            "got {error}"
        );
    }
    ...
    fn browser_settings_unknown_operation_rejected() {
        let err = browser_config_from_settings(&cfg_map(vec![(
            ...
            cfg_list(vec![cfg_map(vec![(
                "operations",
                cfg_list(vec![cfg_string("teleport")]),
            )])]),
        )]))
        .unwrap_err();
        let error = format!("{err:?}");
        assert!(
            error.contains("unknown browser operation `teleport`"),
            "got {error}"
        );
    }
...
"##,
        ...
        .unwrap();
        let err =
            BrowserHostConfig::from_document_and_runtime(&document, &RuntimeConfig::default())
                .unwrap_err();
        let error = format!("{err:?}");
...
"##,
        ...
        assert!(
            injected
                .hosts
                .iter()
                .any(|host| host.name == "ui" && host.provider == "browser")
        );
        assert!(
            injected
                .hosts
...
"##,
        ...
        .unwrap();
        let err = BrowserRuntimeInjectionConfig::from_document_and_runtime(
            &document,
            &RuntimeConfig::default(),
        )
        .unwrap_err();
...
"##,
    ...
    fn browser_only_fake_profile() -> BrowserRuntimeProviderProfile {
        ...
        profile
            .register(
                fake_manifest("browser", "dom", &["read", "write"]),
                validate_map_settings,
            )
            .unwrap();
        profile
    }
    ...
    fn standard_fake_profile() -> BrowserRuntimeProviderProfile {
        ...
        profile
            .register(
                fake_manifest("time", "clock", &["read"]),
                validate_map_settings,
            )
            .unwrap();
        profile
            .register(
                fake_manifest("timer", "tick", &["read"]),
                validate_map_settings,
            )
            .unwrap();
        profile
            .register(
                fake_manifest("console", "output", &["write"]),
                validate_map_settings,
            )
            .unwrap();
        profile
            .register(
                fake_manifest("scene", "frame", &["write"]),
                validate_map_settings,
            )
            .unwrap();
        profile
...
"##,
        ...
        assert_eq!(
            injected.run_grants.as_slice(),
            &document.run.as_ref().unwrap().grants[..5]
        );
    }
  ...
  run: {{ grants: [{{ target: "{target}" operations: ["{operation}"] paths: ["second"] }}] }}
            ...
            .unwrap();
            assert!(
                BrowserRuntimeInjectionConfig::from_document_and_runtime_with_profile(
                    &document,
                    &RuntimeConfig::default(),
                    &profile,
                )
    ...
    fn profile_validates_settings_and_rejects_duplicate_providers() {
        ...
        profile
            .register(manifest.clone(), validate_empty_map_settings)
            .unwrap();
        let duplicate = profile
            .register(manifest, validate_empty_map_settings)
            .unwrap_err();
...
"#,
    ...
    fn browser_host_config_serializes_expected_shape() {
        ...
        assert_eq!(
            round_tripped.browser.dom_manifest,
            host_config.browser.dom_manifest
        );
    }
