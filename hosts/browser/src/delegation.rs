...
use mech_runtime::{
    HostDelegationHeader, HostDelegationSigningKey, HostDelegationVerificationRequest,
    sign_host_delegation, verify_host_delegation,
};
...
pub type VerifiedBrowserHostDelegation =
    VerifiedHostDelegation<BrowserHostDelegationPayload, BrowserVerifiedAuthority>;
...
pub fn sign_browser_host_delegation(
    ...
    sign_host_delegation(
        header,
        BrowserHostDelegationPayload { runtime_injection },
        signing_key,
    )
}
...
pub fn encode_browser_host_config(out: &mut Vec<u8>, config: &BrowserHostConfig) {
    ...
    entries.sort_by(|left, right| {
        (&left.path, &left.selector, &left.property, &left.attribute).cmp(&(
            &right.path,
            &right.selector,
            &right.property,
            &right.attribute,
...
pub fn encode_browser_runtime_injection_config(
    ...
    hosts.sort_by(|left, right| (&left.name, &left.provider).cmp(&(&right.name, &right.provider)));
    ...
    run_grants.sort_by(|left, right| {
        (&left.target, &left.operations, &left.paths).cmp(&(
            &right.target,
            &right.operations,
            &right.paths,
        ))
...
fn resource_sort_key(resource: &BrowserHostResourceConfig) -> String {
    match resource {
        ...
        BrowserHostResourceConfig::Storage {
            backend,
            scope,
            recursive,
        } => {
...
mod tests {
            ...
            browser: BrowserHostBrowserConfig {
                grants: vec![
                    BrowserHostBrowserGrant {
                        resource: BrowserHostResourceConfig::Dom {
    ...
    fn dom_setting(path: &str, selector: &str, property: &str, operations: &[&str]) -> ConfigValue {
        ConfigValue::Map(BTreeMap::from([
            ...
            (
                "selector".to_string(),
                ConfigValue::String(selector.to_string()),
            ),
            (
                "property".to_string(),
                ConfigValue::String(property.to_string()),
            ),
            (
                ...
                ConfigValue::List(
                    operations
                        .iter()
                        .map(|operation| ConfigValue::String((*operation).to_string()))
                        .collect(),
                ),
            ),
        ]))
    ...
    fn browser_payload_is_deterministic_when_allow_lists_are_reordered() {
        ...
        if let BrowserHostResourceConfig::Network {
            methods: Some(methods),
            ..
        } = &mut reordered.browser.grants[2].resource
        {
            methods.reverse();
        }
        assert_eq!(
    ...
    fn browser_runtime_injection_payload_includes_hosts_and_run_grants() {
        ...
        let payload = BrowserHostDelegationPayload {
            runtime_injection: config.clone(),
        };
        assert!(
            payload
                .runtime_injection
