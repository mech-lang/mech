...
impl GpuProgram {
    ...
    pub fn prepare_cpu(
        &self,
        inputs: &BTreeMap<String, Vec<f32>>,
    ) -> Result<ResidentCpuSession, CpuExecutionError> {
...
impl ResidentCpuSession {
    pub fn write_input(&mut self, name: &str, values: &[f32]) -> Result<(), CpuExecutionError> {
        let Some((slot, (_, elements, _))) = self
            ...
            return Err(CpuExecutionError::MissingInput {
                name: name.to_owned(),
            });
        };
        if values.len() != *elements as usize {
        ...
        self.slots.insert(*slot, values.to_vec());
        Ok(())
    }
...
impl<'a> Compiler<'a> {
        ...
        for node in self.artifact.nodes() {
            ...
            let Some(operation) = binary_operation(&node.operation) else {
                self.reject(
                    ...
                    Some(node.node),
                    Some(operation_name),
                    "the GPU host supports only element-wise math/add, math/sub, math/mul, and math/div",
                );
                continue;
            };
            let inputs = node
    ...
    fn admit_state_contract(
        ...
        if !admitted {
            self.reject(
                ...
                Some(node),
                Some(operation.to_owned()),
                "state Assign must be a pure whole-value write with no aliases",
            );
        }
        admitted
    }
    ...
    fn state_initializer(
        ...
        let values = match value.data() {
            ...
            ValueData::F64(value) => vec![value.to_f64() as f32],
            ValueData::Matrix(matrix) => match matrix.elements() {
                ...
                _ => {
                    return Err((
                        GpuDiagnosticCode::ConstantUnsupported,
                        "initializer is not an f32/f64 matrix".to_owned(),
                    ));
                }
            },
            _ => {
                return Err((
                    GpuDiagnosticCode::ConstantUnsupported,
                    "initializer is not scalar f32/f64 or an f32/f64 matrix".to_owned(),
                ));
            }
        };
    ...
    fn schema_elements(&self, schema: SchemaId) -> Result<u64, (GpuDiagnosticCode, String)> {
        ...
        match schema.body() {
            SchemaBody::FloatingPoint(FloatWidth::W32 | FloatWidth::W64) => Ok(1),
            SchemaBody::Matrix {
                element,
                dimensions,
            } if matches!(
            ...
            body => Err((
                GpuDiagnosticCode::SchemaUnsupported,
                format!("schema {body:?} is not scalar f32/f64 or a fixed-size f32/f64 matrix"),
            )),
        }
    }
        ...
        match source {
            ...
            ArtifactSource::Constant(constant) => {
                ...
                match value.data() {
                    ...
                    ValueData::F64(value) => {
                        self.constants.insert(constant, value.to_f64() as f32);
                        Some(1)
                    }
                    ValueData::Matrix(matrix) => match matrix.elements() {
                        SequenceView::F32(_) => {
...
fn binary_operation(operation: &OperationReference) -> Option<BinaryOperation> {
    ...
    match canonical.as_str() {
        ...
        "math/div" | "math/divide" => Some(BinaryOperation::Divide),
