//! Direct Metal execution of a fixed-shape Mech kernel.
//!
//! State remains in two component-major device buffers. Every numerical turn
//! has one completed Metal command buffer before publication.
...
use super::metal_layout::{component_major_values, instance_major_values, pack_input_values};
use super::{BatchedExecutionError, BatchedIntegrityFault, FixedShapeKernel};
...
/// keeps double-buffered state and uses the same per-turn synchronization in
/// checked and unchecked modes. Source integrity constraints add candidate
/// validation and compact fault reporting before the host publishes a turn.
    ...
    pub fn prepare_metal(
        ...
        if self.instances == 0 {
            return Err(BatchedExecutionError::Native(
                "direct Metal requires at least one instance".to_owned(),
            ));
        }
        if self.constraints.len() >= 256
            || (!self.constraints.is_empty() && self.instances > 1 << 24)
        {
            return Err(BatchedExecutionError::Native(
                "direct Metal fault encoding supports at most 255 constraints and 2^24 instances"
                    .to_owned(),
            ));
        }
        let broadcast_inputs = self
            ...
            .collect::<BTreeSet<_>>();
        let binding_count =
            self.inputs.len() + self.states.len() * 2 + usize::from(!self.constraints.is_empty());
        // Metal exposes buffer argument indices 0..31. Reserve one for Naga's
        // runtime array sizes, and keep all shader byte lengths representable.
        if binding_count >= 31 {
            return Err(BatchedExecutionError::Native(
                "direct Metal requires fewer than 31 data buffers".to_owned(),
            ));
        }
        for width in self
            .inputs
            ...
            .chain(self.states.iter().map(|state| state.shape.elements()))
        {
            let bytes = width
                .checked_mul(self.instances as usize)
                .and_then(|elements| elements.checked_mul(std::mem::size_of::<f32>()));
            if bytes.is_none_or(|bytes| bytes > u32::MAX as usize) {
                return Err(BatchedExecutionError::Native(
                    "direct Metal buffer length exceeds its u32 shader size limit".to_owned(),
                ));
            }
        }
        ...
        Ok(BatchedResidentMetalSession {
            adapter: format!("{} (direct Metal)", device.name()),
            queue,
            pipeline,
            input_buffers,
            state_buffers,
...
impl BatchedResidentMetalSession {
    ...
    /// per turn in both checked and unchecked modes.
        ...
        for _ in 0..turns {
            ...
            if command_buffer.status() != metal::MTLCommandBufferStatus::Completed {
                return Err(BatchedExecutionError::Native(format!(
                    "Metal turn did not complete successfully: {:?}",
...
fn clear_fault(buffer: &metal::Buffer) {
    assert!(buffer.length() as usize >= FAULT_WORDS * std::mem::size_of::<u32>());
    // SAFETY: The shared buffer was allocated with FAULT_WORDS aligned u32s.
    // Dispatches wait to completion, so no GPU access overlaps this write.
    let bytes =
...
fn read_u32(buffer: &metal::Buffer, count: usize) -> Vec<u32> {
    assert!(count <= buffer.length() as usize / std::mem::size_of::<u32>());
    // SAFETY: Metal shared storage is aligned for u32; count is within the
    // allocation and its command buffer completed before this host read.
    unsafe { std::slice::from_raw_parts(buffer.contents().cast::<u32>(), count).to_vec() }
}

fn read_f32(buffer: &metal::Buffer, count: usize) -> Vec<f32> {
    assert!(count <= buffer.length() as usize / std::mem::size_of::<f32>());
    // SAFETY: Metal shared storage is aligned for f32; count is within the
    // allocation and its command buffer completed before this host read.
