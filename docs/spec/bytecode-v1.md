...
A lexical operation body may also be `"Recur"`, `"Suspend"`, or `"Publish"`.
`Recur` and `Suspend` each have exactly one input with the enclosing match
scrutinee schema and produce the enclosing match result schema. `Publish` has
exactly one input with the enclosing result schema and produces that same
schema. These bodies cannot escape their owning match. `Recur` stays on admitted
resident call-frame storage. `Suspend` retains its state and captures for a later
