...
The returned `ElementwiseKernel` includes generated WGSL, its binding layout, and CPU
and GPU resident executors. Both are lowered from the same `ProgramArtifact`.
`ComputeLowerer::plan` reports every node target, GPU region, slot residence, and
upload/readback boundary. `ComputeLowerer::plan` also honors named
...
With the `native` feature, `ElementwiseKernel::run_gpu` dispatches that WGSL through
...
GPU dispatch for one turn. `ElementwiseKernel::prepare_cpu` and
`ElementwiseKernel::prepare_resident` create persistent CPU and GPU sessions. The GPU
...
`ComputeLowerer::compile_broadcast` consumes the compiler's typed
