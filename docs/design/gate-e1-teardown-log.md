...
| Failed legacy bytecode install is the only shown live-binding identity rollback owner | Auto-review rejected deleting `failed_retained_bytecode_install_restores_program_and_live_bindings` because it asserts that malformed bytecode restores the original live target address, live-context template, and binding collection | Potentially. Resident activation failure is atomic and resident input ownership is retained, but no equivalent test was identified for this exact legacy `ValRef` target-identity contract | Left the test unchanged. Continue with other bytecode replacement assertions that expose only the retired interpreter object model | E2 must decide whether any canonical resident input binding has a corresponding identity-stability contract. If it does, assert that at the resident coordinator; if not, delete this test with the legacy live-binding implementation in the complete caller-closure slice |
| Interpreter profiling tests cannot be deleted before their event producer | Auto-review rejected deleting the focused `ProgramProfiled` success/failure assertions while `execution/source.rs`, `execution/bytecode.rs`, the event variant, and runtime profiling configuration still expose that behavior | Potentially. The event is produced only by legacy source/bytecode execution, but the configuration field also feeds the ephemeral compiler environment | Left both tests unchanged. Do not delete tests ahead of still-compiled behavior; remove the producer, event variant, and tests together when the legacy execution caller closure is ready | E2 may define resident-native diagnostics separately if required, but E1 must not synthesize a replacement. Confirm no retained consumer relies on `:program/profiled` before deleting the complete legacy event closure |
...
| Mixed host checkpoint test target cannot be deleted wholesale | Auto-review rejected deleting `host/tests/checkpoint.rs`: its direct `MechProgram` checkpoint case is legacy-engine ownership, but the two source/bytecode cases also assert host planning, encoded host-call presence, reconstruction, invocation count, and returned value | Yes for production source/bytecode external-host equivalence; no for a runtime-owned interpreter checkpoint | Left the file and module declaration unchanged after the rejection. Continue by migrating the observable host/bytecode assertions to production resident compile/load APIs before deleting only the obsolete interpreter wrappers | Name and run a resident source/bytecode host test that preserves planning count, execution count, encoded operation identity, and returned value; engine-local checkpoint tests remain the owner of `MechProgram::restore` |
| Mixed provisional-module tests cannot drop execution assertions before the executor | Auto-review rejected removing the `run_module*` visibility assertions and the “another transaction owns program” case while those APIs remain compiled and tested elsewhere | Module graph/store visibility and idempotent publication are retained; coordination with `MechRuntime.program` is not | Left `provisional_visibility.rs` unchanged after the rejection. Do not weaken the mixed test ahead of deleting the full module-executor caller closure | Remove `run_module*` and the private program owner first; then preserve the graph visibility/publication assertions and delete only the now-unrepresentable interpreter coordination cases |
