#!/usr/bin/env python3
"""Reconstruct E11 solely from E10 and the declared exact frozen byte spans."""
import hashlib
import json
...
manifest = json.loads(Path(__file__).with_name("e11-symbols.json").read_text())
state = {}
...
for entry in manifest["copied"]:
    ...
    if path not in state:
        state[path] = b"" if entry["new_file"] else git_file(manifest["base"], path)
...
subprocess.run(["git", "diff", "--exit-code", manifest["frozen"], "HEAD"], cwd=root, check=True)
head_tree = subprocess.check_output(["git", "rev-parse", "HEAD^{tree}"], cwd=root, text=True).strip()
assert head_tree == manifest["frozen_tree"]
assert not subprocess.check_output(["git", "status", "--porcelain"], cwd=root, text=True)
print(f"Exact clean HEAD tree equals frozen B: {head_tree}")
