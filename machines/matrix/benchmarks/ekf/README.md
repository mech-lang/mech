...
- `julia-loop`: persistent Julia arrays and `LinearAlgebra` operations.
...
turns before measuring. The Lua and Julia scripts validate against the same
256-turn reference. The Rust harness validates all state and covariance
elements, not only the emitted checksum.
...
JULIA_NUM_THREADS=1 julia --startup-file=no \
  machines/matrix/benchmarks/ekf/julia_benchmark.jl
```
