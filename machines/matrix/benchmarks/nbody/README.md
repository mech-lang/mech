...
all scalar nodes, and atomic state staging/commit. Source parsing,
specialization, bytecode compilation, installation, and correctness validation
are untimed.
...
Rust reference. The atomic source lane passes after warmup and all timed turns.
...
On the current `integration/value-executor-v0.4` runtime, retained source Mech
measured `0.267404 ms/step` (range `0.261771` to `0.273812`). This is recorded
separately because it deliberately crosses the complete host/runtime
transaction boundary for every simulation step, while the game programs keep
their inner loops inside one language function. The bytecode fixture was not
timed: its state diverged from the Rust reference after 1,000 turns, so timing
it would measure incorrect work. The retained result is in
`RESULTS_MECH_APPLE_M1_2026-08-08.csv`.
