...
fn runtime() -> (MechRuntime, RuntimeContext) {
...
impl RuntimeFixture {
    fn source() -> (Self, Vec<u8>) {
        let (mut runtime, mut context) = runtime();
        runtime
...
fn benchmark() {
    let (mut source_check, bytecode_bytes) = RuntimeFixture::source();
    let mut bytecode_check = RuntimeFixture::bytecode(&bytecode_bytes);
    for _ in 0..1_000 {
        source_check.turn();
        bytecode_check.turn();
    }
    ...
    let (mut source, _) = RuntimeFixture::source();
    let source_result = measure(|| source.turn());
    source.validate();
    print_result("mech-runtime-source", &source_result, &source);
}
