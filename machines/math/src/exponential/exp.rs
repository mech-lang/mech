...
#[cfg(feature = "source")]
fn impl_exponential_fxn(lhs_value: LegacyValue) -> MResult<Box<dyn MechFunction>> {
...
#[cfg(feature = "source")]
impl FunctionSpecializer for MathExp {
    fn specialize(&self, arguments: &[LegacyValue]) -> MResult<Box<dyn MechFunction>> {
        ...
        match impl_exponential_fxn(input.clone()) {
            Ok(fxn) => Ok(fxn),
            Err(_) => match input {
                LegacyValue::MutableReference(input) => impl_exponential_fxn(input.borrow().clone()),
                x => Err(MechError::new(
