...
fn impl_log2_fxn(lhs_value: Value) -> MResult<Box<dyn MechFunction>> {
      ...
      Err(_) => {
        match (input) {
          ...
          x => Err(MechError2::new(
              UnhandledFunctionArgumentKind1 { arg: x.kind(), fxn_name: "math/log2".to_string() },
              None
            ).with_compiler_loc()
          ),
        }
