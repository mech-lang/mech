            ...
            fn solve_managed(
                ...
                frame.with_binary_port_views(&self.lhs, &self.rhs, &self.out, |lhs, rhs, out| {
                    ...
                    out.try_fill_column_major(|index| {
                        ...
                        let lhs = crate::managed_binary::managed_broadcast_element(&lhs, row, column, rows, columns)?;
                        let rhs = crate::managed_binary::managed_broadcast_element(&rhs, row, column, rows, columns)?;
                        $op!(@managed lhs, rhs)
                    })
                })?;
