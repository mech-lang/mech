...
#[cfg(feature = "source")]
fn impl_trunc_fxn(lhs_value: LegacyValue) -> MResult<Box<dyn MechFunction>> {
...
#[cfg(feature = "source")]
impl FunctionSpecializer for MathTrunc {
    fn specialize(&self, arguments: &[LegacyValue]) -> MResult<Box<dyn MechFunction>> {
        ...
        match impl_trunc_fxn(input.clone()) {
            Ok(fxn) => Ok(fxn),
            Err(_) => match input {
                LegacyValue::MutableReference(input) => impl_trunc_fxn(input.borrow().clone()),
                x => Err(MechError::new(
                    UnhandledFunctionArgumentKind1 {
                        arg: x.kind(),
                        fxn_name: "math/trunc".to_string(),
                    },
                    None,
                )
                .with_compiler_loc()),
