...
`mech-math` separates concrete execution (`runtime`), source specializers
...
Every leaf operation enables `runtime`. For example, a minimal scalar-add
runtime build uses `--no-default-features --features "runtime,f64,add"`.
