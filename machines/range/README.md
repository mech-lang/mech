...
`mech-range` separates concrete execution (`runtime`), source specializers
...
Every leaf operation enables `runtime`. For example, a minimal inclusive-range
runtime build uses:

```text
--no-default-features --features "runtime,f64,row_vectord,inclusive"
```
