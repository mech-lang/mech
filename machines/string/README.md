...
`mech-string` separates concrete execution (`runtime`), source specializers
...
Every leaf operation enables `runtime`. For example, a minimal concatenation
runtime build uses `--no-default-features --features "runtime,string,concat"`.
