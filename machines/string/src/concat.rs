...
macro_rules! concat_op {
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {*$out = (*$lhs).clone() + (*$rhs).clone();}
  };}
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for i in 0..lhs_deref.len() {
        out_deref[i] = lhs_deref[i].clone() + rhs_deref[i].clone();
      }
    }
  };}
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for i in 0..rhs_deref.len() {
        out_deref[i] = (*lhs_deref).clone() + rhs_deref[i].clone();
      }
    }
  };}
...
macro_rules! concat_scalar_lhs_op {
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      let mut out_deref = &mut (*$out);
      for i in 0..lhs_deref.len() {
        out_deref[i] = lhs_deref[i].clone() + (*rhs_deref).clone();
      }
    }
  };}
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for (mut col, lhs_col) in out_deref.column_iter_mut().zip(lhs_deref.column_iter()) {
        for i in 0..col.len() {
          col[i] = lhs_col[i].clone() + rhs_deref[i].clone();
        }
      }
    }
  };}   
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for (mut col, rhs_col) in out_deref.column_iter_mut().zip(rhs_deref.column_iter()) {
        for i in 0..col.len() {
          col[i] = lhs_deref[i].clone() + rhs_col[i].clone();
        }
      }
    }
  };}
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for (mut row, lhs_row) in out_deref.row_iter_mut().zip(lhs_deref.row_iter()) {
          for i in 0..row.len() {
          row[i] = lhs_row[i].clone() + rhs_deref[i].clone();
          }
      }
    }
  };}
  ...
  ($lhs:expr, $rhs:expr, $out:expr) => {
    unsafe {
      ...
      for (mut row, rhs_row) in out_deref.row_iter_mut().zip(rhs_deref.row_iter()) {
          for i in 0..row.len() {
          row[i] = lhs_deref[i].clone() + rhs_row[i].clone();
          }
      }
    }
  };} 
        
impl_fxns!(Concat,String,String,impl_logic_binop);
