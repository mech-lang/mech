    ...
    paste!{
      match $arg {
        ...
        x => Err(MechError2::new(
            UnhandledFunctionArgumentKind1 { arg: x.kind(), fxn_name: "io/print".to_string() },
            None
          ).with_compiler_loc()
        ),      }
    }
...
fn impl_print_fxn(source_value: Value) -> MResult<Box<dyn MechFunction>>  {
  if source_value.is_scalar() {
    match source_value {
      ...
      #[cfg(feature = "complex")]
      Value::C64(value) => { return Ok(Box::new(IoPrintScalar { e0: value })); }
      #[cfg(feature = "rational")]
      Value::R64(value) => { return Ok(Box::new(IoPrintScalar { e0: value })); }
      _ => todo!(),
    }
  }
