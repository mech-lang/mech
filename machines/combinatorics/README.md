...
`mech-combinatorics` separates concrete execution (`runtime`), source
specializers (`source`), and bytecode lowering (`compiler`). `source` and
`compiler` each require `runtime`, but `compiler` does not enable `source`.
Enable both when a consumer needs source elaboration and lowering.

Every leaf operation enables `runtime`. For example, a minimal scalar
n-choose-k runtime build uses:

```text
--no-default-features --features "runtime,f64,n_choose_k"
```

This profile does not require a matrix shape. `runtime_default`,
`source_default`, and `compiler_default` select the complete standalone
profiles; the crate default is `compiler_default`.
