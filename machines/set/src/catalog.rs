...
mod runtime_tests {
    ...
    #[test]
    fn explicit_runtime_factories_match_the_linked_set_inventory() {
        ...
        for descriptor in inventory::iter::<FunctionDescriptor>
            .into_iter()
            .filter(|descriptor| {
                descriptor.name.starts_with("Set") && descriptor.name.ends_with("Fxn")
            })
        {
        ...
        for entry in catalog.runtime_entries() {
            let legacy_factory = legacy
                ...
                .unwrap_or_else(|| panic!("missing legacy set factory {}", entry.name));
            assert_eq!(
        ...
        assert!(
            legacy.is_empty(),
            "unmigrated legacy set factories: {legacy:?}"
        );
    }
}
