...
`mech-compare` separates concrete execution (`runtime`), source specializers
...
Every leaf operation enables `runtime`. For example, a minimal less-than
runtime build uses `--no-default-features --features "runtime,bool,f64,lt"`.
