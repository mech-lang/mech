...
use crate::host::{
  ...
  BrowserNetworkScope, BrowserOperation, BrowserResource, BrowserResourceKind, BrowserStorageBackend, BrowserStorageScope,
...
fn parse_host_dom_property(entry: &JsValue) -> Result<BrowserDomProperty, JsValue> {
  ...
  let attribute = js_optional_string_field(entry, "attribute")?;
  BrowserDomProperty::parse_config_name(&property, attribute.as_deref())
    .map_err(|error| js_error(format!("invalid host DOM property: {error}")))
}

fn parse_host_browser_resource(resource: &JsValue) -> Result<BrowserResource, JsValue> {
  let kind = js_string_field(resource, "kind")?;
  match BrowserResourceKind::parse(&kind).ok_or_else(|| js_error(format!("unknown host browser resource kind `{kind}`")))? {
    BrowserResourceKind::Dom => Ok(BrowserResource::Dom(
      BrowserDomScope::new(js_string_field(resource, "selector")?)
        .map_err(|error| js_error(format!("invalid host DOM selector: {error}")))?,
    )),
    BrowserResourceKind::Clipboard => Ok(BrowserResource::Clipboard),
