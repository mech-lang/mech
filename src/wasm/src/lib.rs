...
use mech_host_browser::{BrowserHostFactory, BrowserRuntimeInjectionConfig};
use mech_runtime::{
    ConfigProfileOptions, ConfigValue, HostInstanceConfig, MechConfigDocument, MechRuntime,
    RuntimeBuilder, RuntimeConfig, parse_config_document,
};
#[cfg(feature = "host_delegation_signing")]
use mech_runtime::{
    HostDelegationKeyStore, HostDelegationPublicKey, HostDelegationVerificationRequest,
};
use mech_syntax::*;
#[cfg(feature = "host_delegation_signing")]
...
use web_sys::{
    Element, HashChangeEvent, HtmlElement, HtmlInputElement, HtmlTextAreaElement, Node, Url, window,
};
...
fn browser_storage_for_backend(backend: &str) -> Result<web_sys::Storage, JsValue> {
    let window = web_sys::window().ok_or_else(|| js_error("global window does not exist"))?;
...
fn trusted_host_key_store_from_js(value: JsValue) -> Result<HostDelegationKeyStore, JsValue> {
    ...
    for key in keys {
        let public_key = base64::engine::general_purpose::STANDARD
            ...
            .map_err(|error| {
                js_error(format!(
                    "failed to decode trusted host key `{}`: {error}",
                    key.key_id
                ))
            })?;
...
fn wasm_parts_from_delegated_host_config(
    ...
    let envelope: BrowserHostDelegationEnvelope =
        serde_wasm_bindgen::from_value(config).map_err(|error| {
            js_error(format!(
                "failed to deserialize host delegation envelope: {error}"
            ))
        })?;
...
fn wasm_parts_from_runtime_injection_config_result(
    ...
    let mut builder = RuntimeBuilder::new()
        ...
        .host_factory(Box::new(BrowserHostFactory::new(
...
impl WasmMech {
    ...
    pub fn from_host_config() -> Result<WasmMech, JsValue> {
        let config =
            js_sys::Reflect::get(&js_sys::global(), &JsValue::from_str("__MECH_HOST_CONFIG"))
    ...
    pub fn from_delegated_host_config(
        expected_audience: Option<String>,
    ) -> Result<WasmMech, JsValue> {
        let config =
            js_sys::Reflect::get(&js_sys::global(), &JsValue::from_str("__MECH_HOST_CONFIG"))
                .map_err(|error| {
                    js_error(format!("failed to read delegated host config: {error:?}"))
                })?;
        if config.is_undefined() || config.is_null() {
            return Err(js_error(
                "delegated host config was not provided by mech serve",
            ));
        }
        let trusted_keys = js_sys::Reflect::get(
            &js_sys::global(),
            &JsValue::from_str("__MECH_TRUSTED_HOST_KEYS"),
        )
        .map_err(|error| js_error(format!("failed to read trusted host keys: {error:?}")))?;
        if trusted_keys.is_undefined() || trusted_keys.is_null() {
            return Err(js_error(
                "trusted host keys were not provided for delegated host config",
            ));
        }
        let expected_audience = match expected_audience {
            ...
            _ => {
                ...
                .map_err(|error| {
                    js_error(format!(
                        "failed to read host delegation audience: {error:?}"
                    ))
                })?;
                value
                    .as_string()
    ...
    fn try_from_config(source_name: &str, source: &str) -> MResult<Self> {
        let document = parse_config_document(source_name, source, ConfigProfileOptions::default())?;
    ...
    pub fn read_dom_text(&self, selector: &str) -> Result<String, JsValue> {
        self.check_dom(selector, BrowserOperation::Read)
        ...
        let element = document
            ...
            .ok_or_else(|| {
                js_error(format!(
                    "DOM selector `{selector}` did not match an element"
                ))
            })?;
    ...
    pub fn write_dom_text(&self, selector: &str, text: &str) -> Result<(), JsValue> {
        self.check_dom(selector, BrowserOperation::Write)
            .map_err(browser_host_error_to_js)?;
    ...
    pub async fn fetch_text(&self, url: &str, method: Option<String>) -> Result<String, JsValue> {
        ...
        self.check_network(&origin, Some(&method), BrowserOperation::Read)
            .map_err(browser_host_error_to_js)?;
    ...
    pub fn read_storage_text(&self, backend: &str, scope: &str) -> Result<Option<String>, JsValue> {
        self.check_storage(backend, scope, BrowserOperation::Read)
            .map_err(browser_host_error_to_js)?;
    ...
    pub fn write_storage_text(
        ...
        self.check_storage(backend, scope, BrowserOperation::Write)
            .map_err(browser_host_error_to_js)?;
    ...
    pub fn remove_storage_item(&self, backend: &str, scope: &str) -> Result<(), JsValue> {
        self.check_storage(backend, scope, BrowserOperation::Write)
            .map_err(browser_host_error_to_js)?;
    ...
    pub fn can_list_storage(&self, backend: &str, scope: &str) -> bool {
        self.check_storage(backend, scope, BrowserOperation::List)
            .is_ok()
    }
    ...
    fn check_dom(
        ...
        BrowserDomScope::new(selector.to_string()).map_err(|error| {
            BrowserHostError::BrowserDeniedOrUnavailable {
    ...
    fn check_network(
        ...
        BrowserNetworkScope::new(origin.to_string(), methods).map_err(|error| {
            BrowserHostError::BrowserDeniedOrUnavailable {
                reason: error.to_string(),
            }
        })?;
    ...
    fn check_storage(
        ...
        let parsed_backend = BrowserStorageBackend::parse(backend).ok_or_else(|| {
            BrowserHostError::BrowserDeniedOrUnavailable {
                reason: format!("unknown storage backend `{backend}`"),
            }
        })?;
        BrowserStorageScope::new(parsed_backend, scope.to_string()).map_err(|error| {
            BrowserHostError::BrowserDeniedOrUnavailable {
                reason: error.to_string(),
            }
        })?;
                                    ...
                                    unsafe {
                                        ...
                                        if !mech.repl_history.is_empty() {
                                            let new_index = match mech.repl_history_index {
                                                ...
                                                None => {
                                                    Some(mech.repl_history.len().saturating_sub(1))
                                                }
                                                Some(0) => Some(0),
                                                _ => None,
                                            };
                                            if let Some(i) = new_index {
                                                input_for_closure
                                                    .set_text_content(Some(&mech.repl_history[i]));
                                        ...
                                        unsafe {
                                            ...
                                            if !mech.repl_history.is_empty() {
                                                let new_index = match mech.repl_history_index {
                                                    Some(i) if i > 0 => Some(i - 1),
                                                    None => Some(
                                                        mech.repl_history.len().saturating_sub(1),
                                                    ),
                                                    Some(0) => Some(0),
                                                    _ => None,
                                                };
    ...
    "<div class=\"mech-output-value\">Enter <code class=\"mech-inline-code\">:help</code> for a list of all commands.</div>",
                        ...
                        if let Some(repl_id) = &mech.repl_id {
                            if let Some(doc) = web_sys::window().unwrap().document() {
                                if let Some(container) = doc.get_element_by_id(repl_id) {
                                    ...
                                    {
                                        let input =
                                            input.dyn_into::<web_sys::HtmlElement>().unwrap();
                                        ...
                                        if let Some(old_input) =
                                            doc.get_element_by_id("repl-active-input")
                                        {
                                            let old_input_parent = old_input
                                                .parent_node()
                                                .expect("input should have a parent");
                                            ...
                                            old_input_parent
                                                .replace_child(&input_span, &old_input)
                                                .unwrap();
                                        }
        ...
        window
            .add_event_listener_with_callback(
                "hashchange",
                hashchange_closure.as_ref().unchecked_ref(),
            )
            .unwrap();
    ...
    pub fn eval(&mut self, input: &str) -> String {
        if input.chars().nth(0) == Some(':') {
            ...
            match parse_repl_command(&input.to_string()) {
                Ok((_, repl_command)) => execute_repl_command(repl_command),
                Err(x) => {
    ...
    pub fn eval_compiled(&mut self, input: &str) -> String {
        match decode_and_decompress::<Program>(input) {
            ...
            Err(err) => {
                Self::format_eval_error_html(format!("failed to decode compiled Mech code: {err}"))
            }
        }
    }
    ...
    pub fn render_codeblock_output_values(&mut self) {
        ...
        if let Ok(programs) = programs {
            for i in 0..programs.length() {
                ...
                let interpreter_id: String =
                    program_el.get_attribute("mech-interpreter-id").unwrap();
                ...
                if let Ok(output_elements) = output_elements {
                    for j in 0..output_elements.length() {
                        let block_node =
                            output_elements.item(j).expect("No output element at index");
                        ...
                        let output = match self
                            .runtime
                            .output_value_for_interpreter(effective_interpreter_id, output_id)
                        {
                            Some(value) => value,
                            None => {
    ...
    pub fn render_inline_values(&mut self) {
        ...
        for j in 0..inline_elements.length() {
            ...
            let inline_output = match self
                .runtime
                .output_value_for_interpreter(inline_interpreter_id, inline_output_id)
            {
                Some(value) => value,
                None => {
                    log!(
            ...
            if is_scalar {
                ...
                let inline_html = format!(
                    ...
                    compact, inline_output_id, inline_interpreter_id
                );
        ...
        for j in 0..var_elements.length() {
            ...
            let output = match self
                .runtime
                .output_value_for_interpreter(interpreter_id, var_id)
            {
                Some(value) => value,
                None => {
                    log!(
            ...
            } else if existing_class
                .split_whitespace()
                .any(|name| name == "mech-clickable")
            {
                existing_class
            } else {
    ...
    #[cfg(all(
        feature = "inline_output_values",
        feature = "clickable_symbol_listeners"
    ))]
    #[wasm_bindgen]
                ...
                if let Some(output_value) = output {
                    ...
                    CURRENT_MECH.with(|mech_ref| {
                        if let Some(ptr) = *mech_ref.borrow() {
                            unsafe {
                                ...
                                let _ = mech
                                    .runtime
                                    .bind_ans_for_interpreter(interpreter_id, &output_value);
                            }
                        }
                    });
                    ...
                    if repl_width == 0 {
                        ...
                        modal
                            .set_attribute(
                                "style",
                                &format!("position:absolute; top:{}px; left:{}px;", y, x),
                            )
                            .unwrap();
                        document.body().unwrap().append_child(&modal).unwrap();
                        let modal_clone = modal.clone();
                    ...
                    if let Some(last_child) = last_child.clone() {
                        mech_output
                            .insert_before(&prompt_line, Some(&last_child))
                            .unwrap();
                    } else {
                        mech_output.append_child(&prompt_line).unwrap();
                    }
                    ...
                    if let Some(last_child) = last_child {
                        mech_output
                            .insert_before(&result_line, Some(&last_child))
                            .unwrap();
                    } else {
                        mech_output.append_child(&result_line).unwrap();
                    }
    ...
    fn emit_runtime_error(&self, error: &MechError) {
        ...
        if !rendered_to_page {
            web_sys::console::error_1(
                &format!("Runtime error: {}", error.full_chain_message()).into(),
            );
        }
    }
            ...
            Err(err) => match parse(src) {
                ...
                Err(parse_err) => {
                    self.emit_runtime_error(
                        &MechError::new(
                            GenericError {
                                msg: format!("Error parsing program: {:?}", parse_err),
                            },
                            None,
                        )
                        .with_compiler_loc(),
        ...
        match parser::parse(&doc_mec) {
            Ok(tree) => {
                ...
                let output_element = document
                    ...
                    .expect("REPL output element not found");
                ...
                if len >= 2 {
                    let repl_result = children
                        .item(len - 2)
                        .expect("Failed to get second-to-last child");
                    repl_result
                        .set_attribute("mech-interpreter-id", "0")
                        .unwrap();
                    let repl_html = repl_result
                        .dyn_ref::<HtmlElement>()
                        .expect("Expected an HtmlElement");
                    ...
                    web_sys::console::log_1(
                        &"Not enough children in #mech-output to update.".into(),
                    );
                }
            }
            Err(err) => {
...
async fn fetch_docs(doc: &str) -> String {
    ...
    if parts.len() >= 2 {
        ...
        let doc = parts[1];
        let url = format!(
            "https://raw.githubusercontent.com/mech-machines/{}/main/docs/{}.mec",
            machine, doc
        );
        match Request::get(&url).send().await {
            Ok(response) => match response.text().await {
                ...
                Err(e) => {
                    web_sys::console::log_1(
                        &format!("Error reading response text: {:?}", e).into(),
                    );
                    "".to_string()
                }
            },
...
mod tests {
    ...
    const RC4_HTML_PAYLOAD: &str =
        "</span><img src=x onerror=\"window.pwned=1\">&\"'\n__MECH_ERROR_HTML__:";
    ...
    fn wasm_output_does_not_trust_error_html_prefix() {
        ...
        assert!(html.contains("window.pwned=1"), "{html}");
...
"#;
  ...
  hosts: [
    {
      ...
      settings: {
        dom: [
          {path: "out/_text" selector: "#mech-output" property: "text" operations: ["write"]}
        ]
      }
    }
  ]
  run: {
    grants: [
      {target: "browser/dom" operations: ["write"] paths: ["out/_text"]}
    ]
  }
}
"##;
...
"##;
  ...
  hosts: [
    {
      ...
      settings: {
        dom: [
          {path: "counter/_text" selector: "#counter" property: "text" operations: ["write"]}
        ]
      }
    }
  ]
  run: {
    grants: [
      {target: "browser/dom" operations: ["write"] paths: ["counter/_text"]}
    ]
  }
}
"##;
  ...
  hosts: [
    {
      ...
      settings: {
        network: [
          {origin: "https://example.com" methods: ["GET"] operations: ["read"]}
        ]
      }
    }
  ]
  ...
  hosts: [
    {
      ...
      settings: {
        storage: [
          {backend: "opfs" scope: "/workspace" operations: ["read", "write", "list"]}
        ]
      }
    }
  ]
  ...
  hosts: [
    {
      ...
      settings: {
        storage: [
          {backend: "opfs" scope: "/workspace" recursive: true operations: ["read", "write", "list"]}
        ]
      }
    }
  ]
...
"#;
    ...
    fn browser_config_dom_read_installs_runtime_grant() {
        ...
        assert!(mech.can_read_dom("#counter"));
        assert!(
            mech.runtime_injection
                .run_grants
                .iter()
                .any(|grant| grant.target == "browser/dom")
        );
        #[cfg(target_arch = "wasm32")]
        {
            let result = mech.runtime.run_string(
                "+> @ui := browser/dom
...
",
            ...
            if let Err(error) = result {
                ...
                assert!(
                    ...
                    "read should pass runtime grant preflight: {error}"
                );
            }
        }
    }
    ...
    fn browser_config_without_dom_read_runtime_grant_denies() {
        ...
        let result = mech
            .runtime
            .run_string("+> @ui := browser/dom\ntitle := @ui/counter/_text\n");
        let error = format!("{:?}", result.err().unwrap());
        assert!(
            error.contains("RuntimeCapabilityGrantDenied"),
            "read should be denied without read grant: {error}"
        );
    }
    ...
    fn browser_config_dom_grants_are_operation_specific() {
        ...
        let read_result = write_only
            .runtime
            .run_string("+> @ui := browser/dom\ntitle := @ui/counter/_text\n");
        let read_error = format!("{:?}", read_result.err().unwrap());
        assert!(
            read_error.contains("RuntimeCapabilityGrantDenied"),
            "write-only grant must not allow reads: {read_error}"
        );
        ...
        assert!(
            write_error.contains("RuntimeCapabilityGrantDenied"),
            "read-only grant must not allow writes: {write_error}"
        );
    }
...
"#;
        ...
        assert!(
            mech.runtime_injection
                .hosts
                .iter()
                .all(|host| host.provider == "browser")
        );
    }
...
",
            ...
            if let Err(error) = result {
                let error = format!("{error:?}");
                assert!(
                    !error.contains("HostInterfaceUnknownInstance"),
                    "ui alias should survive clear: {error}"
                );
                assert!(
                    !error.contains("RuntimeResourceProviderNotFound"),
                    "ui provider should survive clear: {error}"
                );
                assert!(
                    !error.contains("RuntimeCapabilityGrantDenied"),
                    "ui read grant should survive clear: {error}"
                );
            }
        }
    }
...
"##;
        ...
        let write_result = mech
            .runtime
            .run_string("+> @ui := browser/dom\n@ui/counter/_text = \"hello\"\n");
        let write_error = format!("{:?}", write_result.err().unwrap());
        assert!(
            write_error.contains("RuntimeCapabilityGrantDenied"),
            "clear must preserve narrowed run grants: {write_error}"
        );
    }
