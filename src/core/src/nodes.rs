...
impl fmt::Display for ModuleImportPathSegment {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ModuleImportPathSegment::Intrinsic(segment) => {
                write!(f, "_{}", segment.name.to_string())
            }
        }
    }
}
                ...
                for arm in arms {
                    ...
                    match &arm.body {
                        ActivationArmBody::Block(body) => {
                            append_activation_block_tokens(&mut tokens, body)
                        }
...
impl Transition {
    pub fn tokens(&self) -> Vec<Token> {
        match self {
            ...
            Transition::CodeBlock(code) => code
                .iter()
                .flat_map(|(mech_code, comment)| {
...
impl Recoverable for Paragraph {
    fn error_placeholder(skipped_tokens: Token, range: SourceRange) -> Self {
        Paragraph {
            elements: vec![ParagraphElement::Text(Token::new(
                TokenKind::Error,
                range.clone(),
                skipped_tokens.chars.clone(),
            ))],
...
impl Number {
    pub fn from_integer(x: i64) -> Number {
        Number::Real(RealNumber::Integer(Token::new(
            TokenKind::Digit,
            SourceRange::default(),
...
impl OpAssignOp {
    pub fn tokens(&self) -> Vec<Token> {
        match self {
            OpAssignOp::Add => vec![Token::new(
                TokenKind::Plus,
                SourceRange::default(),
                vec!['+'],
            )],
            OpAssignOp::Div => vec![Token::new(
                TokenKind::Slash,
                SourceRange::default(),
                vec!['/'],
            )],
            OpAssignOp::Exp => vec![Token::new(
                TokenKind::Caret,
                SourceRange::default(),
                vec!['^'],
            )],
            OpAssignOp::Mod => vec![Token::new(
                TokenKind::Percent,
                SourceRange::default(),
                vec!['%'],
            )],
            OpAssignOp::Mul => vec![Token::new(
                TokenKind::Asterisk,
                SourceRange::default(),
                vec!['*'],
            )],
            OpAssignOp::Sub => vec![Token::new(
                TokenKind::Dash,
                SourceRange::default(),
                vec!['-'],
            )],
