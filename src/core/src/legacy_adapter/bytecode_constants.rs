//! Compatibility-only decoding of bytecode constants into legacy values.
...
impl ConstantCodecContext {
    fn decode_child(&mut self, ty: &RuntimeType, bytes: &[u8]) -> MResult<LegacyValue> {
        if self.depth >= MAX_CONSTANT_NESTING {
            return Err(super::super::depth_exceeded(MAX_CONSTANT_NESTING));
        }
        self.depth += 1;
...
pub(crate) fn decode_encoded_legacy_constants_for_adapter(
...
#[allow(
    dead_code,
    reason = "retained only for explicit legacy bytecode compatibility tests"
)]
fn decode_constant(
    types: &[RuntimeType],
    entry: &ConstantEntry,
