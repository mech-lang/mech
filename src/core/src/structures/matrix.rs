...
pub trait CopyMat<T> {
    ...
    fn reactive_cell_id(&self) -> CanonicalCellId;
...
where
    T: Debug + Display + Clone + PartialEq + 'static + PrettyPrint,
{
    fn pretty_print(&self) -> String {
        let mut builder = Builder::default();
        match self {
            #[cfg(feature = "row_vector4")]
            Matrix::RowVector4(vec) => {
                let vec_brrw = vec.borrow();
                (0..vec_brrw.nrows()).for_each(|i| {
                    builder.push_record(
                        vec_brrw
                            .row(i)
                            .iter()
            ...
            Matrix::RowDVector(vec) => {
                ...
                let vec_str = if vec_brrw.ncols() > 20 {
                    let mut vec_str = vec_brrw
                        .row(0)
                        .iter()
                        .take(10)
                        .chain(vec_brrw.row(0).iter().rev().take(9).rev())
                        .map(|x| x.pretty_print())
                        .collect::<Vec<_>>();
            ...
            Matrix::DVector(vec) => {
                ...
                let vec_str = if vec_brrw.nrows() > 20 {
                    let mut vec_str = vec_brrw
                        ...
                        .iter()
                        .take(10)
                        .chain(vec_brrw.column(0).iter().rev().take(9).rev())
                        .map(|x| x.pretty_print())
                        .collect::<Vec<_>>();
                    vec_str.insert(10, "…".to_string());
                    vec_str
...
fn quoted<T: Display + Any>(val: &T) -> String {
    if let Some(s) = (val as &dyn Any).downcast_ref::<String>() {
        format!(
            "<div class='mech-string'>\"{}\"</div>",
            crate::escape_html_text(s)
        )
    ...
    T: Debug + Display + Clone + PartialEq + 'static + PrettyPrint,
{
    pub fn to_html(&self) -> String {
        ...
        format!(
            "<div class='mech-matrix-outer'><div class='mech-matrix-inner'></div>{}</div>",
            html
        )
    }
}
    ...
    T: CompileConst + ConstElem + AsValueKind + Debug + PartialEq,
{
    pub fn get_copyable_matrix(&self) -> Box<dyn CopyMat<T>> {
        match self {
            #[cfg(feature = "row_vector4")]
            Matrix::RowVector4(x) => Box::new(x.clone()),
            #[cfg(feature = "row_vector3")]
            Matrix::RowVector3(x) => Box::new(x.clone()),
            #[cfg(feature = "row_vector2")]
            Matrix::RowVector2(x) => Box::new(x.clone()),
            #[cfg(feature = "row_vectord")]
            Matrix::RowDVector(x) => Box::new(x.clone()),
            #[cfg(feature = "vector4")]
            Matrix::Vector4(x) => Box::new(x.clone()),
            #[cfg(feature = "vector3")]
            Matrix::Vector3(x) => Box::new(x.clone()),
            #[cfg(feature = "vector2")]
            Matrix::Vector2(x) => Box::new(x.clone()),
            #[cfg(feature = "vectord")]
            Matrix::DVector(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix4")]
            Matrix::Matrix4(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix3")]
            Matrix::Matrix3(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix2")]
            Matrix::Matrix2(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix1")]
            Matrix::Matrix1(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix3x2")]
            Matrix::Matrix3x2(x) => Box::new(x.clone()),
            #[cfg(feature = "matrix2x3")]
...
impl<T: 'static> Matrix<T> {
    ...
    /// Returns the logical reactive identity shared by clones of this matrix
    /// handle, independent of its allocation address.
    pub fn reactive_cell_id(&self) -> crate::CanonicalCellId {
        match self {
            #[cfg(feature = "matrix1")]
            Matrix::Matrix1(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrix2")]
            Matrix::Matrix2(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrix3")]
            Matrix::Matrix3(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrix4")]
            Matrix::Matrix4(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrix2x3")]
            Matrix::Matrix2x3(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrix3x2")]
            Matrix::Matrix3x2(x) => x.reactive_cell_id(),
            #[cfg(feature = "vector2")]
            Matrix::Vector2(x) => x.reactive_cell_id(),
            #[cfg(feature = "vector3")]
            Matrix::Vector3(x) => x.reactive_cell_id(),
            #[cfg(feature = "vector4")]
            Matrix::Vector4(x) => x.reactive_cell_id(),
            #[cfg(feature = "row_vector2")]
            Matrix::RowVector2(x) => x.reactive_cell_id(),
            #[cfg(feature = "row_vector3")]
            Matrix::RowVector3(x) => x.reactive_cell_id(),
            #[cfg(feature = "row_vector4")]
            Matrix::RowVector4(x) => x.reactive_cell_id(),
            #[cfg(feature = "vectord")]
            Matrix::DVector(x) => x.reactive_cell_id(),
            #[cfg(feature = "row_vectord")]
            Matrix::RowDVector(x) => x.reactive_cell_id(),
            #[cfg(feature = "matrixd")]
            Matrix::DMatrix(x) => x.reactive_cell_id(),
        }
    }
}
    ...
    T: Debug + Clone + PartialEq + 'static,
{
    ...
    pub fn from_element(rows: usize, cols: usize, element: T) -> Matrix<T> {
        match (rows, cols) {
            #[cfg(feature = "matrix1")]
            (1, 1) => Matrix::Matrix1(Ref::new(Matrix1::from_element(element.clone()))),
            #[cfg(all(feature = "matrixd", not(feature = "matrix1")))]
            (1, 1) => Matrix::DMatrix(Ref::new(DMatrix::from_element(1, 1, element.clone()))),
            #[cfg(feature = "matrix2")]
            (2, 2) => Matrix::Matrix2(Ref::new(Matrix2::from_element(element.clone()))),
            #[cfg(feature = "matrix3")]
            (3, 3) => Matrix::Matrix3(Ref::new(Matrix3::from_element(element.clone()))),
            #[cfg(feature = "matrix4")]
            (4, 4) => Matrix::Matrix4(Ref::new(Matrix4::from_element(element.clone()))),
            #[cfg(feature = "matrix2x3")]
            (2, 3) => Matrix::Matrix2x3(Ref::new(Matrix2x3::from_element(element.clone()))),
            #[cfg(feature = "matrix3x2")]
            (3, 2) => Matrix::Matrix3x2(Ref::new(Matrix3x2::from_element(element.clone()))),
            #[cfg(feature = "vector4")]
            (4, 1) => Matrix::Vector4(Ref::new(Vector4::from_element(element.clone()))),
            #[cfg(feature = "vector3")]
            (3, 1) => Matrix::Vector3(Ref::new(Vector3::from_element(element.clone()))),
            #[cfg(feature = "vector2")]
            (2, 1) => Matrix::Vector2(Ref::new(Vector2::from_element(element.clone()))),
            #[cfg(feature = "row_vector4")]
            (1, 4) => Matrix::RowVector4(Ref::new(RowVector4::from_element(element.clone()))),
            #[cfg(feature = "row_vector3")]
            (1, 3) => Matrix::RowVector3(Ref::new(RowVector3::from_element(element.clone()))),
            #[cfg(feature = "row_vector2")]
            (1, 2) => Matrix::RowVector2(Ref::new(RowVector2::from_element(element.clone()))),
            #[cfg(feature = "row_vectord")]
            (1, n) => Matrix::RowDVector(Ref::new(RowDVector::from_element(n, element.clone()))),
            #[cfg(feature = "vectord")]
...
impl MechErrorKind for IncompatibleMatrixAppendToTableError {
    ...
    fn message(&self) -> String {
        format!(
            ...
            self.lhs_rows, self.lhs_cols, self.rhs_rows, self.rhs_cols
        )
    }
}
