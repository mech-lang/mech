...
impl MechRecord {
    ...
    pub fn check_record_schema(&self, record: &MechRecord) -> MResult<()> {
        for (&field_id, _value) in &self.data {
            ...
            if !record.data.contains_key(&field_id) {
                return Err(MechError::new(MissingFieldInRecordError { field_id }, None)
                    .with_compiler_loc());
            }
            // Get expected kind
            let expected_kind = self.kinds.get(self.key_index(field_id)?).ok_or_else(|| {
                MechError::new(MissingKindForFieldError { field_id }, None).with_compiler_loc()
            })?;
            // Get actual kind
            let actual_kind = record
                .kinds
                .get(record.key_index(field_id)?)
                .ok_or_else(|| {
                    MechError::new(MissingKindInComparedRecordError { field_id }, None)
                        .with_compiler_loc()
                })?;
            // Compare kinds
    ...
    fn key_index(&self, field_id: u64) -> MResult<usize> {
        self.data
            ...
            .ok_or_else(|| {
                MechError::new(KeyNotFoundInKeyIndexError { field_id }, None).with_compiler_loc()
            })
    }
...
impl PrettyPrint for MechRecord {
    fn pretty_print(&self) -> String {
        ...
        for (k, v) in &self.data {
            ...
            lines.push(format!("{}{}", field_indent, rendered_value));
        }
...
impl MechErrorKind for MissingKindForFieldError {
    ...
    fn message(&self) -> String {
        format!(
            "Missing expected kind for field `{}` in schema.",
            self.field_id
        )
    }
}
...
impl MechErrorKind for MissingKindInComparedRecordError {
    ...
    fn message(&self) -> String {
        format!(
            "Missing kind for field `{}` in the compared record.",
            self.field_id
        )
    }
}
