...
impl ParsedProgram {
    ...
    pub fn validate(&self) -> MResult<()> {
        ...
        if !self.header.validate_magic(b"MECH") {
            return Err(MechError::new(InvalidMagicNumberError, None).with_compiler_loc());
        }

        // Check version number
        if self.header.version != 1 {
            return Err(MechError::new(UnsupportedBytecodeVersionError, None).with_compiler_loc());
        }

        // Check mech version
        if self.header.mech_ver != parse_version_to_u16(env!("CARGO_PKG_VERSION")).unwrap() {
            return Err(MechError::new(IncompatibleMechVersionError, None).with_compiler_loc());
        }
    ...
    pub fn decode_const_entries(&self) -> MResult<Vec<Value>> {
        ...
        for const_entry in &self.const_entries {
            ...
            if const_entry.enc != ConstEncoding::Inline as u8 {
                return Err(
                    MechError::new(UnsupportedConstantEncodingError, None).with_compiler_loc()
                );
            }

            // Bounds check #1
            if const_entry.offset.checked_add(const_entry.length).is_none() {
                return Err(MechError::new(ConstantEntryOutOfBoundsError, None).with_compiler_loc());
            }
            ...
            if !check_alignment(const_entry.offset, const_entry.align) {
                return Err(MechError::new(ConstantEntryAlignmentError, None).with_compiler_loc());
            }
            ...
            let val: Value = match ty.tag {
                ...
                TypeTag::MatrixU8 => {
                    if data.len() < 1 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[u8]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixI8 => {
                    if data.len() < 1 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[i8]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixF32 => {
                    if data.len() < 4 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[f32]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixF64 => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[f64]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixU16 => {
                    if data.len() < 2 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[u16]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixU32 => {
                    if data.len() < 4 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[u32]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixU64 => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[u64]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixI16 => {
                    if data.len() < 2 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[i16]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixI32 => {
                    if data.len() < 4 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[i32]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixI64 => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[i64]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixC64 => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[c64]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixR64 => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[r64]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::MatrixIndex => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "[ix]" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::Set => {
                    if data.len() < 4 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "set" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::Table => {
                    if data.len() < 8 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "table" },
                            None,
                        )
                        .with_compiler_loc());
                    }
                ...
                TypeTag::Tuple => {
                    if data.len() < 4 {
                        return Err(MechError::new(
                            ConstantTooShortError { type_name: "tuple" },
                            None,
                        )
                        .with_compiler_loc());
                    }
    ...
    if header.types_off != 0 && header.types_off + 4 <= total_len.saturating_sub(4) {
        ...
        for _ in 0..types_count {
            ...
            if let Some(tag) = TypeTag::from_u16(tag) {
                ...
                return Err(
                    MechError::new(UnknownConstantTypeError { tag }, None).with_compiler_loc()
                );
            }
        }
    }
    ...
    if header.dict_off != 0 && header.dict_len > 0 {
        ...
        while cur.position() < dict_bytes.len() as u64 {
            ...
            let name = String::from_utf8(name_bytes)
                .map_err(|_| MechError::new(InvalidUtf8InDictError, None).with_compiler_loc())?;
...
impl ParsedConstEntry {
    pub fn write_to<W: Write>(&self, w: &mut W) -> MResult<()> {
        // type_id (u32)
        w.write_u32::<LittleEndian>(self.type_id).map_err(|e| {
            MechError::new(
        ...
        w.write_u8(self.align).map_err(|e| {
            MechError::new(
                ConstEntryWriteIoError {
                    source: format!("{}", e),
                },
        ...
        w.write_u8(self.flags).map_err(|e| {
            MechError::new(
                ConstEntryWriteIoError {
                    source: format!("{}", e),
                },
        ...
        w.write_u8(self.reserved).map_err(|e| {
            MechError::new(
                ConstEntryWriteIoError {
                    source: format!("{}", e),
                },
        ...
        // offset (u64)
        w.write_u64::<LittleEndian>(self.offset).map_err(|e| {
            MechError::new(
                ConstEntryWriteIoError {
                    source: format!("{}", e),
                },
        ...
        // length (u64)
        w.write_u64::<LittleEndian>(self.length).map_err(|e| {
            MechError::new(
                ConstEntryWriteIoError {
                    source: format!("{}", e),
                },
...
pub enum DecodedInstr {
    ...
    Unknown {
        opcode: u8,
        rest: Vec<u8>,
    }, // unknown opcode or dynamic form
}

impl DecodedInstr {
    pub fn from_u8(num: u8) -> Option<DecodedInstr> {
        match OpCode::from_u8(num) {
            Some(OpCode::ConstLoad) => Some(DecodedInstr::ConstLoad {
                dst: 0,
                const_id: 0,
            }),
            Some(OpCode::NullOp) => Some(DecodedInstr::NullOp { fxn_id: 0, dst: 0 }),
            Some(OpCode::Unop) => Some(DecodedInstr::UnOp {
                fxn_id: 0,
                dst: 0,
                src: 0,
            }),
            Some(OpCode::Binop) => Some(DecodedInstr::BinOp {
                fxn_id: 0,
                dst: 0,
                lhs: 0,
                rhs: 0,
            }),
            Some(OpCode::Ternop) => Some(DecodedInstr::TernOp {
                fxn_id: 0,
                dst: 0,
                a: 0,
                b: 0,
            ...
            Some(OpCode::Quadop) => Some(DecodedInstr::QuadOp {
                fxn_id: 0,
                dst: 0,
                a: 0,
                b: 0,
            ...
            Some(OpCode::VarArg) => Some(DecodedInstr::VarArg {
                fxn_id: 0,
                dst: 0,
                args: vec![],
            }),
...
fn decode_instructions(mut cur: Cursor<&[u8]>) -> MResult<Vec<DecodedInstr>> {
    ...
    while (cur.position() as usize) < cur.get_ref().len() {
        ...
        if rem < 8 {
            return Err(MechError::new(TruncatedInstructionError, None).with_compiler_loc());
        }
        ...
        match OpCode::from_u8(opcode_byte) {
            ...
            Some(unknown) => {
                return Err(MechError::new(UnknownOpcodeError { opcode: unknown }, None)
                    .with_compiler_loc());
            }
            None => {
                return Err(MechError::new(
...
impl MechErrorKind for IncompatibleMechVersionError {
    fn name(&self) -> &str {
        "IncompatibleMechVersion"
    }
    fn message(&self) -> String {
        "Incompatible Mech version".to_string()
    }
}
...
impl MechErrorKind for UnsupportedConstantEncodingError {
    fn name(&self) -> &str {
        "UnsupportedConstantEncoding"
    }
    fn message(&self) -> String {
        "Unsupported constant encoding".to_string()
    }
}
...
impl MechErrorKind for ConstantEntryOutOfBoundsError {
    fn name(&self) -> &str {
        "ConstantEntryOutOfBounds"
    }
    fn message(&self) -> String {
        "Constant entry out of bounds".to_string()
    }
}
...
impl MechErrorKind for ConstantEntryAlignmentError {
    ...
    fn message(&self) -> String {
        "Constant entry alignment error".to_string()
    }
}
...
impl MechErrorKind for TruncatedInstructionError {
    ...
    fn message(&self) -> String {
        "Truncated instruction: cannot read full opcode or operands".to_string()
    }
}
...
impl MechErrorKind for UnknownOpcodeError {
    ...
    fn message(&self) -> String {
        format!("Unknown opcode: {}", self.opcode)
    }
}
...
impl MechErrorKind for InvalidOpcodeError {
    ...
    fn message(&self) -> String {
        format!("Invalid opcode byte: {}", self.opcode)
    }
}
...
impl MechErrorKind for InvalidUtf8InDictError {
    ...
    fn message(&self) -> String {
        "Invalid UTF-8 in dictionary entry".to_string()
    }
}
...
impl MechErrorKind for ConstEntryWriteIoError {
    ...
    fn message(&self) -> String {
        format!("Failed to write constant entry: {}", self.source)
    }
}
