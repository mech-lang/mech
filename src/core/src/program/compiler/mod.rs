...
pub fn encode_value_kind(ts: &mut TypeSection, vk: &ValueKind) -> (TypeTag, Vec<u8>) {
    ...
    let tag = match vk {
        ...
        ValueKind::U8 => TypeTag::U8,
        ValueKind::U16 => TypeTag::U16,
        ValueKind::U32 => TypeTag::U32,
        ValueKind::U64 => TypeTag::U64,
        ValueKind::U128 => TypeTag::U128,
        ValueKind::I8 => TypeTag::I8,
        ValueKind::I16 => TypeTag::I16,
        ValueKind::I32 => TypeTag::I32,
        ValueKind::I64 => TypeTag::I64,
        ValueKind::I128 => TypeTag::I128,
        ValueKind::F32 => TypeTag::F32,
        ValueKind::F64 => TypeTag::F64,
        ...
        ValueKind::Matrix(elem, dims) => {
            ...
            for &d in dims {
                b.write_u32::<LittleEndian>(d as u32).unwrap();
            }
            match &**elem {
        ...
        ValueKind::Enum(id, name) => {
            ...
            b.write_u32::<LittleEndian>(name_bytes.len() as u32)
                .unwrap();
        ...
        ValueKind::Set(elem, max) => {
            ...
            match max {
                Some(m) => {
                    ...
                    b.write_u32::<LittleEndian>(*m as u32).unwrap();
                }
                None => {
                    b.push(0);
                }
