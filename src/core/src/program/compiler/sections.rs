...
impl TypeTag {
    pub fn from_u16(tag: u16) -> Option<Self> {
        match tag {
            ...
            8 => Some(TypeTag::I32),
            9 => Some(TypeTag::I64),
            10 => Some(TypeTag::I128),
            11 => Some(TypeTag::F32),
            12 => Some(TypeTag::F64),
            13 => Some(TypeTag::C64),
            14 => Some(TypeTag::R64),
            15 => Some(TypeTag::String),
            16 => Some(TypeTag::Bool),
            17 => Some(TypeTag::Id),
            18 => Some(TypeTag::Index),
            19 => Some(TypeTag::Empty),
            20 => Some(TypeTag::Any),
            21 => Some(TypeTag::MatrixU8),
            22 => Some(TypeTag::MatrixU16),
            23 => Some(TypeTag::MatrixU32),
            24 => Some(TypeTag::MatrixU64),
            25 => Some(TypeTag::MatrixU128),
            26 => Some(TypeTag::MatrixI8),
            27 => Some(TypeTag::MatrixI16),
            28 => Some(TypeTag::MatrixI32),
            29 => Some(TypeTag::MatrixI64),
            30 => Some(TypeTag::MatrixI128),
            31 => Some(TypeTag::MatrixF32),
            32 => Some(TypeTag::MatrixF64),
            33 => Some(TypeTag::MatrixC64),
            34 => Some(TypeTag::MatrixR64),
            35 => Some(TypeTag::MatrixBool),
            36 => Some(TypeTag::MatrixString),
            37 => Some(TypeTag::MatrixIndex),
            38 => Some(TypeTag::EnumTag),
            39 => Some(TypeTag::Record),
            40 => Some(TypeTag::Map),
            41 => Some(TypeTag::Atom),
            42 => Some(TypeTag::Table),
            43 => Some(TypeTag::Tuple),
            44 => Some(TypeTag::Reference),
            45 => Some(TypeTag::Set),
            46 => Some(TypeTag::OptionT),
            47 => Some(TypeTag::Kind),
            48 => Some(TypeTag::None),
            _ => None,
        }
    }
}
...
impl TypeSection {
    ...
    pub fn get_or_intern(&mut self, vk: &ValueKind) -> TypeId {
        if let Some(id) = self.interner.get(vk) {
            return *id;
        }
...
pub enum EncodedInstr {
    ConstLoad {
        dst: u32,
        const_id: u32,
    }, // [u64 opcode][u32 dst][u32 const_id]
    NullOp {
        fxn_id: u64,
        dst: u32,
    }, // [u64 opcode][u64 fxn_id][u32 dst]
    UnOp {
        fxn_id: u64,
        dst: u32,
        src: u32,
    }, // [u64 opcode][u32 dst][u32 src]
    BinOp {
        fxn_id: u64,
        dst: u32,
        lhs: u32,
        rhs: u32,
    }, // [u64 opcode][u32 dst][u32 lhs][u32 rhs]
    TernOp {
        fxn_id: u64,
        dst: u32,
        a: u32,
        b: u32,
        c: u32,
    }, // [u64 opcode][u32 dst][u32 a][u32 b][u32 c]
    QuadOp {
        fxn_id: u64,
        dst: u32,
        a: u32,
        ...
        c: u32,
        d: u32,
    }, // [u64 opcode][u32 dst][u32 a][u32 b][u32 c][u32 d]
    VarArg {
        fxn_id: u64,
        dst: u32,
        args: Vec<u32>,
    }, // [u64 opcode][u64 fxn_id][u32 dst][u32 arg_count][u32 args...]
    Ret {
        src: u32,
    }, // [u64 opcode][u32 src]
}
