    ...
    pub fn to_value_kind(&self, kinds: &KindTable) -> MResult<ValueKind> {
        match self {
            ...
            Kind::Record(elements) => {
                let val_knds: Vec<(String, ValueKind)> = elements
                    .iter()
            ...
            Kind::Scalar(id) => {
                match kinds.get(id).cloned() {
                    ...
                    None => Err(MechError::new(UndefinedKindError { kind_id: *id }, None)
                        .with_compiler_loc()),
                }
            }
            Kind::Set(kind, size) => {
            ...
            Kind::Tuple(elements) => {
                let val_knds = elements
                    .iter()
                    .map(|k| k.to_value_kind(kinds))
                    .collect::<MResult<Vec<ValueKind>>>()?;
