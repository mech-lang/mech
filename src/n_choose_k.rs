...
impl_stats_urop!(StatsSumColumnRD, RowDVector<T>, Matrix1<T>, sum_column_op);
  ...
  fn impl_combinatorics_n_choose_k_fxn(n: Value, k: Value) -> Result<Box<dyn MechFunction>, MechError> {
    ...
    match (n,k) {
      (Value::F64(n), Value::F64(k)) => {
        let n = n as f64;
        let k = k as f64;
        Ok(Box::new(CombinatoricsNChooseK{n: n, k: k}))
      }
    }



    impl_combinatorics_n_choose_k_match_arms!(
      (n,k),
...
impl NativeFunctionCompiler for CombinatoricsNChooseK {
  fn compile(&self, arguments: &Vec<Value>) -> MResult<Box<dyn MechFunction>> {
    ...
    match impl_combinatorics_n_choose_k_fxn(n.clone(),k.clone()) {
      ...
      Err(_) => {
        match (n,k) {
          (Value::MutableReference(n),Value::MutableReference(k)) => {impl_combinatorics_n_choose_k_fxn(n.clone(),k.clone())}
          (n,Value::MutableReference(k)) => {impl_combinatorics_n_choose_k_fxn(n.clone(),k.clone())}
          (Value::MutableReference(n),k) => {impl_combinatorics_n_choose_k_fxn(n.clone(),k.clone())}
