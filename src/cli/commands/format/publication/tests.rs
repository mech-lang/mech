    ...
    fn visit(path: &Path, artifacts: &mut Vec<PathBuf>) {
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                ...
                .is_some_and(|name| {
                    name.ends_with(".stage") || name.ends_with(".backup") || name.ends_with(".tmp")
                })
            {
                artifacts.push(path);
            }
...
fn publication_restores_js_when_wasm_installation_fails() {
    ...
    let error =
        publish_outputs_recoverably(vec![output(&js, b"new-js"), output(&wasm, b"new-wasm")])
            .unwrap_err();
...
fn publication_failure_restores_existing_html_and_assets() {
    ...
    for path in [&js, &wasm, &html] {
        std::fs::write(
            path,
            format!("old-{}", path.file_name().unwrap().to_string_lossy()),
        )
        .unwrap();
    }
...
fn publication_reports_rollback_failures() {
    ...
    assert!(
        error.contains("failed to restore formatter output"),
        "{error}"
    );
    assert_contents(&wasm, b"old-wasm");
    std::fs::remove_dir_all(root).unwrap();
