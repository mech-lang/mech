...
pub(crate) async fn dispatch(cli_matches: ArgMatches) -> MResult<CliOutcome> {
    ...
    if let Some(serve_matches) = cli_matches.subcommand_matches("serve") {
        ...
        let config_args = crate::cli::config::ConfigCliArgs::from_matches(serve_matches);
        let capability_args =
            crate::cli::capabilities::FilesystemCapabilityArgs::from_matches(serve_matches);
        let host_delegation_args =
            crate::cli::serve_options::ServeHostDelegationArgs::from_matches(serve_matches);
        let plan = crate::cli::commands::serve::prepare(
            args,
            config_args,
            capability_args,
            host_delegation_args,
            resources,
        )?;
