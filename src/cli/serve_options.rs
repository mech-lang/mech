...
use crate::{
    LoadedMechConfig, require_config_file, require_config_wasm_package, resolve_config_path,
};
    ...
    let sole_discovery_selector = config
        .and_then(|loaded| {
            loaded
                ...
                .as_ref()
                .map(|project_dir| (loaded, project_dir))
        })
        .map(|(_loaded, project_dir)| {
            ...
            let project_resolved = project_dir
                .canonicalize()
    ...
    let config_workspace_paths = || {
        ...
        if let Some(loaded) = config {
            for path in loaded
                ...
                .chain(loaded.document.serve.iter().flat_map(|serve| &serve.paths))
            {
