...
fn config_profile_browser_invalid_dom_selector_forms_rejected() {
    for selector in [
        ...
        let msg = err_text(&format!(
            "config := {{browser: {{dom: [{{selector: \"{selector}\", allow: [\"read\"]}}]}}}}\n"
        ));
        assert!(msg.contains("InvalidConfigField"));
...
fn config_profile_browser_invalid_network_origins_rejected() {
    for origin in [
        ...
        let msg = err_text(&format!(
            "config := {{browser: {{network: [{{origin: \"{origin}\", allow: [\"read\"]}}]}}}}\n"
        ));
        assert!(msg.contains("InvalidConfigField"));
...
fn config_profile_browser_resource_specific_operations_rejected() {
    for (config, operation) in [
        (
            r#"config := {browser: {clipboard: [{allow: ["list"]}]}}
"#,
            "list",
        ),
        (
            r##"config := {browser: {dom: [{selector: "#root", allow: ["invoke"]}]}}
"##,
            "invoke",
        ),
        (
            r#"config := {browser: {network: [{origin: "https://example.com", allow: ["write"]}]}}
"#,
            "write",
        ),
        (
            r#"config := {browser: {storage: [{backend: "opfs", scope: "/workspace", allow: ["watch"]}]}}
"#,
            "watch",
        ),
    ] {
...
fn config_profile_browser_storage_recursive_scope_is_explicit() {
    let exact = parse(
        r#"config := {browser: {storage: [{backend: "opfs", scope: "/workspace", allow: ["read"]}]}}
"#,
    )
    .unwrap();
    assert!(exact
    ...
    let recursive = parse(
        r#"config := {browser: {storage: [{backend: "opfs", scope: "/workspace", recursive: true, allow: ["read"]}]}}
"#,
    )
    .unwrap();
    assert!(recursive
