...
fn duplicate_host_instance_registration_fails_generically() {
    let error = RuntimeBuilder::new()
        ...
        .unwrap()
        .host_factory(Box::new(FakeRobotFactory::new(Arc::new(Mutex::new(
            Vec::new(),
        )))))
        .unwrap()
        .host_instance(HostInstanceConfig {
...
impl RuntimeHostFactory for PlotterFactory {
    ...
    fn instantiate(
        ...
        Ok(RuntimeHostInstallation {
            interface: materialize_host_manifest(instance_name, self.manifest())?,
            input_drivers: Vec::new(),
            resource_providers: vec![Box::new(PlotterProvider {
                instance: instance_name.to_string(),
                log: self.log.clone(),
            })],
        })
...
impl PlotterProvider {
    fn base(&self) -> String {
        format!("plotter://{}/commands", self.instance)
    }
}
...
impl RuntimeResourceProvider for PlotterProvider {
    ...
    fn preflight_write(&self, request: RuntimeResourceWritePreflightRequest) -> MResult<()> {
        ...
        if request.intent != RuntimeResourceWriteIntent::Send {
            return Err(fake_error("plotter commands accept only send intent"));
        }
        match request.path.as_str() {
            "line" | "text" | "read" => Ok(()),
            ...
            _ => Err(fake_error(format!(
                "unsupported plotter command path `{}`",
                request.path
            ))),
        }
    }
...
fn custom_text_send_is_not_forced_to_write() {
    ...
    runtime
        .run_string("+> @plotter := plotter/commands\n@plotter/text <- { message: \"hello\" }\n")
        .unwrap();
...
fn context_send_read_path_does_not_use_read_grant() {
    ...
    assert!(
        message.contains("context_send") || message.contains("reserved"),
        "got {message}"
    );
...
fn scoped_custom_line_does_not_fallback_to_write() {
    ...
    let error = runtime
        .run_string(
            "+> @plotter := plotter/commands{:line(line/safe/*), :write(*)}
