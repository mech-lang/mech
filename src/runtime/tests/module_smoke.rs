...
fn runtime_resource_grant_for(
    ...
    runtime_exact_resource_grant_for(subject, resource, operation, path)
}
...
fn grant_runtime_stdout_line(runtime: &mut MechRuntime) {
    ...
    runtime
        ...
        .unwrap();
    runtime
        .grant_capability(runtime_exact_resource_grant_for(
            &subject,
            "cli://stdout",
            "write",
            "line",
...
fn context_for_subject(
    ...
    capabilities.dedup();
    let task = TaskRecord::new(runtime.next_task_id(), subject).with_capabilities(capabilities);
...
fn resolved_retained_root_imports_sibling_module() {
    ...
    assert_f64(
        runtime.root_symbol_value("answer").unwrap().into_value(),
        42.0,
        "retained root answer",
    );
...
fn resolved_retained_root_imports_parent_relative_module() {
    ...
    let mut runtime = runtime_with_root(&root);
    let result = runtime
        .resolve_and_run_root_module("app/main.mec", module_options())
        .unwrap();
...
fn resolved_retained_root_import_namespace_exports_are_visible() {
    ...
    runtime
        ...
        .unwrap();
    assert_f64(
        runtime.root_symbol_value("dep/value").unwrap().into_value(),
        41.0,
        "retained root namespace export",
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn resolved_retained_root_missing_dependency_reports_dependency_error() {
    let root = setup_main_only_module(
        "retained-root-missing-dependency",
        "+> ./missing.mec\nanswer := 1\n",
    );
    let mut runtime = runtime_with_root(&root);
    let error = runtime
    ...
    assert!(
        error
            ...
            .is_some(),
        "expected dependency missing error, got {error:?}"
    );
...
fn resolved_retained_root_missing_source_reports_root_error() {
    ...
    let error = runtime
        ...
        .unwrap_err();
    assert!(
        error.kind_as::<RuntimeRootModuleSourceNotFound>().is_some(),
        "expected root source not found, got {error:?}"
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn resolved_retained_root_dependency_cycle_reports_existing_cycle_error() {
    ...
    assert!(
        error
            ...
            .is_some(),
        "expected dependency cycle, got {error:?}"
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn retained_root_dependency_context_read_does_not_create_live_binding() {
    let root = setup_main_only_module(
        "retained-root-dependency-isolated",
        "+> ./dep.mec\nanswer := dep/value + 1\nanswer\n",
    );
    std::fs::write(
        root.join("dep.mec"),
    ...
    assert!(
        ...
        "dependency context reads must remain isolated"
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn retained_root_context_read_creates_live_binding_and_recomputes() {
    ...
    runtime
        ...
        .unwrap();
    assert_f64(
        runtime.root_symbol_value("output").unwrap().into_value(),
        11.0,
        "root recompute after host input",
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn failed_retained_root_restores_live_state_and_imported_environment() {
    ...
    runtime
        ...
        .unwrap();
    runtime
        .resolve_and_run_root_module("ok.mec", module_options())
        .unwrap();
    assert_f64(
        runtime.root_symbol_value("dep/value").unwrap().into_value(),
        41.0,
        "initial imported environment",
    );
    ...
    let error = runtime
        .resolve_and_run_root_module("bad.mec", module_options())
        .unwrap_err();
    assert!(
        format!("{error:?}").contains("missing"),
        "expected root execution failure, got {error:?}"
    );
    assert!(
        !runtime.has_live_input_bindings(),
        "failed root must restore live input bindings"
    );
    assert_f64(
        runtime.root_symbol_value("dep/value").unwrap().into_value(),
        41.0,
        "restored imported environment",
    );
    std::fs::remove_dir_all(root).unwrap();
}
...
fn resource_registry_write_then_read_returns_value() {
    ...
    runtime
        ...
        .unwrap();
    runtime
        .write_bound_resource("manual", "intro/title", &bool_value(true))
        .unwrap();
    let value = runtime
        .read_bound_resource("manual", "intro/title")
        .unwrap();
...
fn resource_registry_write_missing_provider_fails() {
    ...
    runtime
        ...
        .unwrap();
    let result = runtime.write_bound_resource("manual", "intro/title", &bool_value(true));
...
fn config_spec_conflicts_with_existing_same_docs_base() {
    ...
    let result = RuntimeBuilder::new()
        .in_memory_docs(docs_provider_with(
            "docs://manual",
            "other/title",
            bool_value(true),
        ))
...
fn registered_module_manifest_context_import_still_resolves() {
    let root =
        setup_modules("+> @manual := docs/page\nresult := @manual/title\n<+ result\nresult\n");
...
fn host_instance_unknown_context_does_not_fallback_to_module_manifest() {
    ...
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("HostInterfaceUnknownContext"),
        "expected host unknown context, got {error}"
    );
}
...
fn derived_context_unknown_base_is_rejected() {
    ...
    assert!(
        ...
        "expected invalid binding error, got {error}"
    );
    assert!(
        error.contains("base"),
...
fn direct_runtime_derived_context_without_host_read_preflights() {
    ...
    runtime
        .run_string("@base := docs://manual{:read(*)}\n@docs := @base\nx := 1\n")
        .unwrap()
        .into_value();
}
...
fn derived_context_cannot_expand_capabilities() {
    ...
    let result = runtime.run_string(
        "@base := docs://manual{:read(intro/title)}\n@docs := @base{:write(intro/title)}\n",
    );
    assert!(
        result.is_err(),
        "derived write should not be allowed to expand read-only base"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("RuntimeContextInvalidBinding"),
...
fn interpreter_scope_imports_work() {
    ...
    let result = runtime
        .run_module_scope(version, scope)
        .unwrap()
        .result
        .into_value();
    match result {
...
fn workspace_load_target_without_imports() {
    ...
    assert_bool_true(
        runtime
            .run_module(target.module_version)
            .unwrap()
            .result
            .into_value(),
        "workspace target",
...
fn manifest_context_import_ordering_supports_derived_alias() {
    ...
    assert!(
        result.is_ok(),
        "manifest context import should be available to later derived alias: {result:?}"
    );
}
...
fn stored_module_manifest_context_import_ordering_supports_derived_alias() {
    ...
    assert!(
        result.is_err(),
        "without browser provider this should fail at provider lookup"
    );
...
fn stored_module_derived_context_before_manifest_base_is_rejected() {
    ...
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("RuntimeContextInvalidBinding") && error.contains("unknown context `@ui`"),
...
fn resolving_module_with_fenced_context_import_does_not_pollute_direct_runtime_bindings() {
    ...
    assert!(
        error.contains("UnknownAddressTarget")
            ...
            || error.contains("direct_context_target"),
...
fn with_test_cli<B: CliBackend + Clone + 'static>(
    builder: RuntimeBuilder,
    backend: B,
) -> RuntimeBuilder {
    builder
        .host_factory(Box::new(TestCliFactory::new(backend)))
        .unwrap()
        .host_instance(HostInstanceConfig {
            name: "cli".to_string(),
            provider: "cli".to_string(),
...
",
    ...
    assert_eq!(
        values.len(),
        1,
        "named fence export should remain observable after direct run"
    );
}
...
"#,
    ...
    assert!(
        result.is_err(),
        "function body context read should be rejected"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
...
"#,
    ...
    assert!(
        result.is_err(),
        "assignment target subscript context read should be preflighted"
    );
    let error = format!("{:?}", result.err().unwrap());
...
"#,
    ...
    assert!(
        result.is_err(),
        "op-assignment target subscript context read should be preflighted"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
...
",
    ...
    assert!(
        result.is_err(),
        "match arm context read should fail in preflight"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
...
",
    ...
    assert!(
        result.is_err(),
        "activation arm context read should fail in preflight"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("CapabilityDenied") || error.contains("context_read"),
...
fn activation_arm_context_read_pattern_is_lowered_when_allowed() {
    ...
    state
        ...
        .unwrap()
        .env
        .insert("MECH_ACTIVATION_EVENT".to_string(), "expected".to_string());
    let subject = runtime.runtime_context().unwrap().subject().to_string();
    runtime
        .grant_capability(runtime_exact_resource_grant_for(
            &subject,
...
",
    ...
    assert!(
        result.is_ok(),
        "allowed activation context pattern failed: {result:?}"
    );
...
",
    ...
    assert!(
        result.is_err(),
        "FSM pipe transition context read should fail in preflight"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("CapabilityDenied") || error.contains("context_read"),
...
",
    ...
    assert!(
        result.is_err(),
        "FSM declaration context read should fail in preflight"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
        error.contains("CapabilityDenied") || error.contains("context_read"),
...
fn match_arm_context_read_pattern_compares_value_when_allowed() {
    ...
    state
        ...
        .unwrap()
        .env
        .insert("MECH_MATCH_PATTERN".to_string(), "expected".to_string());
...
",
    ...
    let bool_matching = bool_runtime
        .run_string(
...
",
    ...
    let bool_mismatching = bool_runtime
        .run_string(
            "@manual := docs://manual{:read(flag), :write(flag)}
@manual/flag = true
mismatched := false?
...
fn run_tree_with_context_preflight_failure_emits_failure_and_profile_events() {
    ...
    let mut runtime = with_test_cli(RuntimeBuilder::new().config(config), backend)
        .build()
        .unwrap();
    let mut context = runtime.runtime_context().unwrap();
...
impl RecordingResourceProvider {
    ...
    fn with_equivalent_base_uri_group(mut self, bases: &[&str]) -> Self {
...
fn explicit_transaction_abort_discards_buffered_stdout() {
    ...
    assert!(state.lock().unwrap().stdout.is_empty());
    runtime
        .abort_runtime_transaction(&mut context, "discard output")
        .unwrap();
    assert!(state.lock().unwrap().stdout.is_empty());
}
...
fn failed_explicit_operation_discards_only_its_buffered_stdout() {
    ...
    assert!(state.lock().unwrap().stdout.is_empty());
    runtime.commit_runtime_transaction(&mut context).unwrap();
    assert_eq!(state.lock().unwrap().stdout, vec!["keep-me\n".to_string()],);
}
...
",
    ...
    assert!(
        result.is_err(),
        "stdout assignment should fail in preflight"
    );
    let error = format!("{:?}", result.err().unwrap());
    assert!(
...
",
    ...
    assert_bool_true(
        runtime.run_module(version).unwrap().result.into_value(),
        "interpreter address from program",
    );
}
...
fn module_function_unknown_address_target_is_preflighted_before_send() {
    ...
    fn ident(name: &str) -> mech_core::Identifier {
        mech_core::Identifier {
            name: token(mech_core::TokenKind::Identifier, name),
        }
    }

    fn addressed_var(target: &str, name: &str) -> mech_core::Expression {
        mech_core::Expression::Var(mech_core::Var {
            name: ident(name),
                ...
                elements: vec![mech_core::SectionElement::MechCode(vec![
                    (
                        mech_core::MechCode::Statement(mech_core::Statement::ContextDeclaration(
                            mech_core::ContextDeclaration {
                                name: ident("out"),
                                base: mech_core::ContextBase::ResourceUri(token(
                    ...
                    (
                        mech_core::MechCode::Statement(mech_core::Statement::ContextSend(
                            mech_core::ContextSend {
                                target: mech_core::Var {
                                    name: ident("line"),
                                    context: Some(ident("out")),
                                    kind: None,
                                },
                                expression: mech_core::Expression::Literal(
                                    mech_core::Literal::String(mech_core::MechString {
                    ...
                    (
                        mech_core::MechCode::FunctionDefine(mech_core::FunctionDefine {
                            ...
                            input: vec![],
                            output: vec![],
                            statements: vec![mech_core::Statement::VariableDefine(
                                mech_core::VariableDefine {
                                    mutable: false,
                                    var: mech_core::Var {
                                        name: ident("value"),
                                        context: None,
                    ...
                    (
                        mech_core::MechCode::Statement(mech_core::Statement::VariableDefine(
                            mech_core::VariableDefine {
                                ...
                                expression: mech_core::Expression::Literal(
                                    mech_core::Literal::Boolean(token(
    ...
    assert!(
        state.lock().unwrap().stdout.is_empty(),
        "stdout write leaked before module preflight failed"
    );
}
...
fn run_bytecode_does_not_leave_symbol_state_for_next_source() {
    ...
    runtime
        .run_source_with_context(
            &mut context,
            &MechSourceCode::String("y := 1.0".to_string()),
        )
        .unwrap();
    ...
    let result = runtime
        .run_source_with_context(
            &mut context,
            &MechSourceCode::String("x := 3.0".to_string()),
        )
        .unwrap();
...
fn run_bytecode_error_restores_previous_program_state() {
    ...
    let result = runtime
        .run_source_with_context(
            &mut context,
            &MechSourceCode::String("z := y + 1".to_string()),
        )
        .unwrap();
