...
use mech_runtime::__resident_recording::{
    InputSequence, InputSequenceRange, LedgerPermit, OwnedTurnRecord, RecordEstimate,
    ResidentEkfReceipt, RetainedTurnLedger, TurnId, TurnRecordHeader, TurnRecordStatus,
...
pub struct EpochFixture {
    ...
    next_epoch: u64,
    ledger: RetainedTurnLedger<OwnedTurnRecord<ResidentEkfReceipt>>,
...
impl EpochFixture {
    pub fn new(instances: usize) -> Self {
        let ledger = RetainedTurnLedger::new(
            EPISODE_LENGTH,
            EPISODE_LENGTH * ResidentEkfReceipt::RETAINED_BYTES,
        )
        .expect("resident EKF retained ledger");
        let estimate = RecordEstimate {
            records: 1,
            bytes: ResidentEkfReceipt::RETAINED_BYTES,
        };
        let permits = (0..EPISODE_LENGTH)
            .map(|_| Some(reserve_retained(&ledger, estimate).expect("resident EKF admission")))
            .collect();
        Self {
            versions: [
            ...
            probe: EpochProbe {
                ...
                receipt_bytes: ResidentEkfReceipt::RETAINED_BYTES,
    ...
    fn run_turn(
        ...
        let identity = u64::try_from(turn + 1).expect("resident EKF turn identity");
        let input_sequence = InputSequence::new(identity).expect("non-zero resident EKF input");
        let receipt = OwnedTurnRecord {
            header: TurnRecordHeader {
                turn_id: TurnId::new(identity).expect("non-zero resident EKF turn"),
                transaction_id: TransactionId::new(u128::from(identity)),
                input_range: Some(
                    InputSequenceRange::new(input_sequence, input_sequence)
                        .expect("one-input resident EKF range"),
                ),
            ...
            body: ResidentEkfReceipt::accepted(
                ...
                u16::try_from(self.versions[working_index].len() * 2)
                    .expect("resident EKF touched count"),
                u16::try_from(self.versions[working_index].len() * 2)
                    .expect("resident EKF changed count"),
                u16::try_from(self.versions[working_index].len() * 15)
                    .expect("resident EKF dirty count"),
            ),
        };
        let permit = self.permits[turn]
            .take()
            .expect("unused resident EKF admission");
        let prepared = prepare_retained(&mut self.ledger, permit, receipt)
            .expect("resident EKF exact receipt preparation");
