use mech_engine::__resident_ekf::{ResidentEkfBatch, ResidentFullWrite};
use mech_runtime::__resident_recording::{ResidentEkfReceipt, ResidentTurnRecorder};
...
impl ResidentTurnFixture {
    pub fn new(instances: usize, retained_history: usize, next_epoch: u64) -> Self {
        ...
        resident.set_next_epoch_for_benchmark(next_epoch);
        Self {
            resident,
            recorder: ResidentTurnRecorder::new(EPISODE_LENGTH, retained_history)
                .expect("resident EKF recorder setup"),
            instances,
            retained_history,
        }
    }
    ...
    pub fn abort_output_hash_with_rejection(&mut self) -> String {
        ...
        self.recorder
            .prepare_rejected(
                permit,
                before_epoch,
                mech_engine::__resident_ekf::ResidentExecutionError::NonFiniteState,
            )
