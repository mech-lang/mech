...
use crate::{
    HOST_DELEGATION_ALGORITHM_ED25519, HostDelegationEnvelope, HostDelegationExpiredError,
    HostDelegationHeader, HostDelegationKeyNotFoundError, HostDelegationPayload,
    HostDelegationPublicKey, HostDelegationSignature, HostDelegationSignatureError,
    HostDelegationVerificationRequest, HostDelegationWrongAudienceError, VerifiedHostDelegation,
};
...
impl HostDelegationSigningKey {
    pub fn from_ed25519_private_key_bytes(bytes: &[u8]) -> MResult<Self> {
        let bytes: [u8; 32] = bytes
            .try_into()
            .map_err(|_| signature_error("ed25519 private key must contain exactly 32 bytes"))?;
        Ok(Self {
...
pub fn sign_host_delegation<P: HostDelegationPayload>(
    ...
    envelope.signature = Some(HostDelegationSignature {
        bytes: signer.sign(&payload),
    });
    Ok(envelope)
}
...
pub fn verify_host_delegation<P: HostDelegationPayload>(
    ...
    let public_key: [u8; 32] = key
        ...
        .as_slice()
        .try_into()
        .map_err(|_| signature_error("ed25519 public key must contain exactly 32 bytes"))?;
    ...
    let signature_bytes: [u8; 64] = signature
        ...
        .as_slice()
        .try_into()
        .map_err(|_| signature_error("ed25519 signature must contain exactly 64 bytes"))?;
...
mod tests {
    ...
    use crate::host::delegation::tests::{TestPayload, header, test_payload};
    ...
    fn signed_test_envelope() -> (
        ...
        HostDelegationEnvelope<TestPayload>,
    ) {
        let key = signing_key();
    ...
    fn wrong_audience_fails_validation() {
        ...
        let error = format!(
            "{:?}",
            verify_host_delegation(&envelope, request).unwrap_err()
        );
