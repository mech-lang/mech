...
impl HostFunctionPlan for ActorMessageKindHostFunction {
  fn name(&self) -> &str { "actor/message/kind" }
...
impl HostFunctionPlan for ActorMessagePayloadHostFunction {
  fn name(&self) -> &str { "actor/message/payload" }
...
impl HostFunctionPlan for ActorStateIdHostFunction {
  fn name(&self) -> &str { "actor/state/id" }
...
impl HostFunctionPlan for ActorStateGetHostFunction {
  fn name(&self) -> &str { "actor/state/get" }
...
impl HostFunctionPlan for ActorStatePutHostFunction {
  fn name(&self) -> &str { "actor/state/put" }
...
impl RuntimeManagedHostFunction for ActorStatePutHostFunction {
  fn invoke(
    ...
    services.put_object(ObjectRecord::text(
      object_id,
      "actor-state",
      text,
    ))?;
    let mut actor = services.get_actor(actor_id)?.ok_or_else(|| {
      invalid_context(self.name(), "actor record was not found")
    })?;
