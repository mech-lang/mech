use mech_core::{
    LegacyValue, ReactiveCellId, ReactiveDependencyKind, ReactiveNodeId, ReactiveNodeKind, hash_str,
};

use super::super::MechRuntime;
use crate::{HostArgumentValue, RuntimeHostInputSource};

pub(crate) fn f64_value(value: &LegacyValue) -> f64 {
    match value {
...
pub(crate) fn bool_value(value: &LegacyValue) -> bool {
    match value {
        LegacyValue::Bool(value) => *value.borrow(),
        other => panic!("expected bool, got {other:?}"),
    }
}

pub(crate) fn string_value(value: &LegacyValue) -> String {
    match value {
...
pub(crate) fn symbol_value(runtime: &MechRuntime, name: &str) -> LegacyValue {
    runtime
        .program
        .interpreter()
        .symbols()
...
pub(crate) fn source_value(runtime: &MechRuntime, source: &RuntimeHostInputSource) -> LegacyValue {
    let input = runtime
        .live_input_bindings
        .get(source)
