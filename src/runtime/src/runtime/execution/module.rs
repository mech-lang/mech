use super::{
    AddressedReadPreflight, PreparedModuleScopeExecution, ProgramEnvironmentOverlay,
    RuntimeAddressTarget, RuntimeProgramTarget, context_registry_for_scope,
    execution_scope_for_extracted_module_source, exports_for_scope,
    materialize_function_imports_for_scope, merge_module_environment, module_source_for_scope,
...
use crate::resolver::{
    SourceAddressReference, SourceImportKind, SourceScope, module_namespace_for_import,
};
use crate::runtime::module::validate_module_import_edges;
use crate::runtime::{
    MechRuntime, RuntimeInvalidOperationError, RuntimeModuleExportNotFound,
    RuntimeModuleImportConflict, RuntimeRecordNotFoundError, UnknownAddressTarget,
};
...
#[cfg(feature = "invariant_define")]
pub(in crate::runtime) type IntegrityEvaluationCollector = Vec<IntegrityConstraintEvaluation>;
...
fn append_integrity_report(
    ...
    let Some(failures) =
        IntegrityConstraintReport::from_evaluations(collector.clone()).into_violation_set()
    else {
        return Ok(());
    };
    let tokens = failures
...
impl ModuleInstance {
    pub(in crate::runtime) fn try_detached_result(&self) -> MResult<RuntimeModuleResult> {
        Ok(RuntimeModuleResult {
            ...
            exports: self
                ...
                .map(|(name, value)| {
                    Ok((
                        name.clone(),
                        RuntimeValueSnapshot::try_capture(&value.borrow())?,
                    ))
                })
                .collect::<MResult<_>>()?,
            result: RuntimeValueSnapshot::try_capture(&self.result)?,
        })
    }
}

impl MechRuntime {
    pub fn run_module(&mut self, version: ModuleVersionId) -> MResult<RuntimeModuleResult> {
    ...
    pub fn run_module_scope_with_context(
        ...
        self.with_atomic_program_operation(
            ...
            |runtime, context| {
                ...
                let mut integrity_evaluations = IntegrityEvaluationCollector::default();
    ...
    pub(in crate::runtime) fn preflight_module_graph_for_scope(
        ...
        let Some(record) = self.get_module_version_visible(context, version)? else {
        ...
        for requirement in &record.capability_requirements {
            ...
            self.check_capability_with_context(context, &requirement)?;
        }
    ...
    fn prepare_module_scope_execution(
        ...
        Ok(PreparedModuleScopeExecution {
            record,
    ...
    fn execute_module_isolated_for_scope(
        ...
        self.register_runtime_program_host_functions(context, &mut module_program)?;
        ...
        let result = self.run_module_source_on_program(
            context,
            &mut RuntimeProgramTarget::Isolated(&mut module_program),
            &prepared.source,
            scope,
        ...
        #[cfg(feature = "invariant_define")]
        {
            ...
            if let Err(error) = append_integrity_report(integrity_evaluations, report) {
                self.emit_event_to_context(
                    context,
                    RuntimeEventKind::ModuleExecutionFailed {
                        module_version: version,
                ...
                let Some(value_ref) = symbols_brrw.get(id) else {
                    let error = MechError::new(
                        RuntimeModuleExportNotFound {
                            dependency: prepared.record.id.to_string(),
        ...
        if !matches!(scope, SourceScope::Program) {
            return Err(MechError::new(
                RuntimeInvalidOperationError {
                    ...
                    reason: "retained root module execution only supports program scope"
                        .to_string(),
                },
                None,
            ));
        ...
        let result = self.with_live_registration_mode(registration_mode, |runtime| {
            match source {
                ...
                MechSourceCode::Html(_) | MechSourceCode::ByteCode(_) => {
                    runtime.execute_program_target_source(context, target, source)
                }
                MechSourceCode::Image(_, _) => Err(MechError::new(
                    RuntimeInvalidOperationError {
                        operation: "run_module_preflight",
