...
use super::{
    RuntimeCommitResolution, RuntimeContextCheckpoint, RuntimeExecutionTransactionMode,
    RuntimeOperationSavepoint, RuntimeProgramBaseline, RuntimeProgramOperationSavepoint,
};
...
pub(in crate::runtime) fn integrity_failure_audit(
    ...
    let violations = failures
        ...
        .map(|violation| RuntimeIntegrityConstraintViolation {
            ...
            evaluated_kind: violation.evaluated_kind.as_ref().map(ToString::to_string),
...
impl MechRuntime {
    ...
    pub(in crate::runtime) fn with_atomic_program_operation<T>(
        ...
        let (transaction_id, program_checkpoint, live_checkpoint) = if implicit {
            let program_checkpoint = self.capture_runtime_program_checkpoint()?;
            ...
            match self.acquire_program_transaction_ownership(transaction_id, operation)? {
                RuntimeProgramOwnershipAcquisition::NewlyAcquired { program, live } => {
        ...
        #[cfg(feature = "invariant_define")]
        let integrity_audit =
            integrity_failure_audit(&original_error, transaction_id, context.task);
        let rollback_failures =
            self.rollback_program_operation(context, transaction_id, &savepoint);
            ...
            if implicit {
                ...
                if let Err(error) = self.release_new_program_transaction_ownership(transaction_id) {
                    return Err(self.poison_program_operation(
                        ...
                        Some(transaction_id),
                        original_error_text,
                        vec![format!("program ownership release failed: {:?}", error,)],
                    ));
                }
            }
            #[cfg(feature = "invariant_define")]
