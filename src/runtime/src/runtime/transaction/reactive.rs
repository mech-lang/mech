...
use super::{
    ActiveRuntimeProgramOperation, RuntimeCommitResolution, RuntimeExecutionTransactionMode,
    RuntimeOperationSavepoint, RuntimeProgramOwnershipAcquisition,
};
...
fn execution_services_borrow_conflict(operation: &'static str) -> MechError {
    MechError::new(ExecutionServicesBorrowConflict { operation }, None).with_compiler_loc()
}

fn reactive_panic_message(panic: &(dyn std::any::Any + Send)) -> String {
...
impl MechRuntime {
    pub(in crate::runtime) fn prepare_runtime_host_input(
        ...
        for update in &input.updates {
            let Some(bindings) = self.live_input_bindings.get(&update.source) else {
    ...
    pub(in crate::runtime) fn validate_live_turn_context(
        ...
        let identity_matches = template.runtime == context.runtime
            ...
            && template.budget_limits.max_steps == context.budget.max_steps
            && template.budget_limits.max_bytes == context.budget.max_bytes
            && template.budget_limits.max_items == context.budget.max_items
            && template.budget_limits.max_messages == context.budget.max_messages;
        if !identity_matches {
            return Err(MechError::new(
        RuntimeInvalidOperationError {
    ...
    pub(in crate::runtime) fn with_atomic_reactive_turn<T>(
        ...
        let (transaction_id, newly_acquired_ownership) = if implicit {
            ...
            let newly_acquired = matches!(
                self.acquire_program_transaction_ownership(transaction_id, operation,)?,
        ...
        let runtime_savepoint =
            match self.capture_runtime_operation_savepoint(context, transaction_id) {
        ...
        let execution_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut finalize = |value: &T| {
                let mut turn = match turn.try_borrow_mut() {
                    ...
                    Err(_) => {
                        return ProgramTurnFinalization::Rollback(
                            execution_services_borrow_conflict("runtime_finalize_reactive_turn"),
                        );
                    }
                };
                let RuntimeCoordinatedTurn {
                ...
                if let Err(error) = after_result {
                    *finalization = RuntimeReactiveFinalization::RollbackRequired;
                    return ProgramTurnFinalization::Rollback(error);
                }
                if !implicit {
                    *finalization = RuntimeReactiveFinalization::ExplicitCommit;
                    return ProgramTurnFinalization::Commit;
                }
                match runtime.commit_runtime_transaction_internal(context) {
                    Ok(RuntimeCommitResolution::Committed(_)) => {
                        *finalization = RuntimeReactiveFinalization::ImplicitCommitted;
                        ProgramTurnFinalization::Commit
                    }
                    Ok(RuntimeCommitResolution::CommittedWithError { error, .. }) => {
                        *finalization = RuntimeReactiveFinalization::ImplicitCommittedWithError;
        ...
        let finalization = { turn.borrow().finalization };
        drop(turn);

        match finalization {
            RuntimeReactiveFinalization::ExplicitCommit
            | RuntimeReactiveFinalization::ImplicitCommitted
            | RuntimeReactiveFinalization::ImplicitCommittedWithError => execution_result,
    ...
    fn finish_failed_reactive_runtime_turn<T>(
        ...
        let integrity_audit =
            super::program::integrity_failure_audit(&original_error, transaction_id, context.task);
        ...
        } else if newly_acquired_ownership && rollback_failures.is_empty() {
            if let Err(error) = self.release_new_program_transaction_ownership(transaction_id) {
                rollback_failures.push(format!("program ownership release failed: {:?}", error,));
            }
        }

        if rollback_failures.is_empty() {
            #[cfg(feature = "invariant_define")]
            self.emit_integrity_failure_audit(context, integrity_audit);
    ...
    fn finish_panicked_reactive_runtime_turn(
        ...
        cleanup_failures.extend(self.finish_transaction_cleanup_best_effort(
            context,
            transaction_id,
            &reason,
        ));
        cleanup_failures.extend(
            self.validate_transaction_cleanup_complete(context, transaction_id)
                .into_iter()
                .map(|failure| {
                    format!("panic cleanup invariant remained unsatisfied: {}", failure,)
                }),
        );
