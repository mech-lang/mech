//! Deterministic counters for runtime cost benchmarks.
//!
...
pub struct RuntimeCostSnapshot {
...
thread_local! {
    static RUNTIME_COSTS: RefCell<RuntimeCostSnapshot> =
        RefCell::new(RuntimeCostSnapshot::default());
}

fn add(field: impl FnOnce(&mut RuntimeCostSnapshot)) {
    RUNTIME_COSTS.with(|costs| field(&mut costs.borrow_mut()));
}

pub fn reset_runtime_costs() {
    RUNTIME_COSTS.with(|costs| *costs.borrow_mut() = RuntimeCostSnapshot::default());
}

pub fn runtime_cost_snapshot() -> RuntimeCostSnapshot {
    RUNTIME_COSTS.with(|costs| *costs.borrow())
}
...
mod tests {
    use super::{
        RuntimeCostSnapshot, record_commit_runtime_call, record_context_event_compaction,
        record_event_appended, record_in_memory_store_apply_duration,
        record_in_memory_store_prepare_duration, record_runtime_transaction_savepoint_clone,
        record_transaction_committed, reset_runtime_costs, runtime_cost_snapshot,
    };
    ...
    fn counters_are_thread_local_and_resettable() {
        ...
        assert_eq!(
            ...
            RuntimeCostSnapshot {
                ...
                ..RuntimeCostSnapshot::default()
            },
        );

        reset_runtime_costs();
        assert_eq!(runtime_cost_snapshot(), RuntimeCostSnapshot::default());
    }
    ...
    fn counters_do_not_cross_threads() {
        reset_runtime_costs();
        record_event_appended();
        let other = std::thread::spawn(runtime_cost_snapshot).join().unwrap();
        assert_eq!(other, RuntimeCostSnapshot::default());
        assert_eq!(runtime_cost_snapshot().events_appended, 1);
    }
}
