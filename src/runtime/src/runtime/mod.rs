...
use mech_core::{
    BROWSER_DOM_PROVIDER_URI, BrowserDomPath, CompileCtx, MResult, MechError, MechErrorKind,
    MechFunctionCompiler, MechFunctionImpl, MechSourceCode, ModuleManifestCatalog,
    ModuleManifestConfig, NativeFunctionCompiler, Register, Value, browser_capability_error,
    hash_str,
};
...
use crate::context::{
    ResourceBudget, RuntimeContext, RuntimeContextBase, RuntimeContextBinding,
    RuntimeContextBuilder, RuntimeContextCapabilityScope, RuntimeContextRegistry,
    RuntimeTurnOutcome,
};
...
use crate::resolver::{
    InMemorySourceResolver, ResolvedSource, SourceAddressReference, SourceExportDeclaration,
    SourceImportAlias, SourceImportKind, SourceRequest, SourceResolver, SourceScope,
...
use crate::{
    HostInstanceConfig, HostInterfaceCatalog, InMemoryDocsProvider, RunResourceGrantConfig,
    RuntimeCapabilityGrant, RuntimeCapabilityGrantDenied, RuntimeCapabilityGrantInput,
    RuntimeCapabilityGrantRegistry, RuntimeCapabilityOperation, RuntimeConfigSpec,
    RuntimeHostFactory, RuntimeHostFactoryRegistry, RuntimeResourceCapabilityDenied,
    RuntimeResourceProvider, RuntimeResourceReadRequest, RuntimeResourceRegistry,
    RuntimeResourceWriteIntent, RuntimeResourceWriteRequest, register_config_spec_grants,
    register_config_spec_resources, resource_base_matches,
};
...
fn default_host_interfaces() -> MResult<HostInterfaceCatalog> {
    ...
    catalog.register(crate::MaterializedHostInterface {
        ...
        contexts: vec![
            crate::MaterializedHostContext {
                name: "env".to_string(),
                base_uri: "cli://cli/env".to_string(),
            ...
            crate::MaterializedHostContext {
                ...
                base_uri: "cli://cli/stderr".to_string(),
                operations: vec!["write".to_string()],
            },
        ],
    })?;
    catalog.register(crate::MaterializedHostInterface {
        ...
        contexts: vec![crate::MaterializedHostContext {
            name: "dom".to_string(),
            base_uri: mech_core::BROWSER_DOM_PROVIDER_URI.to_string(),
...
impl RuntimeBuilder {
    ...
    pub fn build(mut self) -> MResult<MechRuntime> {
        ...
        for host_instance in &self.host_instances {
            ...
            if host_interfaces
                .interface(&installation.interface.instance)
                .is_none()
            {
...
impl MechErrorKind for BrowserRuntimeResourceError {
    ...
    fn message(&self) -> String {
        format!(
            "browser runtime resource `{}` failed: {}",
...
impl MechRuntime {
    ...
    pub fn bind_context_export(&mut self, alias: &str, module: &str, item: &str) -> MResult<()> {
    ...
    pub fn read_bound_resource(&self, binding: &str, child_path: &str) -> MResult<Value> {
    ...
    pub fn read_browser_dom_resource(&self, binding: &str, child_path: &str) -> MResult<Value> {
    ...
    pub fn apply_config_spec(&mut self, spec: RuntimeConfigSpec) -> MResult<()> {
    ...
    pub fn read_resource(&self, request: RuntimeResourceReadRequest) -> MResult<Value> {
