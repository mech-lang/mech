...
pub fn module_id(name: &str) -> ModuleId {
    ModuleId(blake3_u128(b"mech.module.id.v1", &[name.as_bytes()]))
}
...
pub fn module_version_id(
    ...
    let dep_bytes: Vec<u8> = deps.iter().flat_map(|dep| dep.0.to_le_bytes()).collect();
...
mod tests {
    ...
    fn module_version_id_includes_module_identity() {
        let first = module_version_id(
            module_id("memory://first.mec"),
            "x := 1",
            "0.3.5",
            "2021",
            "native",
        ...
        let second = module_version_id(
            module_id("memory://second.mec"),
            "x := 1",
            "0.3.5",
            "2021",
            "native",
    ...
    fn module_version_inputs_remain_identity_significant() {
        let module = module_id("memory://main.mec");
        let baseline =
            module_version_id(module, "x := 1", "0.3.5", "2021", "native", &[], &[], &[]);
        let feature = module_version_id(
            module,
            "x := 1",
            "0.3.5",
            "2021",
        ...
        let dependency = module_version_id(
            module,
            "x := 1",
            "0.3.5",
            "2021",
        ...
        let capability = module_version_id(
            module,
            "x := 1",
            "0.3.5",
            "2021",
