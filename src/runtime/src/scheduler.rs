...
impl ScheduledWork {
    ...
    pub fn queued_event(&self) -> RuntimeEventKind {
        RuntimeEventKind::SchedulerWorkQueued { work: self.label() }
    }

    pub fn started_event(&self) -> RuntimeEventKind {
        RuntimeEventKind::SchedulerWorkStarted { work: self.label() }
    }

    pub fn completed_event(&self) -> RuntimeEventKind {
        RuntimeEventKind::SchedulerWorkCompleted { work: self.label() }
    }
...
impl Scheduler for InMemoryScheduler {
    ...
    fn complete_work(&mut self, work: ScheduledWork, outcome: RuntimeTurnOutcome) -> MResult<()> {
        ...
        self.completed.push(ScheduledWorkOutcome { work, outcome });
    ...
    fn complete_tick(&mut self, work_count: u64) -> MResult<()> {
        self.push_event(RuntimeEventKind::RuntimeTickCompleted { work_count });
...
impl MechErrorKind for InvalidScheduledWorkError {
    ...
    fn message(&self) -> String {
        format!(
            "Invalid scheduled work field `{}`: {}",
            self.field, self.reason
        )
    }
}

fn invalid_scheduled_work<T>(field: &'static str, reason: &'static str) -> MResult<T> {
    Err(MechError::new(
...
mod tests {
    ...
    fn collect_tick_respects_limit_and_records_events() {
        ...
        let policy = SchedulerPolicy::default().with_max_turns_per_tick(2);
