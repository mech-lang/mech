...
pub(super) fn discover_workspace_files(
    ...
    Vec<RuntimeWorkspaceDiscoveredFile>,
    Vec<RuntimeWorkspaceDiagnostic>,
)> {
    ...
    for folder in folders {
        ...
        if let Some(subject) = capability_subject {
            if let Err(error) = crate::check_fs_capability(
        ...
        for result in builder.build() {
            ...
            discovered.insert(
                ...
                RuntimeWorkspaceDiscoveredFile {
...
fn workspace_folder_path(root: &Path, folder: &RuntimeWorkspaceFolder) -> MResult<PathBuf> {
...
mod tests {
    ...
    fn discovery_denial_returns_diagnostic_without_files() {
        ...
        let mut runtime = RuntimeBuilder::new()
            ...
            .build()
            .unwrap();
        let (files, diagnostics) = discover_workspace_files(
            &root,
        ...
        assert!(
            diagnostics
                .iter()
                .any(|diagnostic| diagnostic.message.contains("CapabilityDenied"))
        );
