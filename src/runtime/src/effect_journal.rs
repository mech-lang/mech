...
pub(in crate::runtime) enum RuntimeEffectState {
    ...
    Prepared,
    Applied,
    /// Compensation was invoked but returned an error. Its external outcome
    /// is ambiguous, so no cleanup path may invoke it again.
...
impl RuntimeEffectJournal {
    ...
    pub(in crate::runtime) fn stage_exact(
        ...
        id: RuntimeEffectId,
        effect: PreparedRuntimeEffect,
    ) -> Result<(), RuntimeEffectStageFailure> {
        let validation = (|| -> MResult<u64> {
            if id.sequence < self.next_sequence {
                return Err(MechError::new(
                    RuntimeInvalidOperationError {
                        operation: "stage_exact",
                        reason: format!(
                            "effect sequence {} precedes the next exact sequence {}",
            ...
            if self
                .entries
                .first()
                .is_some_and(|entry| entry.id.transaction != id.transaction)
            {
                return Err(MechError::new(
                    RuntimeInvalidOperationError {
                        operation: "stage_exact",
                        reason: "exact resident effects must share one transaction identity"
                            .to_owned(),
                    },
                    None,
                ));
            }
            id.sequence.checked_add(1).ok_or_else(|| {
                MechError::new(
                    RuntimeInvalidOperationError {
                        operation: "stage_exact",
                        reason: "effect sequence exhausted".to_owned(),
                    },
                    None,
                )
            })
        })();
        let next_sequence = match validation {
            Ok(next_sequence) => next_sequence,
            Err(error) => {
                let mut entry = RuntimeEffectEntry {
                    id,
                    state: RuntimeEffectState::Staged,
                    effect,
                    resource_write: None,
                };
                let cleanup = abort_effect_entry(&mut entry).err().into_iter().collect();
                return Err(RuntimeEffectStageFailure { error, cleanup });
            }
        };
    ...
    pub(in crate::runtime) fn deliver_after_commit_exact(
        ...
        let Some(entry) = self.entries.iter_mut().find(|entry| entry.id == id) else {
            return Err(RuntimeEffectFailure {
                effect_id: id,
                phase: RuntimeEffectFailurePhase::Deliver,
                message: "prepared after-commit effect is absent from the journal".to_owned(),
            });
        };
    ...
    pub(in crate::runtime) fn validate_after_commit_exact(
        &mut self,
        id: RuntimeEffectId,
    ) -> Result<(), RuntimeAfterCommitValidationFailure> {
        let Some(entry) = self.entries.iter_mut().find(|entry| entry.id == id) else {
            return Err(RuntimeAfterCommitValidationFailure {
                failure: RuntimeEffectFailure {
                    effect_id: id,
                    phase: RuntimeEffectFailurePhase::Prepare,
                    message: "prepared after-commit effect is absent from the journal".to_owned(),
                },
                cleanup: Vec::new(),
            });
        };
        validate_prepared_after_commit(id, &mut entry.effect)
    }
}
...
pub(in crate::runtime) fn validate_prepared_after_commit(
    ...
    if matches!(effect, PreparedRuntimeEffect::AfterCommit(_)) {
        return Ok(());
    }
    let cleanup = match effect {
        PreparedRuntimeEffect::Transactional(effect) => invoke_extension(
            format!("mismatched transactional effect {id}"),
            "abort",
            || effect.abort(),
        ),
        PreparedRuntimeEffect::Compensatable(effect) => invoke_extension(
            format!("mismatched compensatable effect {id}"),
            "abort",
            || effect.abort(),
        ),
        PreparedRuntimeEffect::AfterCommit(_) => unreachable!(),
    };
    let cleanup = match cleanup {
        Ok(()) => Vec::new(),
        Err(error) => vec![RuntimeEffectFailure {
            effect_id: id,
            phase: RuntimeEffectFailurePhase::Abort,
            message: format!("mismatched after-commit handle abort failed: {error:?}"),
        }],
    };
    Err(RuntimeAfterCommitValidationFailure {
        failure: RuntimeEffectFailure {
            effect_id: id,
            phase: RuntimeEffectFailurePhase::Prepare,
            message: if cleanup.is_empty() {
                "prepared effect protocol is not AfterCommit; mismatched handle aborted".to_owned()
            } else {
                "prepared effect protocol is not AfterCommit; mismatched handle cleanup is indeterminate"
                    .to_owned()
            },
        },
        cleanup,
...
pub(in crate::runtime) fn deliver_prepared_after_commit(
    ...
    let PreparedRuntimeEffect::AfterCommit(effect) = effect else {
        return Err(RuntimeEffectFailure {
            effect_id: id,
            phase: RuntimeEffectFailurePhase::Deliver,
            message: "prepared effect protocol is not AfterCommit".to_owned(),
        });
    };
    invoke_extension(format!("after-commit effect {id}"), "deliver", || {
        effect.deliver()
    })
    .map_err(|error| RuntimeEffectFailure {
        effect_id: id,
        phase: RuntimeEffectFailurePhase::Deliver,
        message: format!("{error:?}"),
    })
...
fn abort_effect_entry(entry: &mut RuntimeEffectEntry) -> Result<(), RuntimeEffectFailure> {
    if entry.state == RuntimeEffectState::CompensationIndeterminate {
        return Err(RuntimeEffectFailure {
            effect_id: entry.id,
            phase: RuntimeEffectFailurePhase::Compensate,
            message: "compensation outcome is indeterminate and must not be retried".to_owned(),
        });
    }
    let result = match (&mut entry.effect, entry.state) {
        ...
        (
            PreparedRuntimeEffect::Compensatable(_),
            RuntimeEffectState::CompensationIndeterminate,
        )
        | (
            PreparedRuntimeEffect::Transactional(_),
            RuntimeEffectState::CompensationIndeterminate,
        ) => {
            unreachable!("indeterminate compensation is handled without reinvocation")
        }
    };
...
#[cfg(test)]
#[path = "runtime/transaction/tests/effects/mod.rs"]
mod tests;
