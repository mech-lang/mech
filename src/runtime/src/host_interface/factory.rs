...
use crate::extension::{invoke_extension, invoke_extension_value};
...
impl RuntimeHostFactoryRegistry {
    ...
    pub fn register(&mut self, factory: Box<dyn RuntimeHostFactory>) -> MResult<()> {
        let provider = invoke_extension_value("host factory", "provider_name", || {
            factory.provider_name().to_string()
        })?;
        let manifest =
            invoke_extension_value("host factory", "manifest", || factory.manifest().clone())?;
    ...
    pub fn instantiate(&self, config: &HostInstanceConfig) -> MResult<RuntimeHostInstallation> {
        ...
        invoke_extension("host factory", "validate_settings", || {
            factory.validate_settings(&config.name, &config.settings)
        })?;
        let installation = invoke_extension("host factory", "instantiate", || {
