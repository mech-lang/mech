...
pub struct RuntimeHostFactoryRegistry { factories: HashMap<String, Box<dyn RuntimeHostFactory>> }
...
#[derive(Debug, Clone)] struct RuntimeHostFactoryError { name: &'static str, message: String }
impl MechErrorKind for RuntimeHostFactoryError { fn name(&self) -> &str { self.name } fn message(&self) -> String { self.message.clone() } }
fn error(name: &'static str, message: impl Into<String>) -> MechError { MechError::new(RuntimeHostFactoryError { name, message: message.into() }, None) }
