...
impl MechErrorKind for InvalidStoreRecordError {
  ...
  fn message(&self) -> String {
    format!("Invalid store record field `{}`: {}", self.field, self.reason)
  }
}

fn invalid_store_record<T>(
  field: &'static str,
  reason: &'static str,
) -> MResult<T> {
  Err(MechError::new(
...
mod tests {
  ...
  fn active_module_version_round_trip() {
    ...
    store.put_module(ModuleRecord::new(ModuleId(1), "main")).unwrap();
  ...
  fn task_round_trip() {
    ...
    let task = TaskRecord::new(TaskId(1), "task:1")
  ...
  fn transaction_round_trip() {
    ...
    let loaded = store
      .get_transaction(TransactionId(1))
      .unwrap()
      .unwrap();
