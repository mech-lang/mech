...
impl ModuleVersionRecord {
    ...
    pub fn validate_import_edges(&self) -> MResult<()> {
        ...
        for import in self
            .imports
            .iter()
            .filter(|import| import_requires_edge(import))
        {
            let Some((index, _)) = self
                .import_edges
                .iter()
                .enumerate()
                .find(|(index, edge)| !matched_edges[*index] && &edge.import == import)
            else {
                return Err(MechError::new(
                    InvalidModuleImportEdgesError {
                        module: self.id,
                        reason: format!(
                            "required source dependency import `{}` has no import edge",
                            import.specifier
                        ),
                    },
                    None,
                ...
                return Err(MechError::new(
                    InvalidModuleImportEdgesError {
                        ...
                        reason: format!(
                            ...
                            edge.import.specifier, edge.import.kind
                        ),
                    },
                    None,
                ));
                ...
                return Err(MechError::new(
                    InvalidModuleImportEdgesError {
                        ...
                        reason: format!(
                            ...
                            edge.import.specifier, edge.import.kind, edge.scope
                        ),
                    },
                    None,
                ));
...
impl MechErrorKind for InvalidModuleImportEdgesError {
    ...
    fn message(&self) -> String {
        format!(
            "invalid import edges for module version `{}`: {}",
...
impl InMemoryAppendEventFailureKind {
    fn matches(self, kind: &RuntimeEventKind) -> bool {
        matches!(
            ...
            ) | (Self::EffectAborted, RuntimeEventKind::EffectAborted { .. },)
        )
    }
...
mod tests {
    ...
    fn runtime_commit(
        ...
        RuntimeStoreCommit {
            transaction: TransactionRecord::new(TransactionId(id), "test"),
            module_puts,
            module_version_puts,
    ...
    fn runtime_commit_publishes_module_and_version_together() {
        let mut store = InMemoryStore::new();
        let module = ModuleRecord::new(module_id("memory://main.mec"), "memory://main.mec");
        let version = ModuleVersionRecord::new(ModuleVersionId(10), module.id, 1);
        ...
        assert_eq!(store.get_module_version(version.id).unwrap(), Some(version),);
    }
    ...
    fn runtime_commit_missing_module_owner_is_atomic() {
        let mut store = InMemoryStore::new();
        let version =
            ModuleVersionRecord::new(ModuleVersionId(10), module_id("memory://missing.mec"), 1);
    ...
    fn runtime_commit_missing_dependency_is_atomic() {
        ...
        assert!(
            store
                .commit_runtime(runtime_commit(1, vec![module.clone()], vec![version],))
                .is_err(),
        );
    ...
    fn identical_runtime_module_publication_is_idempotent() {
        ...
        store
            .commit_runtime(runtime_commit(
                2,
                vec![ModuleRecord::new(module.id, module.name.clone()).with_description("second")],
                vec![version],
            ))
            .unwrap();
    ...
    fn runtime_module_conflict_leaves_other_categories_unchanged() {
        ...
        conflict
            .object_puts
            .push(ObjectRecord::text(ObjectId(20), "note", "must not commit"));

        assert!(store.commit_runtime(conflict).is_err());
        assert!(store.get_object(ObjectId(20)).unwrap().is_none());
        assert!(store.get_transaction(TransactionId(2)).unwrap().is_none(),);
    }
    ...
    fn context_import_without_import_edge_validates() {
        let import = SourceImportDeclaration {
            ...
            kind: crate::resolver::SourceImportKind::Single {
                name: "dom".to_string(),
            },
        };
