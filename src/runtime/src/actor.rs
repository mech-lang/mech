...
use crate::id::{ActorId, MessageId, ModuleVersionId, ObjectId};

use crate::store::{ActorRecord, MessageRecord};
...
impl ActorTurn {
    pub fn new(actor: ActorRecord, message: MessageRecord) -> MResult<Self> {
        actor.validate()?;
        message.validate()?;
    ...
    pub fn validate(&self) -> MResult<()> {
        ...
        if self.message.actor != self.actor {
            return invalid_actor_turn("message.actor", "message actor does not match actor");
        }
...
fn invalid_actor_turn<T>(field: &'static str, reason: &'static str) -> MResult<T> {
    Err(MechError::new(
...
mod tests {
    ...
    fn actor_turn_builds_from_actor_and_message() {
        ...
        let message = MessageRecord::new(MessageId(4), ActorId(1), "ping", b"hello".to_vec());
    ...
    fn actor_turn_rejects_mismatched_message_actor() {
        ...
        let message = MessageRecord::new(MessageId(4), ActorId(2), "ping", b"hello".to_vec());
