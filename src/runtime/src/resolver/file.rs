...
impl SourceRequest {
    ...
    /// `FileSourceResolver`.
    pub fn from_filesystem_path(path: impl AsRef<Path>) -> MResult<Self> {
...
impl MechErrorKind for SourceFilesystemSpecifierInvalid {
    fn name(&self) -> &str {
        "SourceFilesystemSpecifierInvalid"
    }

    fn message(&self) -> String {
        format!(
            "Invalid filesystem source specifier `{}`: {}",
...
impl FileSourceResolver {
                    ...
                    for candidate in path_candidates(root, &specifier) {
                        if candidate.is_file() {
                            return self.authorize_resolved(
                                request,
                                canonicalize_source_path(&candidate)?,
                            );
                        }
                    }
                }
    ...
    fn normalize_referrer_path(&self, referrer: &str) -> MResult<Option<PathBuf>> {
        match parse_filesystem_source_specifier(referrer)? {
            FilesystemSourceSpecifier::OtherScheme => Ok(None),
            FilesystemSourceSpecifier::Ordinary(path)
    ...
    fn authorize_resolved(
        ...
        if request.referrer.is_some() {
            self.check(FS_IMPORT, &path)?;
        }
        Ok(Some(path))
    }
}
...
fn parse_filesystem_source_specifier(specifier: &str) -> MResult<FilesystemSourceSpecifier> {
    let Some((scheme, rest)) = split_uri_scheme(specifier) else {
        return Ok(FilesystemSourceSpecifier::Ordinary(PathBuf::from(
            specifier,
        )));
    };
    match scheme {
...
fn parse_fs_uri(specifier: &str, rest: &str) -> MResult<FilesystemSourceSpecifier> {
    ...
    Ok(FilesystemSourceSpecifier::RootRelative(
        normalize_root_relative_path(specifier, &decoded)?,
    ))
}
...
fn normalize_root_relative_path(specifier: &str, decoded: &str) -> MResult<PathBuf> {
    ...
    if looks_like_windows_drive_path(decoded) {
        return Err(filesystem_specifier_error(
            specifier,
            "Windows drive prefixes are not permitted in fs:// paths",
        ));
    }
    ...
    if path.is_absolute() {
        return Err(filesystem_specifier_error(
            specifier,
            "absolute paths are not permitted in fs:// paths",
        ));
    }
    ...
    for component in path.components() {
        match component {
            ...
            Component::ParentDir => {
                return Err(filesystem_specifier_error(
                    specifier,
                    "parent traversal is not permitted in fs:// paths",
                ));
            }
            Component::RootDir => {
                return Err(filesystem_specifier_error(
                    specifier,
                    "root components are not permitted in fs:// paths",
                ));
            }
            Component::Prefix(_) => {
                return Err(filesystem_specifier_error(
                    specifier,
                    "path prefixes are not permitted in fs:// paths",
                ));
            }
        }
    }
...
pub fn file_uri_to_path(uri: &str) -> MResult<PathBuf> {
    ...
    let (authority, raw_path) = if rest.starts_with('/') {
        ...
        return Err(filesystem_specifier_error(
            uri,
            "file URI must include an absolute path",
        ));
    };
...
fn local_file_uri_to_path(uri: &str, authority: &str, raw_path: &str) -> MResult<PathBuf> {
    ...
    if !(authority.is_empty() || authority.eq_ignore_ascii_case("localhost")) {
        return Err(filesystem_specifier_error(
            uri,
            "non-local file URI authorities are not supported on this platform",
        ));
    }
...
fn local_file_uri_to_path(uri: &str, authority: &str, raw_path: &str) -> MResult<PathBuf> {
    ...
    if authority.is_empty() || authority.eq_ignore_ascii_case("localhost") {
            ...
            && decoded_path.as_bytes()[3] == b'/'
        {
            ...
            return Err(filesystem_specifier_error(
                uri,
                "local Windows file URI path must include a drive prefix",
            ));
        };
    ...
    if path.is_empty() {
        return Err(filesystem_specifier_error(
            uri,
            "UNC file URI must include a share path",
        ));
    }
    ...
    while index < bytes.len() {
        if bytes[index] == b'%' {
            ...
            let high = hex_value(bytes[index + 1])
                .ok_or_else(|| filesystem_specifier_error(specifier, "malformed percent escape"))?;
...
fn percent_decode_to_string(specifier: &str, text: &str) -> MResult<String> {
    String::from_utf8(percent_decode_to_bytes(specifier, text)?).map_err(|error| {
        filesystem_specifier_error(
            specifier,
            format!("percent-decoded path is not valid UTF-8: {}", error),
        )
    })
}
...
pub fn read_runtime_source_file_with_capabilities(
    path: &Path,
...
fn read_runtime_source_file_with_capabilities_and_import_checks(
    ...
    match extension.as_str() {
        ...
        "html" | "htm" | "md" | "css" => Ok(MechSourceCode::Html(
            read_file_string_with_capabilities(path, kernel, subject)?,
        )),

        "mdoc" | "mpkg" | "m" | "csv" | "js" => Ok(MechSourceCode::String(
            read_file_string_with_capabilities(path, kernel, subject)?,
        )),

        "png" | "jpg" | "jpeg" | "gif" | "svg" => Ok(MechSourceCode::Image(
            extension,
            read_file_bytes_with_capabilities(path, kernel, subject)?,
        )),
...
fn check_optional_fs(
    kernel: Option<&SharedCapabilityKernel>,
    subject: Option<&str>,
    operation: &str,
    path: &Path,
) -> MResult<()> {
    if let (Some(kernel), Some(subject)) = (kernel, subject) {
        check_fs_capability(&mut kernel.clone(), subject, operation, path)?;
    }
    Ok(())
}

pub fn read_file_string_with_capabilities(
    path: &Path,
    kernel: Option<&SharedCapabilityKernel>,
    subject: Option<&str>,
) -> MResult<String> {
...
pub fn read_file_bytes_with_capabilities(
    path: &Path,
    kernel: Option<&SharedCapabilityKernel>,
    subject: Option<&str>,
) -> MResult<Vec<u8>> {
...
pub fn expand_mechdown_includes(path: &Path) -> MResult<String> {
    expand_mechdown_includes_with_capabilities(path, None, None)
}

pub fn expand_mechdown_includes_with_capabilities(
    path: &Path,
    kernel: Option<&SharedCapabilityKernel>,
    subject: Option<&str>,
) -> MResult<String> {
    ...
    for line in source.split_inclusive('\n') {
        ...
        if let Some(inner) = standalone_braced_content(line_without_newline) {
            if looks_like_mech_include(inner) {
                ...
                let include_source = expand_mechdown_includes_inner(
                    &include_canonical,
                    active,
                    kernel,
                    subject,
...
mod tests {
    ...
    fn resolves_fs_uri_beneath_configured_root() {
        ...
        let resolver = FileSourceResolver::new(&root);
        let resolved = resolver
            .resolve(&SourceRequest::new("fs://lib/dep.mec"))
            .unwrap()
            .unwrap();
    ...
    fn rejects_fs_uri_parent_traversal_before_filesystem_probe() {
        ...
        let resolver = FileSourceResolver::new(&root);
        let error = resolver
            .resolve(&SourceRequest::new("fs://../dep.mec"))
            .unwrap_err();
        assert!(
            error
                .kind_as::<SourceFilesystemSpecifierInvalid>()
                .is_some()
        );
        std::fs::remove_dir_all(root).unwrap();
    }
    ...
    fn resolves_percent_encoded_fs_uri_path() {
        ...
        let resolver = FileSourceResolver::new(&root);
        let resolved = resolver
            .resolve(&SourceRequest::new("fs://lib/space%20dep.mec"))
            .unwrap()
            .unwrap();
    ...
    fn resolves_file_uri_localhost_path() {
        ...
        let path = root.join("main.mec");
        std::fs::write(&path, "value := 1").unwrap();
        let uri = path_to_file_uri(&path.canonicalize().unwrap())
            .unwrap()
    ...
    fn other_uri_schemes_are_ignored_by_file_resolver() {
        ...
        assert!(
            resolver
                .resolve(&SourceRequest::new("https://example.com/main.mec"))
                .unwrap()
                .is_none()
        );
        assert!(
            resolver
                .resolve(&SourceRequest::new("memory://main.mec"))
                .unwrap()
                .is_none()
        );
        std::fs::remove_dir_all(root).unwrap();
    ...
    fn supports_windows_drive_and_unc_file_uri_forms() {
        assert_eq!(
            file_uri_to_path("file:///C:/project/main.mec").unwrap(),
            PathBuf::from("C:\\project\\main.mec")
        );
        assert_eq!(
            file_uri_to_path("file://server/share/project/main.mec").unwrap(),
            PathBuf::from("\\\\server\\share\\project\\main.mec")
        );
    }
    ...
    fn windows_path_codec_normalizes_drive_and_verbatim_drive_prefixes() {
        ...
        let verbatim = path_to_file_uri(Path::new(r"\\?\C:\project dir\main.mec")).unwrap();
    ...
    fn windows_path_codec_normalizes_unc_and_verbatim_unc_prefixes() {
        let unc = path_to_file_uri(Path::new(r"\\server\share\project dir\main.mec")).unwrap();
        let verbatim =
            path_to_file_uri(Path::new(r"\\?\UNC\server\share\project dir\main.mec")).unwrap();
    ...
    fn resolves_absolute_file_directory_index_extensionless_and_missing() {
        ...
        let module = resolver
            .resolve(&SourceRequest::new(
                root.join("module").to_string_lossy().to_string(),
            ))
            .unwrap()
            .unwrap();
        ...
        let other = resolver
            .resolve(&SourceRequest::new(
                root.join("other").to_string_lossy().to_string(),
            ))
            .unwrap()
            .unwrap();
...
mod capability_tests {
    use super::*;
    use crate::{
        FS_IMPORT, FS_READ, FS_RESOLVE, HostFilesystemAuthority, MECH_TOOL_SUBJECT,
        SERVE_HOST_SUBJECT, SequentialIdGenerator,
    };

    fn temp_root(label: &str) -> PathBuf {
        let root = std::env::temp_dir().join(format!(
            "mech-resolver-capability-{}-{}",
            label,
    ...
    fn resolver(root: &Path, allowed: &Path) -> FileSourceResolver {
        let mut ids = SequentialIdGenerator::new();
        let mut authority =
            HostFilesystemAuthority::new(MECH_TOOL_SUBJECT, SharedCapabilityKernel::new());
        authority
            .grant_path(&mut ids, allowed, true, [FS_READ, FS_RESOLVE, FS_IMPORT])
            .unwrap();
        authority
            .delegate_path_to(
                &mut ids,
    ...
    #[test]
    fn denies_source_outside_grant() {
        let root = temp_root("outside");
        let allowed = root.join("allowed");
        let outside = root.join("outside");
        std::fs::create_dir_all(&allowed).unwrap();
        std::fs::create_dir_all(&outside).unwrap();
        std::fs::write(allowed.join("main.mec"), "x := 1\n").unwrap();
        std::fs::write(outside.join("secret.mec"), "x := 2\n").unwrap();
        let resolver = resolver(&root, &allowed);
        assert!(
            resolver
    ...
    #[test]
    fn denies_include_outside_grant() {
        let root = temp_root("include");
        let allowed = root.join("allowed");
        let outside = root.join("outside");
        std::fs::create_dir_all(&allowed).unwrap();
        std::fs::create_dir_all(&outside).unwrap();
        std::fs::write(allowed.join("main.mec"), "{../outside/secret.mec}\n").unwrap();
        std::fs::write(outside.join("secret.mec"), "x := 2\n").unwrap();
        let resolver = resolver(&root, &allowed);
        assert!(
            resolver
    ...
    fn imported_source_requires_import_capability_for_each_include() {
        ...
        let mut authority =
            HostFilesystemAuthority::new(MECH_TOOL_SUBJECT, SharedCapabilityKernel::new());
        authority
            .grant_path(&mut ids, &allowed, true, [FS_READ, FS_RESOLVE])
            .unwrap();
        authority
            .grant_path(&mut ids, &main, false, [FS_IMPORT])
            .unwrap();
        authority
            .grant_path(&mut ids, &child, false, [FS_IMPORT])
            .unwrap();
        authority
            .delegate_path_to(
                &mut ids,
            ...
            .unwrap();
        authority
            .delegate_path_to(&mut ids, SERVE_HOST_SUBJECT, &main, false, [FS_IMPORT])
            .unwrap();
        authority
            .delegate_path_to(&mut ids, SERVE_HOST_SUBJECT, &child, false, [FS_IMPORT])
            .unwrap();
        let resolver = FileSourceResolver::new(&root)
            .with_capabilities(authority.kernel().clone(), SERVE_HOST_SUBJECT);
        ...
        assert!(
            resolver
                .resolve(
                    &SourceRequest::new("allowed/main.mec").with_referrer("memory://parent.mec")
                )
                .is_err()
        );
