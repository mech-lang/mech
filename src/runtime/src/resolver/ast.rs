...
mod tests {
    ...
    fn resolved_source_extracts_resource_context() {
        ...
        assert_eq!(
            contexts[0].base,
            SourceContextBase::ResourceUri("db://main".to_string())
        );
        assert_eq!(contexts[0].capabilities.len(), 2);
        assert_eq!(contexts[0].capabilities[0].operation, "read");
        assert_eq!(
            contexts[0].capabilities[0].scope,
            SourceContextCapabilityScope::Path("users/*".to_string())
        );
        assert_eq!(contexts[0].capabilities[1].operation, "write");
        assert_eq!(
            contexts[0].capabilities[1].scope,
            SourceContextCapabilityScope::Path("users/name".to_string())
        );
    }
    ...
    fn resolved_source_extracts_derived_context() {
        ...
        assert_eq!(
            contexts[1].base,
            SourceContextBase::Context("main".to_string())
        );
    }
    ...
    fn source_index_collects_program_declarations() {
        ...
        assert!(matches!(
            ...
            SourceDeclaration::Context(_)
        ));
        assert!(matches!(
            index.declarations[1],
            SourceDeclaration::Import(_)
        ));
        assert!(matches!(
            index.declarations[2],
            SourceDeclaration::Export(_)
        ));
    }
    ...
    fn source_index_collects_fenced_address_reference() {
        ...
        let foo_scope = index
            .module_scopes()
            .into_iter()
            .find(|metadata| match &metadata.scope {
