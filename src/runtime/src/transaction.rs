...
use crate::id::{ActorId, EventId, MessageId, ObjectId, TaskId, TransactionId};

use crate::event::RuntimeEvent;
use crate::store::{ActorRecord, MessageRecord, ObjectRecord, TaskRecord, TransactionRecord};
...
impl RuntimeTransaction {
    ...
    pub fn stage_event(&mut self, event: RuntimeEvent) -> MResult<EventId> {
        self.ensure_open()?;
        event.validate()?;
    ...
    pub fn stage_message_ack(&mut self, actor: ActorId, message: MessageId) -> MResult<()> {
        self.ensure_open()?;
    ...
    pub fn staged_message_ack_occurrences(&self, actor: ActorId, message: MessageId) -> usize {
        self.staged_message_acks
            .get(&actor)
            .map(|messages| {
                messages
    ...
    pub fn staged_message_acks(&self) -> impl Iterator<Item = (&ActorId, &Vec<MessageId>)> {
        self.staged_message_acks.iter()
    }

    pub fn pop_staged_enqueued_message(&mut self, actor: ActorId) -> Option<MessageRecord> {
    ...
    pub fn peek_staged_enqueued_message(&self, actor: ActorId) -> Option<MessageRecord> {
        self.staged_message_enqueues
            .get(&actor)
    ...
    pub fn staged_message_enqueues(&self) -> impl Iterator<Item = (&ActorId, &Vec<MessageRecord>)> {
...
fn invalid_runtime_transaction<T>(field: &'static str, reason: &'static str) -> MResult<T> {
    Err(MechError::new(
