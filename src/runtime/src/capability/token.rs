        ...
        if let Some(not_before) = self.not_before_ms {
            if not_before < self.issued_at_ms {
                return Err(MechError::new(
                    InvalidCapabilityTokenError {
                        token: self.id,
                        reason: "not_before_ms must be greater than or equal to issued_at_ms"
                            .to_string(),
                    },
                    None,
                ));
        ...
        if let Some(expires_at) = self.expires_at_ms {
            if expires_at < self.issued_at_ms {
                return Err(MechError::new(
                    InvalidCapabilityTokenError {
                        token: self.id,
                        reason: "expires_at_ms must be greater than or equal to issued_at_ms"
                            .to_string(),
                    },
                    None,
                ));
            ...
            if let Some(not_before) = self.not_before_ms {
                if expires_at < not_before {
                    return Err(MechError::new(
                        InvalidCapabilityTokenError {
                            token: self.id,
                            reason: "expires_at_ms must be greater than or equal to not_before_ms"
                                .to_string(),
                        },
                        None,
                    ));
    ...
    pub fn validate(&self) -> MResult<()> {
        ...
        if let (Some(start), Some(end)) = (self.not_before_ms, self.not_after_ms) {
            if start > end {
                return invalid_key(
                    "not_before_ms",
                    "must be less than or equal to not_after_ms",
                );
            }
        }
        ...
        if let Some(audience) = token.audience() {
            let Some(expected) = request.audience else {
                return Err(MechError::new(
                    InvalidCapabilityTokenError {
                        token: token.id(),
                        reason: "token has an audience but no expected audience was supplied"
                            .to_string(),
                    },
                    None,
                ));
            ...
            if audience != expected {
                return Err(MechError::new(
                    InvalidCapabilityTokenError {
                        token: token.id(),
                        reason: format!(
                            "wrong audience: expected `{}`, actual `{}`",
                            expected, audience
                        ),
                    },
                    None,
