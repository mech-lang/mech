...
pub fn fs_resource_key(path: &Path) -> MResult<String> {
    Ok(format!("fs://{}", normalized_fs_path(path)?))
}
...
pub fn fs_request(subject: &str, operation: &str, path: &Path) -> MResult<CapabilityRequest> {
    Ok(CapabilityRequest::from_keys(
        subject,
        operation,
        fs_resource_key(path)?,
    ))
}

pub fn check_fs_capability(
    kernel: &mut dyn CapabilityKernel,
    subject: &str,
    operation: &str,
    path: &Path,
...
impl HostFilesystemAuthority {
    ...
    pub fn grant_path(
        ...
        let mut capability = BasicCapability::from_keys(
        ...
        .attenuable(true);
        if recursive {
            capability = capability.with_constraints(
                BasicConstraints::default().with_resource_prefix(fs_resource_prefix_for_dir(path)?),
            );
        }
        let id = self
            .kernel
    ...
    pub fn delegate_path_to(
        ...
        for source in &self.source_capabilities {
            let mut derivation = CapabilityDerivation::attenuate(
                *source,
                id_generator.capability_id(),
                &BasicSubject::new(&self.subject),
            )
        ...
        Err(last_error.unwrap_or_else(|| {
            MechError::new(
                CapabilityDeniedError {
                    ...
                    operation: "attenuate".to_string(),
                    resource,
                    reason: "requested path is outside the default Mech tool grant".to_string(),
                },
                None,
            )
        }))
...
mod tests {
    ...
    fn recursive_delegation_allows_nested_read() {
        ...
        authority
            ...
            .unwrap();
        authority
            .delegate_path_to(&mut ids, SERVE_HOST_SUBJECT, &child, true, [FS_READ])
            .unwrap();
        check_fs_capability(&mut authority.kernel, SERVE_HOST_SUBJECT, FS_READ, &file).unwrap();
    ...
    fn delegation_outside_default_grant_fails() {
        ...
        let mut authority =
            HostFilesystemAuthority::new(MECH_TOOL_SUBJECT, SharedCapabilityKernel::new());
        authority
            .grant_path(&mut ids, &allowed, true, [FS_READ])
            .unwrap();
        assert!(
            authority
