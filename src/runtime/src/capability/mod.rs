...
mod tests {
    ...
    fn basic_capability_denies_wrong_operation() {
        ...
        let request = CapabilityRequest::new(&subject, &BasicOperation::write(), &resource);
    ...
    fn basic_capability_resource_prefix_allows_nested_resource() {
        ...
        let cap = BasicCapability::new(CapabilityId(1), &subject, &root, [BasicOperation::read()])
            .with_constraints(BasicConstraints::default().with_resource_prefix("db://"));

        let mut kernel = BasicCapabilityKernel::new();

        kernel.grant(CapabilityGrant::new(Arc::new(cap))).unwrap();

        let request = CapabilityRequest::new(&subject, &BasicOperation::read(), &users);
    ...
    fn attenuation_can_reduce_operations() {
        ...
        let derivation =
            CapabilityDerivation::attenuate(CapabilityId(1), CapabilityId(2), &subject)
        ...
        let read_request = CapabilityRequest::new(&subject, &BasicOperation::read(), &resource);

        assert!(derived.check(&read_request).unwrap().allowed);

        let write_request = CapabilityRequest::new(&subject, &BasicOperation::write(), &resource);
    ...
    fn rollback_grant_bypasses_non_revocable_policy() {
        let mut kernel = BasicCapabilityKernel::new();
        let capability =
            BasicCapability::from_keys(CapabilityId(1), "task://1", "db://users", [":read"])
                .revocable(false);
    ...
    fn basic_kernel_enforces_authority_allowlists() {
        ...
        for id in [CapabilityId(1), CapabilityId(2)] {
            kernel
                .grant(CapabilityGrant::new(Arc::new(BasicCapability::new(
                    id,
                    &subject,
                    &resource,
                    [BasicOperation::read()],
        ...
        assert!(
            kernel
                .check_scoped(&request, &RuntimeAuthorityScope::allow_list([]),)
                .is_err(),
        );
    }
    ...
    fn constrained_capability_denies_missing_metrics() {
        ...
        let request = request.with_context(CapabilityContext::local().with_bytes(10).with_items(2));
        assert!(!cap.check(&request).unwrap().allowed);
    ...
    fn pending_use_fixture(
        max_uses: u64,
    ) -> (BasicCapabilityKernel, CapabilityRequest, CapabilityId) {
        let subject = BasicSubject::new("task://pending");
        let resource = BasicResource::new("db://users");
        let id = CapabilityId(900);
        let capability = BasicCapability::new(id, &subject, &resource, [BasicOperation::read()])
        ...
        (
            kernel,
            CapabilityRequest::new(&subject, &BasicOperation::read(), &resource),
            id,
        )
    }
    ...
    impl CapabilityKernel for LegacyPreviewKernel {
        ...
        fn check(&mut self, _request: &CapabilityRequest) -> MResult<CapabilityId> {
            unimplemented!()
        }
        ...
        fn get(&self, _id: CapabilityId) -> MResult<Option<Arc<dyn Capability>>> {
            unimplemented!()
        }

        fn list_for_subject(&self, _subject: &dyn Subject) -> MResult<Vec<CapabilityId>> {
            unimplemented!()
        }
