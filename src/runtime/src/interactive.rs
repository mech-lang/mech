...
impl<F: ResidentReplRuntimeFactory> ResidentReplSession<F> {
    ...
    pub fn submit_finished_stream(
        ...
        candidate_source.push_str(&entry_source);
        ...
        let candidate = if self.source.is_empty() {
            ...
            crate::SourceDocument::parse_resolved(
                ...
                Arc::<str>::from(candidate_source.as_str()),
                ParseConfig::default(),
            )
    ...
    fn submit_without_source_echo(
        ...
        let entry_document = crate::SourceDocument::parse_resolved(
            "runtime:interactive-entry",
            Revision(self.source_revision().saturating_add(1)),
            Arc::<str>::from(appended_source.as_str()),
            ParseConfig::default(),
        )
        .map_err(|error| interactive_error(format!("invalid interactive source: {error:?}")))?;
        if !entry_document.is_strictly_clean() {
            return Err(interactive_error(
                "interactive canonical document contains syntax diagnostics",
            ));
        }
        let changed_state_names = canonical_state_mutations(entry_document.document().syntax());
    ...
    pub fn clear_variables(&mut self, names: &[String]) -> MResult<Vec<String>> {
        ...
        let document = match self.source_document.clone() {
            Some(document) => document,
            None => crate::SourceDocument::parse_resolved(
                "runtime:interactive",
                Revision(self.source_revision()),
                Arc::<str>::from(self.source.as_str()),
                ParseConfig::default(),
            )
            .map_err(|error| interactive_error(format!("invalid interactive source: {error:?}")))?,
        };
