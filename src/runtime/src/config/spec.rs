...
use crate::{
    ...
    RuntimeCapabilityOperation, RuntimeResourceRegistry,
};
...
impl RuntimeConfigSpec {
    ...
    pub fn with_resource(mut self, resource: RuntimeResourceConfigSpec) -> Self {
        self.resources.push(resource);
        self
    }

    pub fn with_capability_grant(mut self, grant: RuntimeCapabilityGrantSpec) -> Self {
...
impl RuntimeCapabilityGrantSpec {
    ...
    pub fn with_operation(mut self, operation: RuntimeCapabilityOperation) -> Self {
        self.operations.push(operation);
        self
    }

    pub fn with_operation_name(mut self, operation: impl Into<String>) -> MResult<Self> {
        self.operations
            .push(RuntimeCapabilityOperation::from_name(operation)?);
        Ok(self)
    }
...
pub(crate) fn materialize_config_spec_grants(
    ...
    for grant in &spec.capability_grants {
        capabilities.push(Arc::new(ResourcePathCapability::from_spec(
