...
use mech_core::{
    ...
    MechSourceCode, NativeFunctionCompiler, Register, Value, hash_str,
};
...
use crate::context::{ResourceBudget, RuntimeContext, RuntimeContextBuilder, RuntimeTurnOutcome};

use crate::event::{RuntimeEvent, RuntimeEventKind};

use crate::host::{
    DefaultHostCallPolicy, HostCall, HostCallPolicy, HostFunction, HostFunctionNotFoundError,
    HostRegistry, InMemoryHostRegistry, default_host_capability_request,
};

use crate::id::{
    ActorId, CapabilityId, DefaultIdGenerator, EventId, IdGenerator, MessageId, ModuleId,
    ModuleVersionId, ObjectId, RuntimeId, TaskId, TransactionId, module_id,
};

use crate::resolver::{
    InMemorySourceResolver, ResolvedSource, SourceImportKind, SourceRequest, SourceResolver,
    module_namespace_for_import,
};

use crate::scheduler::{
    InMemoryScheduler, ScheduledWork, Scheduler, SchedulerPolicy, SchedulerTick, collect_tick,
};
...
impl RuntimeHostNativeFunctionCompiler {
    pub fn new(mech_name: impl Into<String>, host_name: impl Into<String>) -> Self {
        Self {
...
impl ModuleInstance {
    fn new(version: ModuleVersionId, result: Value) -> Self {
        Self {
            version,
            exports: HashMap::new(),
            result,
    ...
    fn with_exports(mut self, exports: HashMap<String, mech_core::ValRef>) -> Self {
...
impl MechRuntime {
    ...
    fn execute_module_isolated(
        ...
        if seen.contains(&version) {
            return Ok(ModuleInstance::new(version, Value::Empty));
        }
        seen.insert(version);
        ...
        let result = match source {
            MechSourceCode::String(source) => module_program.run_source(&MechSourceCode::String(
                strip_module_declarations_for_execution(&source),
            )),
            other => module_program.run_source(&other),
        };
        match &result {
            Ok(_) => self.emit_event_to_context(
                context,
                RuntimeEventKind::ProgramCompleted {
                    task_id: context.task,
                },
            )?,
            Err(error) => self.emit_event_to_context(
                context,
                RuntimeEventKind::ProgramFailed {
                    task_id: context.task,
                    message: format!("{:?}", error),
        ...
        let instance = ModuleInstance::new(version, result).with_exports(exports);
    ...
    fn build_import_environment(
        ...
        for ModuleImportEdge { import, dependency } in &importer.import_edges {
            let Some(instance) = self.module_instances.get(dependency) else {
                return Err(MechError::new(
                    RuntimeInvalidOperationError {
            ...
            match &import.kind {
                SourceImportKind::DependencyOnly | SourceImportKind::Namespace => {
                    if let Some(ns) = module_namespace_for_import(import) {
                        for (export_name, val) in &instance.exports {
                            let alias = format!("{}/{}", ns, export_name);
                            if let Some(prev) =
                                owners.insert(alias.clone(), import.specifier.clone())
                            {
                                return Err(MechError::new(
                            ...
                            bindings.insert(alias, val.clone());
                        }
                    }
                }
                SourceImportKind::Single { name } => {
                    let Some(val) = instance.exports.get(name) else {
                        return Err(MechError::new(
                            RuntimeModuleExportNotFound {
                    ...
                    if let Some(prev) = owners.insert(name.clone(), import.specifier.clone()) {
                        return Err(MechError::new(
                            RuntimeModuleImportConflict {
                                binding: name.clone(),
                                first_import: prev,
                ...
                SourceImportKind::Wildcard => {
                    for (export_name, val) in &instance.exports {
                        if let Some(prev) =
                            owners.insert(export_name.clone(), import.specifier.clone())
                        {
                            return Err(MechError::new(
                                RuntimeModuleImportConflict {
                        ...
                        bindings.insert(export_name.clone(), val.clone());
                    }
                }
            }
        }
    ...
    pub fn store_resolved_module_source(
        ...
        self.build_module_from_resolved_source_with_context(&mut context, resolved, options)
    }
    ...
    fn build_module_from_resolved_source_with_context_and_graph(
        ...
        let result = (|| -> MResult<ModuleVersionId> {
            ...
            let concrete_capability_requirements = resolved.capability_requirements.clone();

            let mut dependency_versions = Vec::new();
            let mut import_edges = Vec::new();
            for import in &resolved.imports {
                let dependency_request =
                    crate::resolver::source_request_for_import(import, Some(&canonical_uri));
            ...
            let module = self.ensure_module(&record.name, &record.canonical_uri)?;
            ...
            let version = ModuleVersionRecord::new(record.module_version, module, 1)
    ...
    pub fn resolve_and_store_module_source(
        ...
        self.build_module_from_request_with_context(&mut context, request, options)
    }
    ...
    pub fn put_source_module(
        ...
        self.put_source_module_with_context(&mut context, name, canonical_uri, source, options)
    }
    ...
    pub fn put_source_module_with_context(
        ...
        self.build_module_from_resolved_source_with_context(context, resolved, options)
    }
    ...
    pub fn get_capability(&self, id: CapabilityId) -> MResult<Option<Arc<dyn Capability>>> {
    ...
    pub fn put_task_with_context(
        ...
        self.emit_event_to_context(context, RuntimeEventKind::TaskCreated { task_id: id })?;
    ...
    pub fn start_task(
        ...
        self.emit_event_to_context(&mut context, RuntimeEventKind::TaskStarted { task_id: id })?;
    ...
    pub fn complete_task_with_context(
        ...
        self.emit_event_to_context(context, RuntimeEventKind::TaskCompleted { task_id: id })?;
    ...
    pub fn collect_tick_with_context(
        ...
        let tick = collect_tick(self.scheduler.as_mut(), &self.scheduler_policy)?;
    ...
    pub fn run_scheduled_work(&mut self, work: ScheduledWork) -> MResult<RuntimeTurnOutcome> {
        match work {
    ...
    pub fn run_scheduled_task(&mut self, task_id: TaskId) -> MResult<RuntimeTurnOutcome> {
        ...
        match result {
            Ok(()) => {
                ...
                self.complete_scheduled_work(ScheduledWork::task(task_id), outcome.clone())?;
            ...
            Err(error) => {
                ...
                self.abort_runtime_transaction(&mut context, message.clone())?;

                let _ = self.fail_task_with_context(&mut context, task_id, message.clone());

                self.fail_scheduled_work(ScheduledWork::task(task_id), message)?;
    ...
    pub fn run_actor_turn(&mut self, actor_id: ActorId) -> MResult<RuntimeTurnOutcome> {
        ...
        let result = (|| -> MResult<Option<ActorTurn>> {
            let Some(turn) = self.next_actor_turn_with_context(&mut context, actor_id)? else {
                return Ok(None);
            };
        ...
        match result {
            Ok(Some(_turn)) => {
                ...
                self.complete_scheduled_work(ScheduledWork::actor(actor_id), outcome.clone())?;
            ...
            Err(error) => {
                ...
                self.fail_scheduled_work(ScheduledWork::actor(actor_id), message)?;
    ...
    fn register_runtime_program_host_functions(
        ...
        for name in self.host_registry.list_functions()? {
            program.register_native_function_compiler(
                name.clone(),
                Arc::new(RuntimeHostNativeFunctionCompiler::new(name.clone(), name)),
            );
        }
    ...
    pub fn register_mech_host_function(
        ...
        self.host_registry.register_function(Arc::new(function))?;
    ...
    pub fn call_host_with_context(
        ...
        self.emit_event_to_context(
            context,
            RuntimeEventKind::HostCallStarted { name: name.clone() },
        )?;
        ...
        let Some(function) = self.host_registry.get_function(&call.name)? else {
            ...
            return Err(MechError::new(HostFunctionNotFoundError { name }, None));
        };
        ...
        let result = (|| -> MResult<Value> {
            ...
            let capability_request = function
                ...
                .unwrap_or_else(|| default_host_capability_request(context, function.name()));
        ...
        match &result {
            Ok(_) => {
                self.emit_event_to_context(context, RuntimeEventKind::HostCallCompleted { name })?;
            }
            Err(error) => {
                self.emit_event_to_context(
                    context,
...
impl MechErrorKind for RuntimeModuleExportLinkError {
    ...
    fn message(&self) -> String {
        format!(
            ...
            self.source, self.alias, self.reason,
        )
    }
}
