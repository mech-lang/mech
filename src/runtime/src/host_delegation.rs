...
impl MechErrorKind for HostDelegationSignatureError {
    ...
    fn message(&self) -> String {
        format!(
            "Host delegation signature verification failed: {}",
            self.reason
        )
    }
}
...
impl<P: HostDelegationPayload> HostDelegationEnvelope<P> {
    ...
    pub fn validate_unsigned_header(&self) -> MResult<()> {
        ...
        if self.header.algorithm != HOST_DELEGATION_ALGORITHM_ED25519 {
            return invalid(
                "header.algorithm",
                format!("must equal `{}`", HOST_DELEGATION_ALGORITHM_ED25519),
            );
        }
        if let Some(expires_at_ms) = self.header.expires_at_ms {
            if expires_at_ms < self.header.issued_at_ms {
                return invalid(
                    "header.expiresAtMs",
                    "must be greater than or equal to issuedAtMs",
                );
            }
        }
...
pub(crate) mod tests {
    ...
    fn generic_payload_kind_is_part_of_signing_payload() {
        let first = HostDelegationEnvelope::unsigned(
            header(),
            TestPayload {
                kind: "first",
        ...
        let second = HostDelegationEnvelope::unsigned(
            header(),
            TestPayload {
                kind: "second",
                value: "same".to_string(),
