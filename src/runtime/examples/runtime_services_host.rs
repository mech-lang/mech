...
fn main() -> MResult<()> {
    ...
    let actor_record = runtime.get_actor(actor)?.expect("actor should exist");
    ...
    let state_id = runtime
        .call_host_with_context(&mut context, HostCall::new("actor/state/id", Vec::new()))?;

    let old_state = runtime
