...
fn main() -> MResult<()> {
    ...
    let actor = runtime.create_actor("actor:transactional-mailbox", None, None, Vec::new())?;

    println!("actor: {}", actor);

    let first_message = runtime.send_message(actor, "first", b"keep me after abort".to_vec())?;

    let second_message =
        runtime.send_message(actor, "second", b"remove me after commit".to_vec())?;
    ...
    runtime.abort_runtime_transaction(&mut abort_context, "rollback staged mailbox ack")?;
    ...
    let enqueue_abort_transaction = runtime.begin_transaction(&mut enqueue_abort_context)?;
    ...
    runtime.abort_runtime_transaction(&mut enqueue_abort_context, "discard staged enqueue")?;
    ...
    let enqueue_commit_transaction = runtime.begin_transaction(&mut enqueue_commit_context)?;
