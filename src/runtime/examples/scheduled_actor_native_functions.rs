...
fn main() -> MResult<()> {
    ...
    for outcome in &outcomes {
        let reads: Vec<String> = outcome.access.reads.iter().map(|id| short(*id)).collect();

        let writes: Vec<String> = outcome.access.writes.iter().map(|id| short(*id)).collect();
    ...
    assert_eq!(
        updated_state_object.data, br#"{"count":1}"#,
    ...
    let transaction = transactions
        .iter()
        .find(|transaction| transaction.subject == "actor:native-function-counter")
