...
fn main() -> MResult<()> {
    ...
    let task = TaskRecord::new(runtime.next_task_id(), "program:runtime-dependency-diamond");
    ...
    assert_eq!(a_record.dependencies.len(), 1, "a should depend on shared",);

    assert_eq!(b_record.dependencies.len(), 1, "b should depend on shared",);
    ...
    let shared_resolve_count = counts.get("shared.mec").copied().unwrap_or(0);

    println!("shared resolve count: {}", shared_resolve_count);

    assert_eq!(
        shared_resolve_count, 2,
