...
use mech_runtime::{
    ...
    CapabilityId, HostCallActorBehaviorDriver, InMemoryHostRegistry, InMemorySourceResolver,
    ModuleBuildOptions, ObjectRecord, SourceRequest, register_actor_context_host_functions,
};
...
fn main() -> MResult<()> {
    ...
    runtime.put_object(ObjectRecord::text(initial_state, "actor-state", "count=0"))?;
    ...
    let message = runtime.send_message(actor, "increment", b"count by 1".to_vec())?;
    ...
    assert_eq!(
        updated_state_object.data, b"count=1",
    ...
    for transaction in &transactions {
        let reads: Vec<String> = transaction.read_set.iter().map(|id| short(*id)).collect();

        let writes: Vec<String> = transaction.write_set.iter().map(|id| short(*id)).collect();
        ...
        let events: Vec<String> = transaction.events.iter().map(|id| short(*id)).collect();
    ...
    let transaction = transactions
        .iter()
        .find(|transaction| transaction.subject == "actor:scheduled-services-host")
