...
pub fn print_recurse(node: &Node, level: usize, f: &mut fmt::Formatter) {
  ...
  let children: Option<&Vec<Node>> = match node {
    ...
    Node::TableSelect{children} => {write!(f,"TableSelect\n").ok(); Some(children)},
    ...
    Node::FlattenData{children} => {write!(f,"FlattenData\n").ok(); Some(children)},
...
impl Ast {
  ...
  pub fn build_syntax_tree(&mut self, node: &parser::Node) -> Vec<Node> {
    ...
    match node {
      ...
      parser::Node::FlattenData{children} => {
        let result = self.compile_nodes(children);
        let mut children: Vec<Node> = Vec::new();
        for node in result {
          match node {
        ...
        compiled.push(Node::FlattenData{children});
      },
      ...
      parser::Node::TableSelect{children} => {
        let result = self.compile_nodes(children);
        let mut children: Vec<Node> = Vec::new();
        for node in result {
          match node {
        ...
        compiled.push(Node::TableSelect{children});
      },      
