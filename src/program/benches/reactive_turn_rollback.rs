    ...
    *self.output.borrow_mut() += 1.0;
    if self.fail {
      return Err(MechError::new(
        mech_core::GenericError { msg: "benchmark tail failure".into() },
        None,
      ));
    }
...
fn add_combinational(
  interpreter: &Interpreter,
  input: Ref<f64>,
  output: Ref<f64>,
  fail: bool,
) {
  interpreter.plan().0.borrow_mut().register(
...
fn independent_fixture(
  length: usize,
) -> (MechProgram, ProgramInputId) {
  let mut program = MechProgram::new(MechProgramConfig::default());
  let mut selected = None;
  for index in 0..length {
    let (input, source) = root_input(
      &mut program,
      &format!("input-{index}"),
      index as f64,
    );
...
fn register_fixture(count: usize) -> (MechProgram, ProgramInputId) {
  ...
  for _ in 0..count {
    program.interpreter().plan().0.borrow_mut().register(
...
fn two_interpreter_fixture() -> (MechProgram, Vec<ProgramInputUpdate>) {
  ...
  for id in [10, 20] {
    ...
    program.interpreter().sub_interpreters.borrow_mut()
...
fn step_failure_fixture() -> MechProgram {
  ...
  for index in 0..100 {
    program.interpreter().plan().add_function(Box::new(BenchCombinational {
...
fn reactive_turn_rollback_benchmarks(c: &mut Criterion) {
  c.bench_function("reactive_turn_rollback/one_scalar_one_node", |b| {
    b.iter_batched(
      ...
      |(mut program, input)| black_box(
        program.update_inputs_and_advance_turn(&[update(input, 2.0)]).unwrap(),
      ),
  ...
  c.bench_function("reactive_turn_rollback/independent_1000_one_affected", |b| {
    b.iter_batched(
  ...
  c.bench_function("reactive_turn_rollback/two_affected_interpreters", |b| {
    b.iter_batched(
      ...
      |(mut program, updates)| black_box(
        program.update_inputs_and_advance_turn(&updates).unwrap(),
      ),
      BatchSize::SmallInput,
    )
  });
  c.bench_function("reactive_turn_rollback/failed_tail_with_rollback", |b| {
    b.iter_batched(
      ...
      |(mut program, input)| black_box(
        program.update_inputs_and_advance_turn(&[update(input, 2.0)]).unwrap_err(),
      ),
      BatchSize::LargeInput,
    )
  });
  ...
  c.bench_function("reactive_turn_rollback/full_checkpoint_independent_1000", |b| {
    b.iter_batched(
