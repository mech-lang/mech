...
fn passing_constraints(count: usize) -> String {
    (0..count)
        .map(|index| format!("integrity-{index}! := {index}.0 <= {index}.0"))
        .collect::<Vec<_>>()
        .join("\n")
}
...
fn failing_constraints(count: usize) -> String {
    (0..count)
        .map(|index| format!("integrity-{index}! := {}.0 < {index}.0", index + 1))
        .collect::<Vec<_>>()
        .join("\n")
}
