...
impl SemanticBuilder {
    ...
    fn match_expression(
        ...
        let arms = arms
            ...
            .collect::<Result<Vec<_>, SourceSemanticError>>()?;
        self.lower_match_expression(scrutinee, &arms, syntax, false, false)
    }
    ...
    fn lower_match_expression(
        ...
        if self.control_depth != 0 {
            return self.lower_match_expression_inner(scrutinee, arms, syntax, partial, activation);
        }
        let saved_next_control_block = self.next_control_block;
        self.next_control_block = 0;
        let result =
            self.lower_match_expression_inner(scrutinee, arms, syntax, partial, activation);
            ...
            if lowered_arm.guard.is_none() {
                match &lowered_arm.pattern {
                    ...
                    crate::MatchPattern::Structural(pattern)
                        if Self::structural_pattern_is_irrefutable(
                            pattern,
                    ...
                    crate::MatchPattern::Structural(crate::CollectionPattern::Enum {
                        ...
                        {
                            let payload_irrefutable = match &scrutinee_schema.body {
                                SchemaBody::Enum {
                                    variants: declarations,
                                    ..
                                } => {
                                    declarations.get(*ordinal as usize).is_some_and(
                                        |variant| match (&variant.payload, payload.as_deref()) {
                                            (None, None) => true,
                                            (Some(schema), Some(pattern)) => {
                                                Self::structural_pattern_is_irrefutable(
                                                    pattern, schema,
                                                )
                                            }
                                            _ => false,
