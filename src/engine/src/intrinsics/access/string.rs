            ...
            StringAccessSource::Mutable(r) => match &*r.borrow() {
                ...
                other => Err(MechError::new(
                    UnhandledFunctionArgumentKind2 {
                        ...
                        fxn_name: "access/scalar-string".to_string(),
                    },
                    None,
                )
                .with_compiler_loc()),
    ...
    fn specialize(&self, arguments: &[LegacyValue]) -> MResult<Box<dyn MechFunction>> {
        ...
        match (src.clone(), ix1.clone()) {
            ...
            (LegacyValue::String(s), LegacyValue::MutableReference(ix_ref)) => {
                ...
                let new_fxn = StringAccessElement {
                    source: StringAccessSource::Direct(s),
                    index: StringAccessIndex::Mutable(ix_ref),
                    out: Ref::new(grapheme),
                    compile_mode: StringAccessCompileMode::Dynamic,
                };
                Ok(Box::new(new_fxn))
            }
