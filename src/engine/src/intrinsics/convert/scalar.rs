...
#[cfg(all(feature = "matrix", feature = "table"))]
fn resolve_table_column_kinds(
    tbl: &Vec<(String, ValueKind)>,
    mat_knd: ValueKind,
) -> MResult<Vec<(String, ValueKind)>> {
    tbl.iter()
        .map(|(col_name, col_kind)| {
            if *col_kind == ValueKind::Any {
                ...
                Err(MechError::new(
                    ColumnConvertKindMismatchError {
                        from: mat_knd.clone(),
                        to: col_kind.clone(),
                    },
                    None,
...
impl MechFunctionImpl for ConvertMatToSet {
    fn solve_result(&self) -> MResult<()> {
        ...
        let converted_values = values
            .into_iter()
            .map(|value| {
                value.convert_to(&self.target_kind).unwrap_or_else(|| {
                    panic!(
                        "Unable to convert matrix element to target set kind {}",
                        self.target_kind
                    )
                })
            })
    ...
    match (&source_value, &target_kind) {
        ...
        (LegacyValue::MatrixString(mat), LegacyValue::Kind(ValueKind::Table(tbl, sze))) => {
            ...
            if in_shape[1] != tbl.len() {
                return Err(MechError::new(
                    ConvertIncorrectNumberOfColumnsError {
                        from: in_shape[1],
                        to: tbl.len(),
                    },
                    None,
            ...
            let out_rows = if *sze == 0 { in_shape[0] } else { *sze };
            let out = MechTable::from_kind(ValueKind::Table(resolved_tbl, out_rows))?;
            return Ok(Box::new(ConvertMat2Table::<String> {
                arg: mat.clone(),
                out: Ref::new(out),
            }));
        }
        ...
        (LegacyValue::MatrixBool(mat), LegacyValue::Kind(ValueKind::Table(tbl, sze))) => {
            ...
            let out_rows = if *sze == 0 { in_shape[0] } else { *sze };
            let out = MechTable::from_kind(ValueKind::Table(resolved_tbl, out_rows))?;
            return Ok(Box::new(ConvertMat2Table::<bool> {
                arg: mat.clone(),
                out: Ref::new(out),
            }));
        }
