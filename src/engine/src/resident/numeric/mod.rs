...
struct SnapshotAccessPlan {
    dense_source: Option<SnapshotAccessSelectorLayout>,
    ...
    output_dimensions_dynamic: bool,
...
fn bind_hold_state(
    ...
    if input.kind != request.output.kind || input.shape != request.output.shape {
        // A state whose closed initializer resolves aggregate dimensions only
        // during activation is snapshot-backed. Its turn candidate may still
        // have a fixed dense layout, so publish that single matrix through the
        // canonical snapshot constructor instead of inventing another state
        // conversion path.
        if request.output.kind == ResidentValueKind::Snapshot
            && request.output.shape == ResidentShape::SCALAR
        {
            return bind_matrix_constructor(request, true);
        }
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
...
fn bind_unary_float_snapshot(
    ...
    let output_schema = request
        ...
        .get(request.output.schema_id)
        .ok_or(ResidentKernelBindError::UnsupportedLayout)?;
    let input_element = match input_schema.body() {
    ...
    if input.kind != ResidentValueKind::Snapshot
        ...
        || core::mem::discriminant(input_schema.body())
            != core::mem::discriminant(output_schema.body())
    {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    validate_full_write(
    ...
    Ok(bound(executor, Vec::<u64>::new().into_boxed_slice())?
...
fn bind_unary_float(
    ...
    bind_unary_f64(request, f64_executor)
        .or_else(|_| bind_unary_float_snapshot(request, f32_executor))
}
...
macro_rules! float_unary_resident {
    ($binder:ident, $f64_executor:ident, $f32_executor:ident, $f64_operation:expr, $f32_operation:expr) => {
        ...
        fn $f32_executor(
            ...
            unary_float_snapshot(kernel, inputs, output, $f64_operation, $f32_operation)
        }
    };
}
...
fn bind_atan2(
    ...
    bind_binary(request, atan2).or_else(|_| bind_binary_float_snapshot(request, atan2_snapshot))
}
...
fn bind_math_copysign(
    ...
    bind_binary(request, math_copysign)
        .or_else(|_| bind_binary_float_snapshot(request, math_copysign_snapshot))
}
...
fn bind_math_fdim(
    ...
    bind_binary(request, math_fdim)
        .or_else(|_| bind_binary_float_snapshot(request, math_fdim_snapshot))
}
...
fn bind_math_fmod(
    ...
    bind_binary(request, math_fmod)
        .or_else(|_| bind_binary_float_snapshot(request, math_fmod_snapshot))
}
...
fn bind_math_nextafter(
    ...
    bind_binary(request, math_nextafter)
        .or_else(|_| bind_binary_float_snapshot(request, math_nextafter_snapshot))
}
...
fn bind_math_remainder(
    ...
    bind_binary(request, math_remainder)
        .or_else(|_| bind_binary_float_snapshot(request, math_remainder_snapshot))
}
...
fn bind_math_bessel_jn(
    ...
    bind_binary(request, math_bessel_jn)
        .or_else(|_| bind_binary_float_snapshot(request, math_bessel_jn_snapshot))
}
...
fn bind_math_bessel_yn(
    ...
    bind_binary(request, math_bessel_yn)
        .or_else(|_| bind_binary_float_snapshot(request, math_bessel_yn_snapshot))
}

fn bind_binary_float_snapshot(
    request: &ResidentKernelBindRequest<'_>,
    executor: mech_core::ResidentKernelExecutor,
) -> Result<BoundResidentKernel, ResidentKernelBindError> {
    ...
    let left_snapshot = left.kind == ResidentValueKind::Snapshot;
    let right_snapshot = right.kind == ResidentValueKind::Snapshot;
    let snapshot_output = request.output.kind == ResidentValueKind::Snapshot
        && request.output.shape == ResidentShape::SCALAR;
    let dense_output = request.output.kind == ResidentValueKind::F64
        && request.output.shape == output_shape
        && output_element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64);
    let dense_f64 = |port: &mech_core::ResidentPortLayout,
                     element: &SchemaBody,
                     logical_shape: ResidentShape| {
        port.kind == ResidentValueKind::F64
            && port.shape == logical_shape
            && element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
    };
    if (!snapshot_output && !dense_output)
        || (left_snapshot && left.shape != ResidentShape::SCALAR)
        || (right_snapshot && right.shape != ResidentShape::SCALAR)
        || (!left_snapshot && !dense_f64(left, left_element, left_shape))
        || (!right_snapshot && !dense_f64(right, right_element, right_shape))
    ...
    let kernel = bound(
        executor,
        vec![
            u64::from(snapshot_output),
            u64::from(left_snapshot),
            u64::from(right_snapshot),
            left_shape.rows as u64,
            left_shape.columns as u64,
            right_shape.rows as u64,
            right_shape.columns as u64,
        ]
        .into_boxed_slice(),
    )?
    .with_snapshot_schemas(request.schemas.clone());
    if snapshot_output {
        Ok(kernel.with_snapshot_output(snapshot_output_metadata(request)))
    } else {
        Ok(kernel)
    }
}
...
fn comparison_element_body(body: &SchemaBody) -> &SchemaBody {
    match body {
        SchemaBody::Matrix { element, .. } => element.as_ref(),
        body => body,
    }
...
fn dense_kind_matches_schema_body(kind: ResidentValueKind, body: &SchemaBody) -> bool {
    matches!(
        (kind, body),
...
fn bind_snapshot_comparison(
    ...
    let snapshot_output = request.output.kind == ResidentValueKind::Snapshot
        && request.output.shape == ResidentShape::SCALAR;
    let dense_output = if snapshot_output {
        validate_full_write(
            request,
            2,
            ShapeRule::Declared,
        ...
        Some(validate_semantic_comparison_contract(request)?)
    };
    let output_schema = request
        .schemas
        .get(request.output.schema_id)
    ...
    if snapshot_output && !matches!(output_schema.body(), SchemaBody::Matrix { .. }) {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let dense = |port: &mech_core::ResidentPortLayout,
                 element: &SchemaBody,
                 logical_shape: ResidentShape| {
        port.shape == logical_shape
            && matches!(
    ...
    if (!left_snapshot && !dense(left, left_element, left_shape))
        || (!right_snapshot && !dense(right, right_element, right_shape))
        || (!left_snapshot && !right_snapshot)
    {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    if left_element != right_element
        || output_element != &SchemaBody::Bool
    ...
    if let Some(output) = dense_output {
        let bound_mode = |snapshot: bool, shape| {
            if snapshot {
                // The snapshot's axes are runtime values. The executor checks
                // their resolved broadcast mode against the fixed output.
                Ok(u64::MAX)
            } else {
                binary_broadcast_mode(shape, output)
                    .ok_or(ResidentKernelBindError::UnsupportedLayout)
            }
        };
        parameters.extend([
            output.rows as u64,
            output.columns as u64,
            bound_mode(left_snapshot, left_shape)?,
            bound_mode(right_snapshot, right_shape)?,
        ]);
    }
    let kernel = bound(snapshot_comparison, parameters.into_boxed_slice())?
        .with_snapshot_schemas(request.schemas.clone());
    if snapshot_output {
        Ok(kernel.with_snapshot_output(snapshot_output_metadata(request)))
    } else {
...
fn bind_semantic_bool_and(
    ...
    bind_bool_and(request)
        .or_else(|_| bind_bool_vector_and(request))
        .or_else(|_| bind_snapshot_bool_binary(request, 0))
}
...
fn bind_semantic_bool_or(
    ...
    bind_bool_or(request)
        .or_else(|_| bind_bool_vector_binary(request, bool_vector_or))
        .or_else(|_| bind_snapshot_bool_binary(request, 1))
}
...
fn bind_semantic_bool_xor(
    ...
    bind_bool_xor(request)
        .or_else(|_| bind_bool_vector_binary(request, bool_vector_xor))
        .or_else(|_| bind_snapshot_bool_binary(request, 2))
}

fn bind_snapshot_bool_binary(
    request: &ResidentKernelBindRequest<'_>,
    operation: u64,
) -> Result<BoundResidentKernel, ResidentKernelBindError> {
    validate_full_write(
    ...
    for port in [left, right, &request.output] {
        let schema = request
            .schemas
            .get(port.schema_id)
            .ok_or(ResidentKernelBindError::UnsupportedLayout)?;
...
fn bind_bool_not(
    ...
    bind_dense_bool_not(request).or_else(|_| bind_snapshot_bool_not(request))
}

fn bind_dense_bool_not(
    request: &ResidentKernelBindRequest<'_>,
) -> Result<BoundResidentKernel, ResidentKernelBindError> {
    let [input] = request.inputs else {
        return Err(ResidentKernelBindError::UnsupportedLayout);
...
fn bind_snapshot_bool_not(
    ...
    for port in [input, &request.output] {
        let schema = request
            .schemas
            .get(port.schema_id)
            .ok_or(ResidentKernelBindError::UnsupportedLayout)?;
    ...
    let output_schema = request
        ...
        .get(request.output.schema_id)
        .ok_or(ResidentKernelBindError::UnsupportedLayout)?;
    if !matches!(input_schema.body(), SchemaBody::Matrix { .. })
        || !matches!(output_schema.body(), SchemaBody::Matrix { .. })
    {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    Ok(
        bound(snapshot_bool_not, Vec::<u64>::new().into_boxed_slice())?
            .with_snapshot_output(snapshot_output_metadata(request))
            .with_snapshot_schemas(request.schemas.clone()),
    )
}
...
fn bind_strict_comparison(
    ...
    if !strict_inputs_share_identity(request) {
        if let Some(parameters) = strict_mixed_layout_parameters(request) {
            return Ok(bound(executor, parameters)?.with_snapshot_schemas(request.schemas.clone()));
        }
...
fn strict_mixed_layout_parameters(request: &ResidentKernelBindRequest<'_>) -> Option<Box<[u64]>> {
    let [left, right] = request.inputs else {
        return None;
    };
    let left_snapshot =
    ...
    let (snapshot, dense, snapshot_left) = if left_snapshot {
        (left, right, true)
    } else {
        (right, left, false)
    };
    let snapshot_schema = request.schemas.get(snapshot.schema_id)?;
    let dense_schema = request.schemas.get(dense.schema_id)?;
    let (
        SchemaBody::Matrix {
            element: snapshot_element,
            ..
        },
        SchemaBody::Matrix {
            element: dense_element,
            dimensions,
        },
    ) = (snapshot_schema.body(), dense_schema.body())
    else {
        return None;
    };
    if snapshot_element != dense_element
        || dimensions.len() != 2
        || !dense_kind_matches_schema_body(dense.kind, dense_element)
    {
        return None;
    }
    let rows = dense
    ...
    let columns = dense
        ...
        .ok()?;
    let resident_shape =
        resident_shape_from_dimensions(usize::try_from(rows).ok()?, usize::try_from(columns).ok()?)
            .ok()?;
    if dense.shape != resident_shape {
        return None;
    }
    Some(vec![u64::from(snapshot_left), rows, columns].into_boxed_slice())
}
...
fn bind_snapshot_numeric_binary(
    ...
    if (left_snapshot && left.shape != ResidentShape::SCALAR)
        || (right_snapshot && right.shape != ResidentShape::SCALAR)
        || (!left_snapshot && !dense_f64(left, left_element, left_shape))
        || (!right_snapshot && !dense_f64(right, right_element, right_shape))
        || (!left_snapshot && !right_snapshot)
...
fn bind_add_assign(
    ...
    bind_f64_assign(request, add_assign)
        .or_else(|_| bind_snapshot_whole_numeric_assign(request, SemanticArithmetic::Add))
}
...
fn bind_sub_assign(
    ...
    bind_f64_assign(request, sub_assign)
        .or_else(|_| bind_snapshot_whole_numeric_assign(request, SemanticArithmetic::Subtract))
}
...
fn bind_mul_assign(
    ...
    bind_f64_assign(request, mul_assign)
        .or_else(|_| bind_snapshot_whole_numeric_assign(request, SemanticArithmetic::Multiply))
}
...
fn bind_div_assign(
    ...
    bind_f64_assign(request, div_assign)
        .or_else(|_| bind_snapshot_whole_numeric_assign(request, SemanticArithmetic::Divide))
}

fn bind_snapshot_whole_numeric_assign(
    request: &ResidentKernelBindRequest<'_>,
    arithmetic: SemanticArithmetic,
) -> Result<BoundResidentKernel, ResidentKernelBindError> {
    let kernel = bind_matrix_selection_assign(
        request,
...
fn bind_transpose(
    ...
    {
        return Ok(
            bound(transpose_snapshot, Vec::<u64>::new().into_boxed_slice())?
                .with_snapshot_output(snapshot_output_metadata(request))
                .with_snapshot_schemas(request.schemas.clone()),
        );
    }
...
fn bind_snapshot_sum(
    ...
    if input.kind != ResidentValueKind::Snapshot || input.shape != ResidentShape::SCALAR {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let input_schema = request
        .schemas
    ...
    let snapshot_output = request.output.kind == ResidentValueKind::Snapshot
        && request.output.shape == ResidentShape::SCALAR;
    let dense_f64_output = request.output.kind == ResidentValueKind::F64
    ...
    let mut parameters = vec![u64::from(column), u64::from(snapshot_output)];
    if dense_f64_output {
        parameters.extend([
            request.output.shape.rows as u64,
            request.output.shape.columns as u64,
    ...
    let kernel = bound(sum_snapshot, parameters.into_boxed_slice())?
        .with_snapshot_schemas(request.schemas.clone());
    if snapshot_output {
        Ok(kernel.with_snapshot_output(ResidentSnapshotOutput {
            schema: request.output.schema_id,
            schema_key: request.output.schema_key,
            shape: request.output.shape_instance.clone(),
            exact_cardinality: None,
...
#[derive(Clone, Debug)]
struct SnapshotConstructorInputPlan {
    schema: SchemaId,
    kind: ResidentValueKind,
...
#[derive(Clone, Debug)]
struct SnapshotConstructorPlan {
    inputs: Box<[SnapshotConstructorInputPlan]>,
    empty_dimensions: Option<(usize, usize)>,
}

fn dense_constructor_kind(element: &SchemaBody) -> Option<ResidentValueKind> {
    match element {
...
fn bind_matrix_constructor(
    ...
    let snapshot_layout = request.output.kind == ResidentValueKind::Snapshot
        && request.output.shape == ResidentShape::SCALAR;
    if snapshot_layout {
        let dense_kind = dense_constructor_kind(output_element);
        ...
        for input in request.inputs {
            let schema = request
                ...
                .get(input.schema_id)
                .ok_or(ResidentKernelBindError::UnsupportedLayout)?;
            let matrix = match schema.body() {
                body if body == output_element.as_ref() => false,
                SchemaBody::Matrix { element, .. } if element == output_element => true,
                _ => return Err(ResidentKernelBindError::UnsupportedLayout),
            };
            let dimensions = if input.kind == ResidentValueKind::Snapshot {
                if input.shape != ResidentShape::SCALAR {
                    return Err(ResidentKernelBindError::UnsupportedLayout);
                }
                None
            } else {
                if Some(input.kind) != dense_kind {
                    return Err(ResidentKernelBindError::UnsupportedLayout);
                }
                let dimensions = constructor_input_dimensions(request, input, output_element)?;
                if usize::try_from(input.shape.rows).ok() != Some(dimensions.0)
                    || usize::try_from(input.shape.columns).ok() != Some(dimensions.1)
                {
                    return Err(ResidentKernelBindError::UnsupportedLayout);
                }
                Some(dimensions)
            };
            inputs.push(SnapshotConstructorInputPlan {
                schema: input.schema_id,
                kind: input.kind,
                dimensions,
                matrix,
                shape_parameters: input.shape_instance.parameter_values().len(),
            });
        }
        let empty_dimensions = if inputs.is_empty() {
            Some(declared_matrix_dimensions(request, &request.output)?)
        } else {
            None
        };
        let plan = SnapshotConstructorPlan {
        ...
        return Ok(bound(
            ...
            vec![request.inputs.len() as u64].into_boxed_slice(),
        )?
...
fn validate_snapshot_access_geometry(
        ...
        (source_dimensions, selection_mode)
    {
    ...
    } else if source_dimensions.is_none() {
        match selection_mode {
            ...
            Some(mode) => {
                ...
                match mode {
                    ResolvedSelectionMode::LinearScalar
                        if output_dimensions.is_none() && !first_logical && first_count == 1 =>
                    {
                        ResolvedAccessGeometry {
                            logical_output_rows: 1,
                            logical_output_columns: 1,
...
fn bind_indexed_assign_with_region(
    ...
    if base.kind == ResidentValueKind::Snapshot {
        ...
        let snapshot_source =
            source.kind == ResidentValueKind::Snapshot && source.shape == ResidentShape::SCALAR;
        let dense_snapshot_source = source.kind != ResidentValueKind::Snapshot
            && source.shape.len() == Some(source_len)
            && dense_kind_matches_schema_body(source.kind, element);
        if !is_snapshot_index_assign_element(element)
            || (!snapshot_source && !dense_snapshot_source)
            || request.output.kind != ResidentValueKind::Snapshot
            || base.shape != ResidentShape::SCALAR
            || request.output.shape != ResidentShape::SCALAR
        ...
        return Ok(bound(
            if dense_snapshot_source {
                indexed_assign_snapshot_dense
            } else {
...
fn bind_matrix_selection_assign(
    ...
    let (rows, columns) = declared_matrix_dimensions(request, base)?;
    let snapshot_base_output = base.kind == ResidentValueKind::Snapshot
        && request.output.kind == ResidentValueKind::Snapshot
        && base.shape == ResidentShape::SCALAR
        && request.output.shape == ResidentShape::SCALAR;
    ...
    let dense_snapshot_source = source.kind != ResidentValueKind::Snapshot
        && source.shape.len() == Some(source_len)
        && dense_kind_matches_schema_body(source.kind, element);
    let snapshot_layout = snapshot_base_output && (snapshot_source || dense_snapshot_source);
...
fn bind_snapshot_range(
    ...
    if !is_range_snapshot_element(input_schema.body())
        || request.inputs.iter().any(|input| {
            let same_body = request
                .schemas
                .get(input.schema_id)
                .is_some_and(|schema| schema.body() == input_schema.body());
            let supported_layout = input.shape == ResidentShape::SCALAR
                && match input.kind {
                    ResidentValueKind::Snapshot => true,
                    ResidentValueKind::Index => input_schema.body() == &SchemaBody::Index,
                    ResidentValueKind::F64 => {
                        input_schema.body()
                            == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
                    }
    ...
    if rows != 1 || element.as_ref() != input_schema.body() {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let declared_count = if request.output.activation_fixed_shape {
        if columns == 0 {
            return Err(ResidentKernelBindError::UnsupportedLayout);
        }
        columns
    } else {
        // Zero is a safe dynamic sentinel because resident ranges reject an
        // empty result before publication.
        0
    };
    Ok(bound(
        range_snapshot,
...
fn bind_snapshot_n_choose_k_matrix(
    ...
    let input_snapshot = input.kind == ResidentValueKind::Snapshot;
    let (input_rows, input_columns) = declared_matrix_dimensions(request, input)?;
    let input_dense_f64 = input.kind == ResidentValueKind::F64
        && input.shape == resident_shape_from_dimensions(input_rows, input_columns)?
        && input_element.as_ref() == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64);
    let selection_snapshot = selection.kind == ResidentValueKind::Snapshot;
    let selection_dense_f64 = selection.kind == ResidentValueKind::F64
        && selection_schema.body() == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
        && input_element.as_ref() == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64);
    if input_element != output_element
        || (!input_snapshot && !input_dense_f64)
    ...
    Ok(bound(
        n_choose_k_snapshot,
        vec![
            u64::from(input_snapshot),
            u64::from(selection_snapshot),
            input_rows as u64,
...
fn bind_scalar_index(
    ...
    bind_dense_scalar_index(request).or_else(|_| bind_snapshot_scalar_index(request))
}

fn bind_dense_scalar_index(
    request: &ResidentKernelBindRequest<'_>,
) -> Result<BoundResidentKernel, ResidentKernelBindError> {
    validate_full_write(
        request,
...
fn bind_snapshot_scalar_index(
    ...
    if input.kind != ResidentValueKind::Snapshot
        ...
        || input.shape != ResidentShape::SCALAR
        || request.output.shape != ResidentShape::SCALAR
        || !numeric_positional_selector_layout(request, input)
    {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let output_schema = request
    ...
    if !matches!(
        output_schema.body(),
        SchemaBody::Matrix { element, .. } if element.as_ref() == &SchemaBody::Index
    ) {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    Ok(
        bound(snapshot_scalar_index, Vec::<u64>::new().into_boxed_slice())?
            .with_snapshot_output(snapshot_output_metadata(request))
            .with_snapshot_schemas(request.schemas.clone()),
    )
}
...
fn bind_dense_source_snapshot_access_mode(
    request: &ResidentKernelBindRequest<'_>,
    ...
    ) = (source_schema.body(), output_schema.body())
    else {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    };
    let source_dimensions = declared_matrix_dimensions(request, source)?;
    let source_shape = resident_shape_from_dimensions(source_dimensions.0, source_dimensions.1)?;
    ...
    let selector_schemas = selectors
        ...
        .collect::<Result<Vec<_>, _>>()?;
    let selector_layouts_valid = selectors
        .iter()
    ...
    let has_snapshot_selector = selectors.iter().any(|selector| {
        selector.kind == ResidentValueKind::Snapshot && selector.shape == ResidentShape::SCALAR
    });
    if source.kind == ResidentValueKind::Snapshot
        || source.shape != source_shape
        || !dense_kind_matches_schema_body(source.kind, source_element)
        || source_element != output_element
        || request.output.kind != ResidentValueKind::Snapshot
        || request.output.shape != ResidentShape::SCALAR
        || !selector_layouts_valid
        || !has_snapshot_selector
    {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    ...
    let matrix_mode = explicit_matrix_mode.unwrap_or(inferred_mode);
    let expected_selectors = match matrix_mode {
        ResolvedSelectionMode::Rectangle => 2,
        ResolvedSelectionMode::LinearGather
        | ResolvedSelectionMode::Rows
        | ResolvedSelectionMode::Columns => 1,
        _ => return Err(ResidentKernelBindError::UnsupportedLayout),
    };
    if selector_count != expected_selectors {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let output_dimensions = Some(declared_matrix_dimensions(request, &request.output)?);
    ...
    let plan = SnapshotAccessPlan {
        dense_source: Some(SnapshotAccessSelectorLayout {
            schema: source.schema_id,
            shape: source.shape_instance.clone(),
            resident_shape: source.shape,
        }),
        ...
        selectors: selectors
            ...
            .collect::<Vec<_>>()
            .into_boxed_slice(),
        matrix_mode: Some(matrix_mode),
        source_dimensions: Some(source_dimensions),
        output_dimensions,
        // At least one selector is activation-derived. Its committed
        // population, rather than its lower-bound placeholder, determines the
        // result axes at execution.
...
fn bind_snapshot_access_mode(
    ...
    let source = &request.inputs[0];
    if source.kind != ResidentValueKind::Snapshot {
        return bind_dense_source_snapshot_access_mode(request, explicit_matrix_mode);
    }
    if source.shape != ResidentShape::SCALAR {
        return Err(ResidentKernelBindError::UnsupportedLayout);
    }
    let source_schema = request
        .schemas
    ...
    let (matrix_mode, source_dimensions, output_dimensions, aggregate_ordinal, output_supported) =
        match source_schema.body() {
            ...
            SchemaBody::Matrix { element, .. } if (1..=2).contains(&selector_count) => {
                let declared_source_dimensions = declared_matrix_dimensions(request, source)?;
                ...
                let source_dimensions = if !source_schema.dimension_parameters().is_empty() {
                    None
                } else {
                    Some(declared_source_dimensions)
                };
                let inferred_mode = if selector_count == 2 {
                    ResolvedSelectionMode::Rectangle
                ...
                (Some(mode), source_dimensions, output_dimensions, None, true)
            }
            _ => return Err(ResidentKernelBindError::UnsupportedLayout),
        };
    if !output_supported {
    ...
    let output_dimensions_dynamic = matches!(source_schema.body(), SchemaBody::Matrix { .. })
        && matches!(output_schema.body(), SchemaBody::Matrix { .. })
        && !output_schema.dimension_parameters().is_empty()
        && !request.output.activation_fixed_shape;
    let selectors = request.inputs[1..]
        .iter()
        .map(|selector| SnapshotAccessSelectorLayout {
            schema: selector.schema_id,
...
fn bind_matmul(
    ...
    let (lhs_logical_element, lhs_logical_shape) = comparison_logical_layout(request, lhs)?;
    let (rhs_logical_element, rhs_logical_shape) = comparison_logical_layout(request, rhs)?;
    let (output_logical_element, output_logical_shape) =
        comparison_logical_layout(request, &request.output)?;
    let lhs_snapshot = lhs.kind == ResidentValueKind::Snapshot;
    let rhs_snapshot = rhs.kind == ResidentValueKind::Snapshot;
    let snapshot_output = request.output.kind == ResidentValueKind::Snapshot
        && request.output.shape == ResidentShape::SCALAR;
    let dense_output = request.output.kind == ResidentValueKind::F64
        && request.output.shape == output_logical_shape
        && output_logical_element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64);
    let dense_f64 = |port: &mech_core::ResidentPortLayout,
                     element: &SchemaBody,
                     logical_shape: ResidentShape| {
        port.kind == ResidentValueKind::F64
    ...
    if lhs_snapshot != rhs_snapshot
        && (lhs_snapshot && lhs.shape == ResidentShape::SCALAR
            || dense_f64(lhs, lhs_logical_element, lhs_logical_shape))
        && (rhs_snapshot && rhs.shape == ResidentShape::SCALAR
            || dense_f64(rhs, rhs_logical_element, rhs_logical_shape))
        && lhs_logical_element == rhs_logical_element
        && lhs_logical_element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
        && (snapshot_output || dense_output)
        && matches!(
            output_schema.body(),
            SchemaBody::Matrix { element, .. } if element.as_ref() == lhs_logical_element
        )
    {
        let kernel = bound(
            matrix_multiply_snapshot_mixed,
            vec![
                u64::from(snapshot_output),
                u64::from(lhs_snapshot),
                u64::from(rhs_snapshot),
                lhs_logical_shape.rows as u64,
                lhs_logical_shape.columns as u64,
                rhs_logical_shape.rows as u64,
                rhs_logical_shape.columns as u64,
            ]
            .into_boxed_slice(),
        )?
        .with_snapshot_schemas(request.schemas.clone());
        if snapshot_output {
            return Ok(kernel.with_snapshot_output(snapshot_output_metadata(request)));
        }
        return Ok(kernel);
    }
...
fn bind_matrix_dot(
    ...
    match (left.kind, right.kind, request.output.kind) {
        ...
        (ResidentValueKind::Snapshot, ResidentValueKind::Snapshot, ResidentValueKind::Snapshot)
            if left.shape == ResidentShape::SCALAR && right.shape == ResidentShape::SCALAR =>
        {
            Ok(
        ...
        (ResidentValueKind::Snapshot, ResidentValueKind::Snapshot, ResidentValueKind::F64)
            if left.shape == ResidentShape::SCALAR && right.shape == ResidentShape::SCALAR =>
        {
            Ok(bound(
                matrix_dot_snapshot_f64,
                Vec::<u64>::new().into_boxed_slice(),
            )?
            .with_snapshot_schemas(request.schemas.clone()))
        }
        (ResidentValueKind::Snapshot, ResidentValueKind::F64, ResidentValueKind::F64)
        | (ResidentValueKind::F64, ResidentValueKind::Snapshot, ResidentValueKind::F64)
            if left_element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64) =>
        {
            let (left_logical_element, left_shape) = comparison_logical_layout(request, left)?;
            let (right_logical_element, right_shape) = comparison_logical_layout(request, right)?;
            let left_snapshot = left.kind == ResidentValueKind::Snapshot;
            let right_snapshot = right.kind == ResidentValueKind::Snapshot;
            if left_snapshot == right_snapshot
                || (left_snapshot && left.shape != ResidentShape::SCALAR)
                ...
                || (!left_snapshot
                    && (left.shape != left_shape
                        || left_logical_element
                            != &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)))
                || (!right_snapshot
                    && (right.shape != right_shape
                        || right_logical_element
                            != &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)))
            {
                return Err(ResidentKernelBindError::UnsupportedLayout);
            }
            Ok(bound(
                matrix_dot_snapshot_mixed_f64,
                vec![
                    u64::from(left_snapshot),
                    u64::from(right_snapshot),
                    left_shape.rows as u64,
...
fn bind_matrix_solve(
    ...
    if coefficient_element != right_element
    ...
    let coefficient_snapshot = coefficients.kind == ResidentValueKind::Snapshot;
    let right_snapshot = right.kind == ResidentValueKind::Snapshot;
    if coefficient_snapshot != right_snapshot {
        let (coefficient_logical_element, coefficient_shape) =
            comparison_logical_layout(request, coefficients)?;
        let (right_logical_element, right_shape) = comparison_logical_layout(request, right)?;
        let (output_logical_element, output_shape) =
            comparison_logical_layout(request, &request.output)?;
        let dense_f64 = |port: &mech_core::ResidentPortLayout,
                         element: &SchemaBody,
                         logical_shape: ResidentShape| {
        ...
        let snapshot_output = request.output.kind == ResidentValueKind::Snapshot
            && request.output.shape == ResidentShape::SCALAR;
        let dense_output = dense_f64(&request.output, output_logical_element, output_shape);
        if (coefficient_snapshot && coefficients.shape == ResidentShape::SCALAR
            || dense_f64(coefficients, coefficient_logical_element, coefficient_shape))
            && (right_snapshot && right.shape == ResidentShape::SCALAR
                || dense_f64(right, right_logical_element, right_shape))
            && coefficient_logical_element == right_logical_element
            && coefficient_logical_element == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
            && (snapshot_output || dense_output)
        {
            let kernel = bound(
                matrix_solve_snapshot_mixed_f64,
                vec![
                    u64::from(snapshot_output),
                    u64::from(coefficient_snapshot),
                    u64::from(right_snapshot),
                    coefficient_shape.rows as u64,
                    coefficient_shape.columns as u64,
                    right_shape.rows as u64,
                    right_shape.columns as u64,
                ]
                .into_boxed_slice(),
    ...
    match (coefficients.kind, right.kind, request.output.kind) {
        (ResidentValueKind::F64, ResidentValueKind::F64, ResidentValueKind::F64) => {
            ...
            bound(
                matrix_solve_f64,
                vec![rows as u64, right_columns as u64].into_boxed_slice(),
            )
        }
        (ResidentValueKind::Snapshot, ResidentValueKind::Snapshot, ResidentValueKind::Snapshot)
        ...
        {
            Ok(
                bound(matrix_solve_snapshot, vec![1_u64].into_boxed_slice())?
                    .with_snapshot_output(ResidentSnapshotOutput {
                        schema: request.output.schema_id,
                        schema_key: request.output.schema_key,
                        shape: request.output.shape_instance.clone(),
        ...
        (ResidentValueKind::Snapshot, ResidentValueKind::Snapshot, ResidentValueKind::F64)
            if coefficients.shape == ResidentShape::SCALAR
                && right.shape == ResidentShape::SCALAR
                && matches!(
                    output_element.as_ref(),
                    SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
                )
                && declared_matrix_dimensions(request, &request.output)?
                    == (
                        request.output.shape.rows as usize,
                        request.output.shape.columns as usize,
                    ) =>
        {
            Ok(
                bound(matrix_solve_snapshot, vec![0_u64].into_boxed_slice())?
                    .with_snapshot_schemas(request.schemas.clone()),
            )
        }
        _ => Err(ResidentKernelBindError::UnsupportedLayout),
...
enum DenseComparisonValues<'a> {
    F64(&'a [f64], usize, usize),
    Bool(&'a [u8], usize, usize),
    Index(&'a [u64], usize, usize),
    String(&'a [String], usize, usize),
}

impl<'a> DenseComparisonValues<'a> {
    fn from_resident(
        value: ResidentValueRef<'a>,
        rows: usize,
        columns: usize,
        ...
        let values = match value {
            ResidentValueRef::F64(values) if values.len() == len => {
                Self::F64(values, rows, columns)
            }
            ResidentValueRef::Bool(values)
                if values.len() == len && values.iter().all(|value| *value <= 1) =>
            {
                Self::Bool(values, rows, columns)
            }
            ResidentValueRef::Index(values) if values.len() == len => {
                Self::Index(values, rows, columns)
            }
            ResidentValueRef::String(values) if values.len() == len => {
                Self::String(values, rows, columns)
            }
    ...
    fn resident_index(&self, canonical_index: usize) -> Result<usize, ResidentKernelError> {
        let (rows, columns) = match self {
            Self::F64(_, rows, columns)
            | Self::Bool(_, rows, columns)
            | Self::Index(_, rows, columns)
            | Self::String(_, rows, columns) => (*rows, *columns),
        };
        if canonical_index >= self.len() || columns == 0 {
            return Err(ResidentKernelError::InvalidShape);
        }
        let row = canonical_index / columns;
        let column = canonical_index % columns;
        row.checked_add(
            column
                .checked_mul(rows)
                .ok_or(ResidentKernelError::InvalidShape)?,
    ...
    fn comparison_work(
        ...
        let index = self.resident_index(index)?;
        let work = match self {
            Self::String(values, ..) => super::budget::checked_u64(
                values
                    .get(index)
                    .ok_or(ResidentKernelError::InvalidShape)?
                    .len(),
            ...
            .ok_or(ResidentKernelError::InvalidShape)?,
            Self::F64(..) | Self::Bool(..) | Self::Index(..) => 1,
        }
        .max(1);
    ...
    fn cloned_bytes(&self) -> Result<u64, ResidentKernelError> {
        match self {
            Self::String(values, ..) => values.iter().try_fold(0_u64, |bytes, value| {
                bytes
                    .checked_add(super::budget::checked_u64(value.len())?)
                    .ok_or(ResidentKernelError::InvalidShape)
            }),
            Self::F64(..) | Self::Bool(..) | Self::Index(..) => Ok(0),
        }
    }

    fn retained_nodes(&self) -> Result<u64, ResidentKernelError> {
        match self {
            Self::String(values, ..) => super::budget::checked_u64(values.len()),
            Self::F64(..) | Self::Bool(..) | Self::Index(..) => Ok(0),
        }
    }

    fn materialize(&self) -> Result<Vec<ValueData>, ResidentKernelError> {
        (0..self.len())
            .map(|canonical_index| {
                let index = self.resident_index(canonical_index)?;
                match self {
                    Self::F64(values, ..) => values
                        .get(index)
                        .copied()
                        .map(|value| ValueData::F64(F64Bits::from_f64(value))),
                    Self::Bool(values, ..) => values
                        .get(index)
                        .copied()
                        .map(|value| ValueData::Bool(value != 0)),
                    Self::Index(values, ..) => values.get(index).copied().map(ValueData::Index),
                    Self::String(values, ..) => values
                        .get(index)
                        .map(|value| ValueData::String(value.as_str().into())),
                }
                .ok_or(ResidentKernelError::InvalidShape)
            })
            .collect()
...
fn snapshot_comparison(
    ...
    let comparison =
        SemanticComparison::from_parameter(*comparison).ok_or(ResidentKernelError::InvalidInput)?;
    let snapshot_output = match *snapshot_output {
        0 => false,
        1 => true,
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    ...
    let left_snapshot = flag(*left_snapshot)?;
    let right_snapshot = flag(*right_snapshot)?;
    let bound_dimensions = |rows: u64, columns: u64| {
    ...
    let bound_left_dimensions = bound_dimensions(*bound_left_rows, *bound_left_columns)?;
    let bound_right_dimensions = bound_dimensions(*bound_right_rows, *bound_right_columns)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidInput)?;
    let snapshot_value = |input, expected| match (input, expected) {
        (ResidentValueRef::Snapshot([Some(value)]), true) => Ok(Some(value)),
        (ResidentValueRef::Snapshot(_), _) | (_, true) => Err(ResidentKernelError::InvalidInput),
        (_, false) => Ok(None),
    };
    let left = snapshot_value(left_input, left_snapshot)?;
    let right = snapshot_value(right_input, right_snapshot)?;
    ...
    let right_schema = right
        ...
        .transpose()?;
    let snapshot_element = left_schema
        .map(|schema| comparison_element_body(schema.body()))
        .or_else(|| right_schema.map(|schema| comparison_element_body(schema.body())))
        .ok_or(ResidentKernelError::InvalidInput)?;
    let left_element = left_schema.map_or(snapshot_element, |schema| {
        comparison_element_body(schema.body())
    });
    let right_element = right_schema.map_or(snapshot_element, |schema| {
        comparison_element_body(schema.body())
    });
    if left_element != right_element
        || !snapshot_comparison_element_supported(left_element, comparison)
    ...
    let left_dimensions = match (left, left_schema) {
        ...
        _ => bound_left_dimensions,
    };
    let right_dimensions = match (right, right_schema) {
        (Some(value), Some(schema)) => snapshot_value_dimensions(value, schema.body())?,
        _ => bound_right_dimensions,
    };
    ...
    let left_mode = binary_broadcast_mode(
        resident_shape_for_dimensions(left_dimensions.0, left_dimensions.1)?,
        logical_output,
    )
    .ok_or(ResidentKernelError::InvalidShape)?;
    let right_mode = binary_broadcast_mode(
        resident_shape_for_dimensions(right_dimensions.0, right_dimensions.1)?,
        logical_output,
    )
    .ok_or(ResidentKernelError::InvalidShape)?;
    let output_len = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let snapshot_count = |value: &mech_core::Value| match value.data() {
    ...
    let left_dense = left
        .is_none()
        .then(|| {
            DenseComparisonValues::from_resident(left_input, left_dimensions.0, left_dimensions.1)
        })
        .transpose()?;
    let right_dense = right
        .is_none()
        .then(|| {
            DenseComparisonValues::from_resident(
                right_input,
                right_dimensions.0,
                right_dimensions.1,
            )
        ...
        .transpose()?;
    let left_count = left.map_or_else(
        || left_dense.as_ref().map_or(0, DenseComparisonValues::len),
        snapshot_count,
    );
    let right_count = right.map_or_else(
        || right_dense.as_ref().map_or(0, DenseComparisonValues::len),
        snapshot_count,
    );
    ...
    let output_shape = if snapshot_output {
        if !tail.is_empty() {
            return Err(ResidentKernelError::InvalidInput);
        }
        Some(resolved_snapshot_output_shape(kernel, rows, columns)?)
    } else {
        let [bound_rows, bound_columns, bound_left_mode, bound_right_mode] = tail else {
            return Err(ResidentKernelError::InvalidInput);
        };
        if usize::try_from(*bound_rows).ok() != Some(rows)
            || usize::try_from(*bound_columns).ok() != Some(columns)
            || (!left_snapshot && *bound_left_mode != left_mode)
            || (!right_snapshot && *bound_right_mode != right_mode)
        {
            return Err(ResidentKernelError::InvalidShape);
        }
        let ResidentValueMut::Bool(values) = &output else {
            return Err(ResidentKernelError::InvalidOutput);
        };
    ...
    let staged_elements = left_count
        .checked_add(right_count)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let mut footprint_meter = super::budget::ResidentBudgetMeter::default();
    let (left_bytes, left_nodes) = match left {
        Some(value) => snapshot_clone_cost(&mut footprint_meter, value, schemas)?,
        None => (0, 0),
    };
    let (right_bytes, right_nodes) = match right {
        Some(value) => snapshot_clone_cost(&mut footprint_meter, value, schemas)?,
        None => (0, 0),
    };
    let left_dense_bytes = left_dense
        .as_ref()
        .map(DenseComparisonValues::cloned_bytes)
        .transpose()?
        .unwrap_or(0);
    let right_dense_bytes = right_dense
        .as_ref()
        .map(DenseComparisonValues::cloned_bytes)
        .transpose()?
        .unwrap_or(0);
    let cloned_bytes = left_bytes
        ...
        .and_then(|bytes| bytes.checked_add(left_dense_bytes))
        .and_then(|bytes| bytes.checked_add(right_dense_bytes))
        .ok_or(ResidentKernelError::InvalidShape)?;
    let left_dense_nodes = left_dense
        .as_ref()
        .map(DenseComparisonValues::retained_nodes)
        .transpose()?
        .unwrap_or(0);
    let right_dense_nodes = right_dense
        .as_ref()
        .map(DenseComparisonValues::retained_nodes)
        .transpose()?
        .unwrap_or(0);
    let retained_nodes = left_nodes
        ...
        .and_then(|nodes| nodes.checked_add(left_dense_nodes))
        .and_then(|nodes| nodes.checked_add(right_dense_nodes))
        .ok_or(ResidentKernelError::InvalidShape)?;
    let logical_coordinates = |index: usize| {
        if snapshot_output {
    ...
    let comparison_index = |mode: u64, row: usize, column: usize| match mode {
        BINARY_BROADCAST_SCALAR => 0,
    ...
    for index in 0..output_len {
        let (row, column) = logical_coordinates(index);
        let left_index = comparison_index(left_mode, row, column);
        let right_index = comparison_index(right_mode, row, column);
        let left_work = match (left, left_schema) {
            (Some(value), Some(schema)) => snapshot_element_comparison_work(
                &mut footprint_meter,
                schema.body(),
                value.data(),
                left_index,
            ...
            _ => left_dense
                .as_ref()
                .ok_or(ResidentKernelError::InvalidInput)?
                .comparison_work(left_index, &mut footprint_meter)?,
        };
        let right_work = match (right, right_schema) {
            (Some(value), Some(schema)) => snapshot_element_comparison_work(
                &mut footprint_meter,
                schema.body(),
                value.data(),
            ...
            _ => right_dense
                .as_ref()
                .ok_or(ResidentKernelError::InvalidInput)?
                .comparison_work(right_index, &mut footprint_meter)?,
        };
    ...
    let output_draft_bytes = super::budget::checked_u64(
        output_len
            .checked_mul(core::mem::size_of::<ValueDataDraft>())
            .ok_or(ResidentKernelError::InvalidShape)?,
    )?;
    let output_nodes = if snapshot_output { 3_u64 } else { 0 };
    let output_footprint = ValueFootprint {
        encoded_bytes: super::budget::checked_u64(output_len)?
            .checked_add(output_nodes)
            .ok_or(ResidentKernelError::InvalidShape)?,
        retained_bytes: super::budget::checked_u64(output_len)?,
        node_count: output_nodes,
    };
    let publication_work = if snapshot_output {
        let metadata = kernel
            .snapshot_output()
        ...
        match values.first().and_then(|value| value.as_ref()) {
            Some(previous) => {
                let prior = super::budget::measure_canonical_value_footprint(
                    &mut footprint_meter,
                    previous,
                    schemas,
                )?;
        ...
        super::budget::checked_u64(output_len)?
    };
    ...
    let left_dense_values = left_dense
        .as_ref()
        .map(DenseComparisonValues::materialize)
        .transpose()?;
    let right_dense_values = right_dense
        .as_ref()
        .map(DenseComparisonValues::materialize)
        .transpose()?;
    let left_values = match left {
        Some(value) => match value.data() {
            ValueData::Matrix(matrix) => {
                SnapshotComparisonValues::Matrix(matrix.elements().to_values())
            }
            value => SnapshotComparisonValues::Scalar(value),
        },
        None => SnapshotComparisonValues::Matrix(
            left_dense_values.ok_or(ResidentKernelError::InvalidInput)?,
        ),
    };
    let right_values = match right {
        ...
        None => SnapshotComparisonValues::Matrix(
            right_dense_values.ok_or(ResidentKernelError::InvalidInput)?,
        ),
    };
    ...
    for index in 0..output_len {
        ...
        let left = left_values
            .get(comparison_index(left_mode, row, column))
            .ok_or(ResidentKernelError::InvalidShape)?;
        let right = right_values
            .get(comparison_index(right_mode, row, column))
            .ok_or(ResidentKernelError::InvalidShape)?;
        ...
        result.push(ValueDataDraft::Bool(next));
    }
    if let Some(output_shape) = output_shape {
        ...
        let mut changed = false;
        for (target, next) in output.iter_mut().zip(result) {
            let ValueDataDraft::Bool(next) = next else {
                return Err(ResidentKernelError::InvalidInput);
            };
            let next = u8::from(next);
            changed |= *target != next;
...
#[derive(Clone, Copy)]
enum DenseSnapshotAssignmentSource<'a> {
    Bool(&'a [u8]),
    Index(&'a [u64]),
    F64(&'a [f64]),
    String(&'a [String]),
...
impl<'a> DenseSnapshotAssignmentSource<'a> {
    fn new(value: ResidentValueRef<'a>, element: &SchemaBody) -> Result<Self, ResidentKernelError> {
        match (value, element) {
            (ResidentValueRef::Bool(values), SchemaBody::Bool) => {
                if values.iter().any(|value| *value > 1) {
                    return Err(ResidentKernelError::InvalidInput);
                }
                Ok(Self::Bool(values))
            }
            (ResidentValueRef::Index(values), SchemaBody::Index) => Ok(Self::Index(values)),
            (
                ResidentValueRef::F64(values),
                SchemaBody::FloatingPoint(mech_core::FloatWidth::W64),
            ) => Ok(Self::F64(values)),
            (ResidentValueRef::String(values), SchemaBody::String) => Ok(Self::String(values)),
            _ => Err(ResidentKernelError::InvalidInput),
        }
    }
    ...
    fn draft_at(self, index: usize) -> Result<ValueDataDraft, ResidentKernelError> {
        match self {
            Self::Bool(values) => values
                .get(index)
                .map(|value| ValueDataDraft::Bool(*value != 0)),
            Self::Index(values) => values.get(index).copied().map(ValueDataDraft::Index),
            Self::F64(values) => values
                .get(index)
                .map(|value| ValueDataDraft::F64(F64Bits::from_f64(*value))),
            Self::String(values) => values
                .get(index)
                .map(|value| ValueDataDraft::String(value.clone())),
        }
        .ok_or(ResidentKernelError::InvalidShape)
    }

    fn string_at(self, index: usize) -> Result<&'a str, ResidentKernelError> {
        let Self::String(values) = self else {
            return Err(ResidentKernelError::InvalidInput);
        };
        values
            .get(index)
    ...
    fn retained_nodes(self) -> Result<u64, ResidentKernelError> {
        let nodes = match self {
            Self::String(values) => values.len().saturating_add(1),
            _ => self.len(),
        };
        super::budget::checked_u64(nodes)
    }
}

fn projected_dense_snapshot_matrix_output(
    meter: &mut super::budget::ResidentBudgetMeter,
    current_schema: &SchemaBody,
    current_data: &ValueData,
    current_footprint: ValueFootprint,
    source: DenseSnapshotAssignmentSource<'_>,
    output_len: usize,
    mut selected_source: impl FnMut(
        usize,
        &mut super::budget::ResidentBudgetMeter,
    ...
    let finalization_work = snapshot_data_finalization_work(meter, current_schema, current_data)?;
    if element.as_ref() == &SchemaBody::String {
        let SequenceView::String(current_values) = current_matrix.elements() else {
            return Err(ResidentKernelError::InvalidOutput);
        };
        let container_bytes = super::budget::checked_u64(
            output_len
        ...
        let mut encoded_bytes = 0u64;
        for (destination, current) in current_values.iter().enumerate() {
            let next = match selected_source(destination, meter)? {
                Some(source_index) => source.string_at(source_index)?,
                None => current.as_ref(),
            };
            let bytes = super::budget::checked_u64(next.len())?;
            final_payload = final_payload
    ...
    let storage_matches = matches!(
        (current_matrix.elements(), source),
        (
            SequenceView::Bool(_),
            DenseSnapshotAssignmentSource::Bool(_)
        ) | (
            SequenceView::Index(_),
            DenseSnapshotAssignmentSource::Index(_)
        ) | (SequenceView::F64(_), DenseSnapshotAssignmentSource::F64(_))
    );
    if !storage_matches {
        return Err(ResidentKernelError::InvalidInput);
    }
...
fn indexed_assign_snapshot_dense(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    ...
    let bound_source_routing = ResolvedSourceRouting::from_parameter(*bound_source_routing)
        .ok_or(ResidentKernelError::InvalidInput)?;
    ...
    let current_schema = current
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidOutput)?;
    let SchemaBody::Matrix { element, .. } = current_schema.body() else {
        return Err(ResidentKernelError::InvalidOutput);
    };
    let source = DenseSnapshotAssignmentSource::new(source_ref, element)?;
    let (rows, columns) = snapshot_matrix_dimensions(current, current_schema.body())?;
    let output_len = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let source_len = source_rows
    ...
    let source_routing = if source_len == 1 {
        ...
        ResolvedSourceRouting::CompactSelectionOrder
    } else if bound_source_routing == ResolvedSourceRouting::Positional
        && positions
            .maximum_position()
    ...
    let source_index = |ordinal: usize, position: usize| {
        Ok::<usize, ResidentKernelError>(match source_routing {
            ResolvedSourceRouting::ScalarBroadcast => 0,
            ResolvedSourceRouting::Positional => position,
            ResolvedSourceRouting::CompactSelectionOrder => {
                if source_columns == 0 {
                ...
                let row = ordinal / source_columns;
                let column = ordinal % source_columns;
                column
    ...
    let (final_footprint, finalization_work) = projected_dense_snapshot_matrix_output(
        &mut footprint_meter,
        current_schema.body(),
        current.data(),
        current_footprint,
        source,
        output_len,
        |canonical_destination, meter| {
            meter.charge_compute_work(super::budget::checked_u64(positions.traversal_len())?)?;
            let row = canonical_destination / columns;
            let column = canonical_destination % columns;
            let physical_destination = column
                .checked_mul(rows)
                .and_then(|offset| offset.checked_add(row))
                .ok_or(ResidentKernelError::InvalidShape)?;
            last_source_for_destination(positions, physical_destination, |ordinal, position| {
                source_index(ordinal, position).unwrap_or(usize::MAX)
            })
        },
    )?;
    let metadata = kernel
    ...
    let publication_equality_work = super::budget::projected_language_equality_work(
        ...
        current_footprint,
        metadata.schema,
        current.shape().parameter_values().len(),
        final_footprint,
    )?;
    let container_bytes = super::budget::checked_u64(
        output_len
    ...
    let source_nodes = source.retained_nodes()?;
    let measured = footprint_meter.estimate();
    let canonicalization_work_limit = super::budget::PreparedMutationPlan::new(
        finalization_work,
        super::budget::PublishedOutputFootprint {
            elements: super::budget::checked_u64(output_len)?,
            retained_bytes: final_footprint.retained_bytes,
            retained_nodes: final_footprint.node_count,
        },
        super::budget::MutationRetainedNodeFootprint {
            current_persistent: current_footprint
                .node_count
                ...
                .ok_or(ResidentKernelError::InvalidShape)?,
            normalized_plan: 0,
            temporary_draft: current_footprint
                .node_count
                .checked_add(source_nodes)
                .and_then(|nodes| nodes.checked_add(final_footprint.node_count))
                .ok_or(ResidentKernelError::InvalidShape)?,
        },
        super::budget::resident_cost! {
            comparison_work: measured.comparison_work()
            ...
            temporary_bytes: current_footprint.retained_bytes
                .checked_add(container_bytes)
                .and_then(|bytes| bytes.checked_add(final_footprint.retained_bytes))
                .ok_or(ResidentKernelError::InvalidShape)?,
            cloned_bytes: current_footprint.retained_bytes
                .checked_add(final_footprint.retained_bytes)
                .ok_or(ResidentKernelError::InvalidShape)?,
            ..super::budget::KernelCostEstimate::default()
        },
    )?
    ...
    positions.try_for_each(|ordinal, position| {
        ...
        let destination = row
            ...
            .ok_or(ResidentKernelError::InvalidShape)?;
        let incoming = source.draft_at(source_index(ordinal, position)?)?;
...
fn indexed_assign_snapshot(
    ...
    let [_, _, _, _, bound_source_routing] = kernel.parameters() else {
        return Err(ResidentKernelError::InvalidInput);
    };
    let bound_source_routing = ResolvedSourceRouting::from_parameter(*bound_source_routing)
        .ok_or(ResidentKernelError::InvalidInput)?;
    ...
    let source_schema = source
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    if comparison_element_body(current_schema.body())
        != comparison_element_body(source_schema.body())
    {
        return Err(ResidentKernelError::InvalidInput);
    }
    let (rows, columns) = snapshot_matrix_dimensions(current, current_schema.body())?;
    let output_len = rows
        ...
        .ok_or(ResidentKernelError::InvalidShape)?;
    let (source_rows, source_columns) = match source_schema.body() {
        SchemaBody::Matrix { .. } => snapshot_matrix_dimensions(source, source_schema.body())?,
        _ => (1, 1),
    };
...
fn matrix_selection_last_dense_source(
    plan: &MatrixSelectionAssignPlan,
    inputs: &dyn ResidentKernelInputs,
    destination: usize,
    meter: &mut super::budget::ResidentBudgetMeter,
    ...
    matrix_selection_for_each_coordinate(plan, inputs, |ordinal, row, column| {
        ...
        if candidate == destination {
            selected = Some(match plan.source_routing {
                ResolvedSourceRouting::ScalarBroadcast => 0,
                ResolvedSourceRouting::Positional => column
...
fn indexed_assign_matrix_selection(
    ...
    let declared_plan = kernel
        .retained_state::<MatrixSelectionAssignPlan>()
        .ok_or(ResidentKernelError::InvalidInput)?;
    if inputs.len() != declared_plan.selectors.len() + 1 {
        return Err(ResidentKernelError::InvalidInput);
    }
    let schemas = kernel
        .snapshot_schemas()
    ...
    if let ResidentValueMut::Snapshot([Some(current)]) = &output {
        let current_schema = current
            .validate_against(schemas)
            .map_err(|_| ResidentKernelError::InvalidOutput)?;
        let (rows, columns) = snapshot_matrix_dimensions(current, current_schema.body())?;
        let (source_rows, source_columns) = match source_ref {
            ResidentValueRef::Snapshot([Some(source)]) => {
                ...
                match source_schema.body() {
                    SchemaBody::Matrix { .. } => {
                        snapshot_matrix_dimensions(source, source_schema.body())?
                    }
                    _ => (1, 1),
                }
            }
            ResidentValueRef::Bool(values) => {
                ...
                let declared_len = declared_plan
                    .source_rows
                    .checked_mul(declared_plan.source_columns)
                    .ok_or(ResidentKernelError::InvalidShape)?;
                if values.len() != declared_len {
                    return Err(ResidentKernelError::InvalidShape);
                }
                (declared_plan.source_rows, declared_plan.source_columns)
            }
        ...
        let output_len = rows
            .checked_mul(columns)
            .ok_or(ResidentKernelError::InvalidShape)?;
        if declared_plan.mode == ResolvedSelectionMode::Whole
            && source_len != 1
            && (source_rows, source_columns) != (rows, columns)
        {
            return Err(ResidentKernelError::InvalidShape);
        }
        ...
        plan.source_columns = source_columns;
        plan.source_routing = if source_len == 1 {
            ResolvedSourceRouting::ScalarBroadcast
        } else if source_len == output_len {
            ResolvedSourceRouting::Positional
        } else {
    ...
    let plan = resolved_plan.as_ref().unwrap_or(declared_plan);
    let output_len = plan
        .rows
        .checked_mul(plan.columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    ...
    let write_count = selected_rows_count
        ...
        .ok_or(ResidentKernelError::InvalidShape)?;
    if let Some(plan) = resolved_plan.as_mut() {
        plan.source_routing = if source_len == 1 {
            ResolvedSourceRouting::ScalarBroadcast
        } else if source_len == output_len {
            ResolvedSourceRouting::Positional
        } else if source_len == write_count {
            ResolvedSourceRouting::CompactSelectionOrder
        } else {
            return Err(ResidentKernelError::InvalidShape);
        };
    ...
    let snapshot_finalization_work = match (source_ref, &output) {
        ...
        (
            source @ (ResidentValueRef::Bool(_)
            ...
            let source = DenseSnapshotAssignmentSource::new(source, element)?;
            let current_footprint = super::budget::measure_canonical_value_footprint(
                &mut footprint_meter,
                current,
                schemas,
            ...
            let (final_footprint, finalization_work) = projected_dense_snapshot_matrix_output(
                ...
                output_len,
                |destination, meter| {
                    matrix_selection_last_dense_source(plan, inputs, destination, meter)
                },
            )?;
            let metadata = kernel
                .snapshot_output()
    ...
    match (source_ref, output) {
        ...
        (
            ...
            if next.len() != output_len || source.len() != source_len {
                return Err(ResidentKernelError::InvalidShape);
            }
            let mut ordinal = 0usize;
            for row in &selected_rows {
                for column in &selected_columns {
                    ...
                    let source_index = match plan.source_routing {
                        ...
                        ResolvedSourceRouting::Positional => column
                            .checked_mul(plan.source_rows)
                            .and_then(|offset| offset.checked_add(*row))
                            .filter(|source_index| *source_index < source_len)
                            .ok_or(ResidentKernelError::InvalidShape)?,
                        ResolvedSourceRouting::CompactSelectionOrder => {
                            dense_compact_source_index(ordinal, plan)?
                        }
                    };
                    let incoming = source.draft_at(source_index)?;
...
fn unary_float_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
    operation_f64: impl Fn(f64) -> f64,
    operation_f32: impl Fn(f32) -> f32,
) -> Result<bool, ResidentKernelError> {
    ...
    let input_schema = input
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    let (rows, columns) = snapshot_numeric_dimensions(input, input_schema.body())?;
    let output_len = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    if snapshot_numeric_element_count(input)? != output_len {
        return Err(ResidentKernelError::InvalidShape);
    }
    let output_shape = resolved_snapshot_numeric_output_shape(kernel, rows, columns)?;
...
fn cosine_f32(
    ...
    unary_float_snapshot(kernel, inputs, output, libm::cos, libm::cosf)
}
...
fn absolute_f32(
    ...
    unary_float_snapshot(kernel, inputs, output, f64::abs, f32::abs)
}
...
fn floor_f32(
    ...
    unary_float_snapshot(kernel, inputs, output, libm::floor, libm::floorf)
}
...
fn square_root_f32(
    ...
    unary_float_snapshot(kernel, inputs, output, libm::sqrt, libm::sqrtf)
}
...
fn sine_f32(
    ...
    unary_float_snapshot(kernel, inputs, output, libm::sin, libm::sinf)
}
...
fn math_copysign_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(kernel, inputs, output, libm::copysign, libm::copysignf)
}
...
fn atan2_snapshot(
    ...
    binary_float_snapshot(kernel, inputs, output, libm::atan2, libm::atan2f)
}

fn math_fdim_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(kernel, inputs, output, libm::fdim, libm::fdimf)
}

fn math_fmod_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(kernel, inputs, output, libm::fmod, libm::fmodf)
}

fn math_nextafter_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(kernel, inputs, output, libm::nextafter, libm::nextafterf)
}

fn math_remainder_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(kernel, inputs, output, libm::remainder, libm::remainderf)
}

fn math_bessel_jn_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(
        kernel,
        inputs,
        output,
        |order, value| libm::jn(order as i32, value),
        |order, value| libm::jnf(order as i32, value),
    )
}

fn math_bessel_yn_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    binary_float_snapshot(
        ...
        inputs,
        output,
        |order, value| libm::yn(order as i32, value),
        |order, value| libm::ynf(order as i32, value),
    )
}
...
fn dense_f64_canonical_values(
    input: ResidentValueRef<'_>,
    rows: usize,
    columns: usize,
) -> Result<Vec<f64>, ResidentKernelError> {
    ...
    Ok((0..rows)
        .flat_map(|row| (0..columns).map(move |column| values[row + column * rows]))
        .collect())
}

fn dense_f64_canonical_drafts(
    input: ResidentValueRef<'_>,
    rows: usize,
    columns: usize,
    ...
    Ok((0..rows)
        .flat_map(|row| {
            (0..columns).map(move |column| {
                ValueDataDraft::F64(F64Bits::from_f64(values[row + column * rows]))
            })
        })
        .collect())
}

fn binary_float_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
    operation_f64: impl Fn(f64, f64) -> f64,
    operation_f32: impl Fn(f32, f32) -> f32,
) -> Result<bool, ResidentKernelError> {
    let left_input = inputs.get(0).ok_or(ResidentKernelError::InvalidInput)?;
    let right_input = inputs.get(1).ok_or(ResidentKernelError::InvalidInput)?;
    let [
    ...
    let snapshot_output = flag(*snapshot_output)?;
    let left_snapshot = flag(*left_snapshot)?;
    let right_snapshot = flag(*right_snapshot)?;
    let snapshot_value = |input, expected| match (input, expected) {
        (ResidentValueRef::Snapshot([Some(value)]), true) => Ok(Some(value)),
    ...
    let right_schema = right
        ...
        .transpose()?;
    let float_element = left_schema
        .map(|schema| comparison_element_body(schema.body()))
        .or_else(|| right_schema.map(|schema| comparison_element_body(schema.body())))
        .ok_or(ResidentKernelError::InvalidInput)?;
    if left_schema.is_some_and(|schema| comparison_element_body(schema.body()) != float_element)
        || right_schema
            .is_some_and(|schema| comparison_element_body(schema.body()) != float_element)
        || !matches!(
            float_element,
            SchemaBody::FloatingPoint(mech_core::FloatWidth::W32 | mech_core::FloatWidth::W64)
        )
    ...
    let left_len = match left {
        ...
        None => left_dimensions
            .0
            .checked_mul(left_dimensions.1)
            .ok_or(ResidentKernelError::InvalidShape)?,
    };
    let right_len = match right {
        ...
        None => right_dimensions
            .0
            .checked_mul(right_dimensions.1)
            .ok_or(ResidentKernelError::InvalidShape)?,
    };
    ...
    let output_len = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_shape = snapshot_output
        .then(|| resolved_snapshot_numeric_output_shape(kernel, rows, columns))
        .transpose()?;
    ...
    match (left, right, snapshot_output) {
        ...
        (Some(input), None, false) | (None, Some(input), false) => {
            preflight_snapshot_dense_f64_output(schemas, &[input], input_elements, output_len, 0)?
        }
        (None, None, _) => return Err(ResidentKernelError::InvalidInput),
    }
    let at = |mode, row, column| snapshot_binary_broadcast_index(mode, row, column, columns);
    let output_schema = kernel
        .snapshot_output()
        .map(|metadata| {
            schemas
                .get(metadata.schema)
        ...
        .transpose()?;
    let scalar_or_matrix = |values: Vec<ValueDataDraft>| {
        if output_schema.is_none_or(|schema| matches!(schema.body(), SchemaBody::Matrix { .. })) {
    ...
    let data = match float_element {
        SchemaBody::FloatingPoint(mech_core::FloatWidth::W32) => {
            let left = left
                .and_then(f32_snapshot_values)
                .ok_or(ResidentKernelError::InvalidInput)?;
            let right = right
                .and_then(f32_snapshot_values)
                .ok_or(ResidentKernelError::InvalidInput)?;
            ...
            for row in 0..rows {
                for column in 0..columns {
                    values.push(ValueDataDraft::F32(F32Bits::from_f32(operation_f32(
                        *left
                            .get(at(left_mode, row, column))
                            .ok_or(ResidentKernelError::InvalidShape)?,
                        *right
                            .get(at(right_mode, row, column))
                            .ok_or(ResidentKernelError::InvalidShape)?,
                    ))));
                }
            }
            scalar_or_matrix(values)?
        }
        SchemaBody::FloatingPoint(mech_core::FloatWidth::W64) => {
            let snapshot_f64 = |value: &mech_core::Value| match value.data() {
                ValueData::F64(value) => Ok(vec![value.to_f64()]),
                ValueData::Matrix(matrix) => match matrix.elements() {
                    SequenceView::F64(values) => {
                        Ok(values.iter().map(|value| value.to_f64()).collect())
                    }
                    _ => Err(ResidentKernelError::InvalidInput),
                },
                _ => Err(ResidentKernelError::InvalidInput),
            ...
            let left = match left {
                Some(value) => snapshot_f64(value)?,
                None => {
                    dense_f64_canonical_values(left_input, left_dimensions.0, left_dimensions.1)?
                }
            };
            let right = match right {
                Some(value) => snapshot_f64(value)?,
                None => {
                    dense_f64_canonical_values(right_input, right_dimensions.0, right_dimensions.1)?
                }
            };
            let mut values = Vec::with_capacity(output_len);
            for row in 0..rows {
                for column in 0..columns {
                    values.push(ValueDataDraft::F64(F64Bits::from_f64(operation_f64(
                        *left
                            .get(at(left_mode, row, column))
                            .ok_or(ResidentKernelError::InvalidShape)?,
                        *right
    ...
    if let Some(output_shape) = output_shape {
        ...
        let ValueDataDraft::Matrix(values) = data else {
            return Err(ResidentKernelError::InvalidOutput);
        };
        ...
        Ok(replace_f64(output, |index| {
            ...
            let ValueDataDraft::F64(value) = values[row * columns + column] else {
                unreachable!("dense mixed binary float binds only F64 output")
            };
            value.to_f64()
        }))
    }
...
fn snapshot_bool_elements(value: &mech_core::Value) -> Result<Vec<bool>, ResidentKernelError> {
    match value.data() {
        ValueData::Bool(value) => Ok(vec![*value]),
        ValueData::Matrix(matrix) => match matrix.elements() {
            SequenceView::Bool(values) => Ok(values.to_vec()),
...
fn snapshot_bool_binary(
    ...
    .ok_or(ResidentKernelError::InvalidShape)?;
    let dimension_count = |(rows, columns): (usize, usize)| {
        rows.checked_mul(columns)
            .ok_or(ResidentKernelError::InvalidShape)
    };
    let left_count = dimension_count(left_dimensions)?;
    let right_count = dimension_count(right_dimensions)?;
    validate_binary_broadcast_len(left_count, left_mode, rows, columns)?;
    validate_binary_broadcast_len(right_count, right_mode, rows, columns)?;
    let output_len = rows
        .checked_mul(columns)
    ...
    let snapshot_inputs = [left, right].into_iter().flatten().collect::<Vec<_>>();
    preflight_snapshot_arithmetic(
        kernel,
        schemas,
        &snapshot_inputs,
    ...
    let at = |values: &[bool], mode, row, column| {
        values[snapshot_binary_broadcast_index(mode, row, column, columns)]
    };
    let mut result = Vec::with_capacity(output_len);
    for row in 0..rows {
        for column in 0..columns {
            let left = at(&left_values, left_mode, row, column);
            let right = at(&right_values, right_mode, row, column);
            ...
            result.push(ValueDataDraft::Bool(value));
        }
    }
    write_snapshot_data_for_shape_with_work_budget(
        kernel,
...
fn snapshot_bool_not(
    ...
    let schema = input
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    let (rows, columns) = snapshot_value_dimensions(input, schema.body())?;
    let output_len = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_shape = resolved_snapshot_output_shape(kernel, rows, columns)?;
    ...
    let values = snapshot_bool_elements(input)?;
    if values.len() != output_len {
        return Err(ResidentKernelError::InvalidShape);
    }
    let result = values
        .into_iter()
        .map(|value| ValueDataDraft::Bool(!value))
        .collect::<Vec<_>>();
    write_snapshot_data_for_shape_with_work_budget(
        kernel,
        output,
...
fn strict_mixed_snapshot_dense_equal(
    kernel: &BoundResidentKernel,
    snapshot: &mech_core::Value,
    dense: ResidentValueRef<'_>,
) -> Result<bool, ResidentKernelError> {
    let [_, bound_rows, bound_columns] = kernel.parameters() else {
        return Err(ResidentKernelError::InvalidInput);
    };
    let bound_rows = usize::try_from(*bound_rows).map_err(|_| ResidentKernelError::InvalidShape)?;
    let bound_columns =
        usize::try_from(*bound_columns).map_err(|_| ResidentKernelError::InvalidShape)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidInput)?;
    let schema = snapshot
    ...
    if !dense_kind_matches_schema_body(dense.kind(), element) {
        return Err(ResidentKernelError::InvalidInput);
    }
    let (rows, columns) = snapshot_matrix_dimensions(snapshot, schema.body())?;
    if (rows, columns) != (bound_rows, bound_columns) {
        return Ok(false);
    }
    let len = rows
        .checked_mul(columns)
    ...
    if matrix.elements().len() != len || dense.len() != len {
        return Err(ResidentKernelError::InvalidShape);
    }
    let mut meter = super::budget::ResidentBudgetMeter::default();
    let snapshot_footprint =
        super::budget::measure_canonical_value_footprint(&mut meter, snapshot, schemas)?;
    let dense_work = match dense {
        ResidentValueRef::String(values) => values.iter().try_fold(0u64, |work, value| {
            work.checked_add(super::budget::checked_u64(value.len().max(1))?)
                .ok_or(ResidentKernelError::InvalidShape)
        })?,
        ResidentValueRef::Bool(values) => {
            if values.iter().any(|value| *value > 1) {
        ...
        ResidentValueRef::Index(values) => super::budget::checked_u64(values.len())?,
        ResidentValueRef::F64(values) => super::budget::checked_u64(values.len())?,
        ResidentValueRef::Snapshot(_) => return Err(ResidentKernelError::InvalidInput),
    };
    let equality_work = snapshot_footprint
        .encoded_bytes
        .max(snapshot_footprint.node_count)
        .checked_add(dense_work)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let measured = meter.estimate();
    super::budget::PreparedKernel::new(
        (),
    ...
    .admit()?
    .into_plan();
    let resident_index = |canonical_index: usize| {
        let row = canonical_index / columns;
        let column = canonical_index % columns;
        row.checked_add(
            column
    ...
    match (matrix.elements(), dense) {
        (SequenceView::Bool(snapshot), ResidentValueRef::Bool(dense)) => {
            for (index, snapshot) in snapshot.iter().enumerate() {
                if *snapshot != (dense[resident_index(index)?] != 0) {
                    return Ok(false);
                }
            }
        }
        (SequenceView::Index(snapshot), ResidentValueRef::Index(dense)) => {
            for (index, snapshot) in snapshot.iter().enumerate() {
                if *snapshot != dense[resident_index(index)?] {
                    return Ok(false);
                }
            }
        }
        (SequenceView::F64(snapshot), ResidentValueRef::F64(dense)) => {
            for (index, snapshot) in snapshot.iter().enumerate() {
                if snapshot.to_f64() != dense[resident_index(index)?] {
                    return Ok(false);
                }
            }
        }
        (SequenceView::String(snapshot), ResidentValueRef::String(dense)) => {
            for (index, snapshot) in snapshot.iter().enumerate() {
                if snapshot.as_ref() != dense[resident_index(index)?].as_str() {
                    return Ok(false);
                }
            }
        }
...
fn strict_value_equal(
    ...
    Ok(match (left, right) {
        ...
        (ResidentValueRef::Snapshot([Some(snapshot)]), dense) => {
            let [snapshot_left, ..] = kernel.parameters() else {
                return Err(ResidentKernelError::InvalidInput);
            };
            if *snapshot_left != 1 {
                return Err(ResidentKernelError::InvalidInput);
            }
            strict_mixed_snapshot_dense_equal(kernel, snapshot, dense)?
        }
        (dense, ResidentValueRef::Snapshot([Some(snapshot)])) => {
            let [snapshot_left, ..] = kernel.parameters() else {
                return Err(ResidentKernelError::InvalidInput);
            };
            if *snapshot_left != 0 {
        ...
        (ResidentValueRef::Snapshot(_), _) | (_, ResidentValueRef::Snapshot(_)) => {
            return Err(ResidentKernelError::InvalidInput);
        }
        _ => false,
    })
...
fn transpose_snapshot(
    ...
    let input_schema = input
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    let (rows, columns) = snapshot_matrix_dimensions(input, input_schema.body())?;
    let count = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_shape = resolved_snapshot_output_shape(kernel, columns, rows)?;
    ...
    let publication_work = match (target.as_ref(), prior_footprint) {
        (Some(current), Some(prior)) => super::budget::projected_language_equality_work(
            ...
            prior,
            metadata.schema,
            output_shape.parameter_values().len(),
            output_footprint,
        )?,
        _ => 0,
    };
...
fn sum_snapshot(
    ...
    let [column, snapshot_output, tail @ ..] = kernel.parameters() else {
        return Err(ResidentKernelError::InvalidInput);
    };
    ...
    let (rows, columns) = snapshot_matrix_dimensions(input, schema.body())?;
    ...
    let (output_rows, output_columns) = if column { (rows, 1) } else { (1, columns) };
    let output_shape = snapshot_output
        .then(|| resolved_snapshot_output_shape(kernel, output_rows, output_columns))
        .transpose()?;
    if snapshot_output {
        if !tail.is_empty() {
            return Err(ResidentKernelError::InvalidInput);
        ...
        let [bound_rows, bound_columns] = tail else {
            return Err(ResidentKernelError::InvalidInput);
        };
        let bound_rows =
            usize::try_from(*bound_rows).map_err(|_| ResidentKernelError::InvalidShape)?;
        let bound_columns =
            usize::try_from(*bound_columns).map_err(|_| ResidentKernelError::InvalidShape)?;
        if (bound_rows, bound_columns) != (output_rows, output_columns) {
            return Err(ResidentKernelError::InvalidShape);
        }
        let ResidentValueMut::F64(values) = &output else {
            return Err(ResidentKernelError::InvalidOutput);
        ...
        let mut meter = super::budget::ResidentBudgetMeter::default();
        let (cloned_bytes, retained_nodes) = snapshot_clone_cost(&mut meter, input, schemas)?;
        let measured = meter.estimate();
        let output_bytes = output_count
            .checked_mul(core::mem::size_of::<f64>())
            .ok_or(ResidentKernelError::InvalidShape)?;
        let draft_bytes = input_count
            .checked_add(output_count)
            .and_then(|elements| elements.checked_mul(core::mem::size_of::<ValueDataDraft>()))
            .ok_or(ResidentKernelError::InvalidShape)?;
        super::budget::PreparedKernel::new(
            (),
            super::budget::resident_cost! {
                comparison_work: output_count,
                compute_work: measured.compute_work()
                    .checked_add(super::budget::checked_u64(input_count)?)
                    .ok_or(ResidentKernelError::InvalidShape)?,
                output_elements: output_count,
                output_bytes,
                temporary_bytes: cloned_bytes
                    .checked_add(super::budget::checked_u64(draft_bytes)?)
                    .ok_or(ResidentKernelError::InvalidShape)?,
                cloned_bytes,
                retained_nodes,
                ..super::budget::KernelCostEstimate::default()
    ...
    if let Some(output_shape) = output_shape {
        ...
        if output.len() != sums.len() {
            return Err(ResidentKernelError::InvalidShape);
        }
        if sums
            .iter()
            .any(|value| !matches!(value, ValueDataDraft::F64(_)))
        {
            return Err(ResidentKernelError::InvalidInput);
        }
        let mut changed = false;
        for (target, next) in output.iter_mut().zip(sums) {
            let ValueDataDraft::F64(next) = next else {
                unreachable!("snapshot F64 reduction was validated before publication")
            };
...
fn snapshot_numeric_dimensions(
    value: &mech_core::Value,
    schema: &SchemaBody,
) -> Result<(usize, usize), ResidentKernelError> {
    match schema {
        ...
        | SchemaBody::Complex(_)
        | SchemaBody::Rational64 => {
            if snapshot_numeric_element_count(value)? == 1 {
                Ok((1, 1))
            } else {
                Err(ResidentKernelError::InvalidShape)
            }
...
fn resident_shape_for_dimensions(
    rows: usize,
    columns: usize,
) -> Result<ResidentShape, ResidentKernelError> {
    Ok(ResidentShape {
        rows: u32::try_from(rows).map_err(|_| ResidentKernelError::InvalidShape)?,
        columns: u32::try_from(columns).map_err(|_| ResidentKernelError::InvalidShape)?,
    })
...
fn snapshot_broadcast_dimensions(
    left: (usize, usize),
    right: (usize, usize),
) -> Result<(usize, usize), ResidentKernelError> {
    ...
    Ok((dimension(left.0, right.0)?, dimension(left.1, right.1)?))
}

fn resolved_snapshot_numeric_output_shape(
    kernel: &BoundResidentKernel,
    rows: usize,
    columns: usize,
) -> Result<ShapeInstance, ResidentKernelError> {
    let metadata = kernel
        .snapshot_output()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let schemas = kernel
    ...
    if matches!(schema.body(), SchemaBody::Matrix { .. }) {
        resolved_snapshot_output_shape(kernel, rows, columns)
    } else if (rows, columns) == (1, 1) {
        Ok(metadata.shape.clone())
    } else {
        Err(ResidentKernelError::InvalidShape)
    }
}

fn resolved_snapshot_output_shape(
    kernel: &BoundResidentKernel,
    rows: usize,
    columns: usize,
) -> Result<ShapeInstance, ResidentKernelError> {
    ...
    let schema = schemas
        .get(metadata.schema)
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let rows = u64::try_from(rows).map_err(|_| ResidentKernelError::InvalidShape)?;
    let columns = u64::try_from(columns).map_err(|_| ResidentKernelError::InvalidShape)?;
    mech_core::shape_for_resolved_extents(schema, &[rows, columns])
        .map_err(|_| ResidentKernelError::InvalidShape)
}

struct ConstructorSnapshotInputs {
    staged: Vec<Vec<ValueDataDraft>>,
    dimensions: Vec<(usize, usize)>,
    output_dimensions: (usize, usize),
    output_shape: ShapeInstance,
    output_count: usize,
    canonicalization_work_limit: u64,
}

fn dense_constructor_input_footprint(
    input: ResidentValueRef<'_>,
    plan: &SnapshotConstructorInputPlan,
    meter: &mut super::budget::ResidentBudgetMeter,
) -> Result<ValueFootprint, ResidentKernelError> {
    let dimensions = plan.dimensions.ok_or(ResidentKernelError::InvalidInput)?;
    let count = dimensions
        .0
        .checked_mul(dimensions.1)
        .ok_or(ResidentKernelError::InvalidShape)?;
    if input.kind() != plan.kind || input.len() != count {
        return Err(ResidentKernelError::InvalidShape);
    }
    let (encoded_bytes, payload_bytes, sequence_bytes, nodes) = match input {
        ResidentValueRef::Bool(values) => {
            if values.iter().any(|value| *value > 1) {
                return Err(ResidentKernelError::InvalidInput);
            }
            let bytes = super::budget::checked_u64(values.len())?;
            (bytes, bytes, bytes, 3_u64)
        }
        ResidentValueRef::Index(values) => {
            let bytes = values
                .len()
        ...
        ResidentValueRef::String(values) => {
            ...
            (
                ...
                super::budget::checked_u64(payload)?,
                super::budget::checked_u64(
                    containers
                        .checked_add(payload)
                        .ok_or(ResidentKernelError::InvalidShape)?,
    ...
    let shape_bytes = plan
        .shape_parameters
        .checked_mul(core::mem::size_of::<u64>())
        .ok_or(ResidentKernelError::InvalidShape)?;
    let base = core::mem::size_of::<mech_core::Value>()
        .checked_add(shape_bytes)
        .and_then(|bytes| bytes.checked_add(core::mem::size_of::<ValueData>()))
        .ok_or(ResidentKernelError::InvalidShape)?;
    let retained_bytes = if plan.matrix {
        super::budget::checked_u64(base)?
    ...
    let nodes = if plan.matrix { nodes } else { 2 };
...
fn dense_constructor_input_drafts(
    input: ResidentValueRef<'_>,
    plan: &SnapshotConstructorInputPlan,
) -> Result<Vec<ValueDataDraft>, ResidentKernelError> {
    let (rows, columns) = plan.dimensions.ok_or(ResidentKernelError::InvalidInput)?;
    let count = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    if input.kind() != plan.kind || input.len() != count {
    ...
    match input {
        ResidentValueRef::Bool(values) => dense_indices
            .map(|index| match values[index] {
                0 => Ok(ValueDataDraft::Bool(false)),
                1 => Ok(ValueDataDraft::Bool(true)),
                _ => Err(ResidentKernelError::InvalidInput),
            })
            .collect(),
        ResidentValueRef::Index(values) => Ok(dense_indices
            .map(|index| ValueDataDraft::Index(values[index]))
            .collect()),
        ResidentValueRef::F64(values) => Ok(dense_indices
            .map(|index| ValueDataDraft::F64(F64Bits::from_f64(values[index])))
            .collect()),
        ResidentValueRef::String(values) => Ok(dense_indices
...
fn constructor_snapshot_inputs(
    ...
    horizontal: bool,
) -> Result<ConstructorSnapshotInputs, ResidentKernelError> {
    if kernel.parameters() != [inputs.len() as u64] {
        return Err(ResidentKernelError::InvalidInput);
    }
    let plan = kernel
        .retained_state::<SnapshotConstructorPlan>()
        .ok_or(ResidentKernelError::InvalidInput)?;
    ...
    let mut footprint_meter = super::budget::ResidentBudgetMeter::default();
    for (ordinal, input_plan) in plan.inputs.iter().enumerate() {
        let input_ref = inputs
            .get(ordinal)
            .ok_or(ResidentKernelError::InvalidInput)?;
        if input_ref.kind() != input_plan.kind {
            return Err(ResidentKernelError::InvalidInput);
        }
        if input_plan.kind != ResidentValueKind::Snapshot {
            let input_dimensions = input_plan
                .dimensions
                .ok_or(ResidentKernelError::InvalidInput)?;
            let count = input_dimensions
                .0
                .checked_mul(input_dimensions.1)
                .ok_or(ResidentKernelError::InvalidShape)?;
            dimensions.push(input_dimensions);
            output_count = output_count
                .checked_add(count)
                .ok_or(ResidentKernelError::InvalidShape)?;
            let footprint =
                dense_constructor_input_footprint(input_ref, input_plan, &mut footprint_meter)?;
        ...
        let ResidentValueRef::Snapshot([Some(input)]) = input_ref else {
            return Err(ResidentKernelError::InvalidInput);
        };
        if input.schema() != input_plan.schema {
            return Err(ResidentKernelError::InvalidInput);
        }
        let input_schema = input
            .validate_against(schemas)
            .map_err(|_| ResidentKernelError::InvalidInput)?;
        let (rows, columns) = match input_schema.body() {
            body if body == output_element.as_ref() => (1, 1),
            body @ SchemaBody::Matrix { element, .. } if element == output_element => {
                snapshot_matrix_dimensions(input, body)?
            }
            _ => return Err(ResidentKernelError::InvalidInput),
        };
        let count = rows
    ...
    let output_dimensions = if dimensions.is_empty() {
        plan.empty_dimensions
            .ok_or(ResidentKernelError::InvalidShape)?
    } else if horizontal {
        let rows = dimensions
            .first()
        ...
        let columns = dimensions.iter().try_fold(0usize, |sum, shape| {
            sum.checked_add(shape.1)
                .ok_or(ResidentKernelError::InvalidShape)
        })?;
        (rows, columns)
        ...
        let rows = dimensions.iter().try_fold(0usize, |sum, shape| {
            sum.checked_add(shape.0)
                .ok_or(ResidentKernelError::InvalidShape)
        })?;
        (rows, columns)
    ...
    if output_dimensions
        .0
        .checked_mul(output_dimensions.1)
        .ok_or(ResidentKernelError::InvalidShape)?
        != output_count
    {
        return Err(ResidentKernelError::InvalidShape);
    ...
    let output_shape =
        resolved_snapshot_output_shape(kernel, output_dimensions.0, output_dimensions.1)?;
    ...
    let final_nodes = result_nodes
        .checked_add(1)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let minimum_retained_bytes = core::mem::size_of::<mech_core::Value>()
        .checked_add(core::mem::size_of::<ValueData>())
        .and_then(|bytes| {
            bytes.checked_add(
                output_shape
                    .parameter_values()
        ...
        .ok_or(ResidentKernelError::InvalidShape)?;
    retained_bytes = retained_bytes.max(super::budget::checked_u64(minimum_retained_bytes)?);
    let final_output_footprint = ValueFootprint {
    ...
    for (ordinal, input_plan) in plan.inputs.iter().enumerate() {
        ...
        if input_plan.kind == ResidentValueKind::Snapshot {
            ...
            staged.push(dense_constructor_input_drafts(input_ref, input_plan)?);
        }
    }
    Ok(ConstructorSnapshotInputs {
        staged,
        dimensions,
        output_dimensions,
        output_shape,
...
fn concatenate_horizontal_snapshot(
    ...
    let prepared = constructor_snapshot_inputs(kernel, inputs, target.as_ref(), true)?;
    let mut result = Vec::with_capacity(prepared.output_count);
    let rows = prepared.output_dimensions.0;
    for row in 0..rows {
        for (values, (_, columns)) in prepared.staged.iter().zip(&prepared.dimensions) {
            let start = row
                .checked_mul(*columns)
                .ok_or(ResidentKernelError::InvalidShape)?;
            let end = start
    ...
    if result.len() != prepared.output_count {
        return Err(ResidentKernelError::IncompleteOutput);
    }
    write_snapshot_data_for_shape_with_work_budget(
        kernel,
        ...
        ValueDataDraft::Matrix(result.into_boxed_slice()),
        Some(prepared.canonicalization_work_limit),
    )
}
...
fn concatenate_vertical_snapshot(
    ...
    let prepared = constructor_snapshot_inputs(kernel, inputs, target.as_ref(), false)?;
    let mut result = Vec::with_capacity(prepared.output_count);
    for values in prepared.staged {
...
fn draft_range_number(data: &ValueDataDraft) -> Option<SnapshotRangeNumber> {
    Some(match data {
        ValueDataDraft::Index(value) => SnapshotRangeNumber::Unsigned(u128::from(*value)),
        ValueDataDraft::U8(value) => SnapshotRangeNumber::Unsigned(u128::from(*value)),
        ValueDataDraft::U16(value) => SnapshotRangeNumber::Unsigned(u128::from(*value)),
        ValueDataDraft::U32(value) => SnapshotRangeNumber::Unsigned(u128::from(*value)),
        ValueDataDraft::U64(value) => SnapshotRangeNumber::Unsigned(u128::from(*value)),
        ValueDataDraft::U128(value) => SnapshotRangeNumber::Unsigned(*value),
        ValueDataDraft::I8(value) => SnapshotRangeNumber::Signed(i128::from(*value)),
        ValueDataDraft::I16(value) => SnapshotRangeNumber::Signed(i128::from(*value)),
        ValueDataDraft::I32(value) => SnapshotRangeNumber::Signed(i128::from(*value)),
        ValueDataDraft::I64(value) => SnapshotRangeNumber::Signed(i128::from(*value)),
        ValueDataDraft::I128(value) => SnapshotRangeNumber::Signed(*value),
        ValueDataDraft::F32(value) => SnapshotRangeNumber::Float(f64::from(value.to_f32())),
        ValueDataDraft::F64(value) => SnapshotRangeNumber::Float(value.to_f64()),
        _ => return None,
    })
}
...
fn range_input_draft(
    ...
    let descriptor = kernel
        .bound_call()
        .and_then(|call| call.inputs().get(index))
        .ok_or(ResidentKernelError::InvalidInput)?;
    match (descriptor.schema().body(), inputs.get(index)) {
        (SchemaBody::Index, Some(ResidentValueRef::Index([value]))) => {
            Ok(ValueDataDraft::Index(*value))
        }
        (
            SchemaBody::FloatingPoint(mech_core::FloatWidth::W64),
            Some(ResidentValueRef::F64([value])),
        ) => Ok(ValueDataDraft::F64(F64Bits::from_f64(*value))),
        (_, Some(ResidentValueRef::Snapshot([Some(value)]))) => {
            let schemas = kernel
                .snapshot_schemas()
                .ok_or(ResidentKernelError::InvalidOutput)?;
            let schema = value
            ...
            let data = value
                .canonical_data_draft()
                .map_err(|_| ResidentKernelError::InvalidInput)?;
            if matches!(data, ValueDataDraft::Matrix(_)) {
                return Err(ResidentKernelError::InvalidInput);
            }
            Ok(data)
        }
...
fn range_snapshot(
    ...
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let first = range_input_draft(kernel, inputs, 0)?;
    let second = range_input_draft(kernel, inputs, 1)?;
    let third = if incremented {
        range_input_draft(kernel, inputs, 2)?
    } else {
        second.clone()
    };
    let numbers = [
    ...
    let snapshot_inputs = (0..expected_inputs)
        .filter_map(|index| match inputs.get(index) {
            Some(ResidentValueRef::Snapshot([Some(value)])) => Some(value),
            _ => None,
        })
        .collect::<Vec<_>>();
    preflight_snapshot_arithmetic(
...
fn n_choose_k_snapshot(
    ...
    let input_snapshot = flag(*input_snapshot)?;
    let selection_snapshot = flag(*selection_snapshot)?;
    let values_input = inputs.get(0).ok_or(ResidentKernelError::InvalidInput)?;
    let selection = inputs.get(1).ok_or(ResidentKernelError::InvalidInput)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let output_schema = schemas
    ...
    let values_snapshot = match (input_snapshot, values_input) {
        (true, ResidentValueRef::Snapshot([Some(values)])) => {
        ...
        (false, ResidentValueRef::F64(_))
            if element.as_ref() == &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64) =>
        {
            None
        }
        _ => return Err(ResidentKernelError::InvalidInput),
    ...
    let bound_columns =
        usize::try_from(*bound_columns).map_err(|_| ResidentKernelError::InvalidShape)?;
    let available = match values_snapshot {
        Some(values) => snapshot_numeric_element_count(values)?,
        None => {
            let available = bound_rows
                .checked_mul(bound_columns)
                .ok_or(ResidentKernelError::InvalidShape)?;
            if values_input.len() != available {
                return Err(ResidentKernelError::InvalidShape);
            }
            available
        }
    };
    let (requested, selection_value) = match (selection_snapshot, selection) {
        (true, ResidentValueRef::Snapshot([Some(selection)])) => {
            let selection_schema = selection
                .validate_against(schemas)
                .map_err(|_| ResidentKernelError::InvalidInput)?;
            if selection_schema.body() != element.as_ref() {
                return Err(ResidentKernelError::InvalidInput);
            }
            (
                canonical_n_choose_k_selection(selection.data())
                    .ok_or(ResidentKernelError::InvalidInput)?
                    .0,
                Some(selection),
            )
        }
        (false, ResidentValueRef::F64([selection])) => {
            if !selection.is_finite()
                || *selection < 0.0
                || selection.fract() != 0.0
                || *selection > u128::MAX as f64
            {
                return Err(ResidentKernelError::InvalidInput);
            }
            (*selection as u128, None)
        }
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    ...
    let output_count = requested
        ...
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_shape = resolved_snapshot_output_shape(kernel, requested, combinations)?;
    let input_elements = available
        .checked_add(1)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let snapshot_inputs = match (values_snapshot, selection_value) {
        (Some(values), Some(selection)) => vec![values, selection],
        (Some(values), None) => vec![values],
        (None, Some(selection)) => vec![selection],
        (None, None) => Vec::new(),
    };
    preflight_snapshot_arithmetic(
        kernel,
    ...
    let values = match values_snapshot {
        Some(values) => snapshot_numeric_elements(values)?,
        None => dense_f64_canonical_drafts(values_input, bound_rows, bound_columns)?,
    };
...
fn snapshot_scalar_index(
    ...
    input
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    let input_len = match input.data() {
        ValueData::Matrix(matrix) => matrix.elements().len(),
        _ => 1,
    };
    let output_shape = resolved_snapshot_output_shape(kernel, input_len, 1)?;
    preflight_snapshot_arithmetic(
        kernel,
        schemas,
        &[input],
    ...
    let next = (0..input_len)
        .map(|ordinal| {
            portable_data_index_at(input.data(), ordinal)
                .map(ValueDataDraft::Index)
                .ok_or(ResidentKernelError::InvalidInput)
        })
        .collect::<Result<Vec<_>, _>>()?;
...
fn portable_data_index_at(value: &ValueData, ordinal: usize) -> Option<u64> {
    match value {
        ValueData::Matrix(matrix) => portable_sequence_index_at(matrix.elements(), ordinal),
        value if ordinal == 0 => portable_data_index(value),
        _ => None,
...
fn selector_value(
    ...
    resident_value_to_snapshot(
        schemas,
        layout.schema,
        &layout.shape,
        layout.resident_shape,
        value,
    )
}

pub(crate) fn resident_value_to_snapshot(
    schemas: &mech_core::SchemaTable,
    schema_id: SchemaId,
    shape: &ShapeInstance,
    resident_shape: ResidentShape,
    ...
    let rows = resident_shape.rows as usize;
    let columns = resident_shape.columns as usize;
    let count = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    if value.len() != count {
...
pub(crate) struct ResidentSnapshotMaterializationCost {
...
/// Computes the complete peak for dense-to-snapshot conversion without
/// cloning any payload. The caller must hold this admission while building
/// the draft and final immutable value.
pub(crate) fn resident_value_snapshot_materialization_cost(
    schemas: &mech_core::SchemaTable,
    schema: SchemaId,
    shape: &ShapeInstance,
    value: ResidentValueRef<'_>,
) -> Result<ResidentSnapshotMaterializationCost, ResidentKernelError> {
    if matches!(value, ResidentValueRef::Snapshot(_)) {
        return Err(ResidentKernelError::InvalidInput);
    }
    let overflow = || ResidentKernelError::InvalidShape;
    let elements = u64::try_from(value.len()).map_err(|_| overflow())?;
    let mut payload_bytes = elements
        .checked_mul(core::mem::size_of::<ValueData>() as u64)
        .ok_or_else(overflow)?;
    let mut string_bytes = 0_u64;
    if let ResidentValueRef::String(values) = value {
        for value in values {
    ...
    let body = schema
        .closed_body(shape)
        .map_err(|_| ResidentKernelError::InvalidShape)?;
    let retained_nodes = match (&body, value) {
        (SchemaBody::Matrix { element, .. }, ResidentValueRef::String(_))
            if element.as_ref() == &SchemaBody::String =>
        {
            elements.checked_add(3).ok_or_else(overflow)?
        }
        (SchemaBody::Matrix { .. }, _) => 3,
        (_, _) => 2,
    };
    ...
    let temporary_bytes =
        mech_core::canonical_snapshot_draft_bytes(footprint).map_err(|_| overflow())?;
    let persistent_bytes = mech_core::canonical_snapshot_finalization_bytes(footprint)
        .map_err(|_| overflow())?
        .checked_add(mech_core::Value::memory_budget_claim_metadata_bytes())
        .ok_or_else(overflow)?;
    Ok(ResidentSnapshotMaterializationCost {
        persistent_bytes,
        temporary_bytes,
        cloned_bytes: payload_bytes,
        output_elements: elements,
        retained_nodes: footprint.retained_nodes,
        // Draft construction and canonical finalization each visit every
        // element. Strings additionally copy and validate their complete
        // retained byte population.
        compute_work: elements
            .checked_mul(2)
            .and_then(|work| work.checked_add(string_bytes))
            .ok_or_else(overflow)?,
        // Snapshot publication compares every canonical cell and may scan the
        // complete retained byte population of equal or common-prefix strings.
        comparison_work: elements.checked_add(string_bytes).ok_or_else(overflow)?,
    })
}
...
fn snapshot_access_source_dimensions(
    source: &mech_core::Value,
    source_schema: &SchemaBody,
    plan: &SnapshotAccessPlan,
) -> Result<(usize, usize), ResidentKernelError> {
    match plan.source_dimensions {
        Some(dimensions) => Ok(dimensions),
        None if plan.matrix_mode.is_some() => snapshot_matrix_dimensions(source, source_schema),
        None => Err(ResidentKernelError::InvalidInput),
    }
}
...
fn snapshot_access_output_cost(
    ...
    match (source_schema, source.data()) {
        ...
        (SchemaBody::Matrix { element, .. }, ValueData::Matrix(matrix)) => {
            ...
            let resolved_output_dimensions = if plan.matrix_constructor.is_some() {
                plan.output_dimensions
            } else {
                match plan.matrix_mode {
                    Some(ResolvedSelectionMode::LinearScalar) => None,
                    Some(ResolvedSelectionMode::LinearGather) => Some((
                        if plan.selectors.is_empty() {
                            source_len
                        } else {
                            selector_access_count(selector(0)?, source_len)?
                        },
                        1,
                    )),
                    Some(ResolvedSelectionMode::Rows) => {
                        Some((selector_access_count(selector(0)?, rows)?, columns))
                    }
                    Some(ResolvedSelectionMode::Columns) => {
                        Some((rows, selector_access_count(selector(0)?, columns)?))
                    }
                    Some(ResolvedSelectionMode::Rectangle) => {
                        let selected = (
                            selector_access_count(selector(0)?, rows)?,
                            selector_access_count(selector(1)?, columns)?,
                        );
            ...
            if !plan.output_dimensions_dynamic
                && plan.output_dimensions != resolved_output_dimensions
            {
                return Err(ResidentKernelError::InvalidShape);
            }
            let expected_count = match resolved_output_dimensions {
            ...
            if plan.matrix_constructor.is_some() {
                ...
                return Ok(SnapshotAccessOutputCost {
                    ...
                    selected_ordinal: None,
                    finalization_work,
                    output_dimensions: resolved_output_dimensions,
                });
            }
            let mut footprint = ValueFootprint::zero();
            let mut finalization_work = 0_u64;
            let (count, index_elements) = match plan.matrix_mode {
                ...
                Some(ResolvedSelectionMode::LinearGather) => {
                    ...
                    if plan.selectors.is_empty() {
                        ...
                        selector_for_each_access_index(selector(0)?, source_len, visit)?;
                    }
                    (count, count)
                }
                Some(ResolvedSelectionMode::Rows) => {
...
fn snapshot_access_data(
    ...
    match source_schema {
        ...
        SchemaBody::Matrix { element, .. } => {
            ...
            let (selected_rows, selected_columns, scalar) = match plan.matrix_mode {
                ...
                Some(ResolvedSelectionMode::LinearGather) => {
                    let selected = if selectors.is_empty() {
                        ...
                        access_indices(&selectors[0], source_len)?
                    };
                    let output_elements = output_cost
                        .output_dimensions
            ...
            if output_cost.output_dimensions != Some((selected_rows.len(), selected_columns.len()))
            {
                return Err(ResidentKernelError::InvalidShape);
            }
...
fn write_access_output(
    ...
    plan: &SnapshotAccessPlan,
    output_cost: &SnapshotAccessOutputCost,
    output_shape: Option<&ShapeInstance>,
    data: ValueDataDraft,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    let next = if let Some(shape) = output_shape {
        finalize_snapshot_data_for_shape_with_work_budget(
            kernel,
            shape,
            data,
            Some(output_cost.finalization_work),
        )?
    } else {
        finalize_snapshot_data_with_work_budget(kernel, data, Some(output_cost.finalization_work))?
    };
    ...
    let (rows, columns) = output_cost.output_dimensions.unwrap_or((
        plan.output_geometry.logical_output_rows,
        plan.output_geometry.logical_output_columns,
    ));
    let count = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
...
fn dense_snapshot_access_source(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    plan: &SnapshotAccessPlan,
    layout: &SnapshotAccessSelectorLayout,
) -> Result<(mech_core::Value, u64), ResidentKernelError> {
    let source = input(inputs, 0)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    ...
    let dimensions = plan
        .source_dimensions
        .ok_or(ResidentKernelError::InvalidShape)?;
    let constructor_plan = SnapshotConstructorInputPlan {
        schema: layout.schema,
        kind: source.kind(),
        dimensions: Some(dimensions),
        matrix: true,
        shape_parameters: layout.shape.parameter_values().len(),
    };
    let mut meter = super::budget::ResidentBudgetMeter::default();
    let footprint = dense_constructor_input_footprint(source, &constructor_plan, &mut meter)?;
    let count = dimensions
        .0
        .checked_mul(dimensions.1)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let draft_bytes = count
        .checked_mul(core::mem::size_of::<ValueDataDraft>())
        .ok_or(ResidentKernelError::InvalidShape)?;
    let measured = meter.estimate();
    super::budget::PreparedKernel::new(
        ...
        super::budget::resident_cost! {
            ...
            compute_work: measured.compute_work()
                .checked_add(super::budget::checked_u64(count)?)
                .ok_or(ResidentKernelError::InvalidShape)?,
            temporary_bytes: footprint.retained_bytes
                .checked_add(super::budget::checked_u64(draft_bytes)?)
                .ok_or(ResidentKernelError::InvalidShape)?,
            cloned_bytes: footprint.retained_bytes,
            container_bytes: draft_bytes,
    ...
    .admit()?
    .into_plan();
    let value = resident_value_to_snapshot(
        schemas,
        layout.schema,
        &layout.shape,
        layout.resident_shape,
    ...
    Ok((value, footprint.retained_bytes))
}
...
fn snapshot_access(
    ...
    if let Some(layout) = &plan.dense_source {
        let (source, temporary_bytes) = dense_snapshot_access_source(kernel, inputs, plan, layout)?;
        return snapshot_access_from_source(kernel, inputs, output, plan, &source, temporary_bytes);
    }
    let Some(ResidentValueRef::Snapshot([Some(source)])) = inputs.get(0) else {
        return Err(ResidentKernelError::InvalidInput);
    };
    snapshot_access_from_source(kernel, inputs, output, plan, source, 0)
}

fn snapshot_access_from_source(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
    plan: &SnapshotAccessPlan,
    source: &mech_core::Value,
    source_temporary_bytes: u64,
) -> Result<bool, ResidentKernelError> {
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    ...
    // Resolve parameterized result axes before admission. This validates
    // fixed axes independently even when a compensating axis would preserve
    // the same total element count.
    let resolved_output_shape = output_cost
        .output_dimensions
        .map(|(rows, columns)| resolved_snapshot_output_shape(kernel, rows, columns))
        .transpose()?;
    let footprint_work = footprint_meter.estimate();
    ...
    let cloned_bytes = selector_cost
        ...
        .and_then(|bytes| bytes.checked_add(source_temporary_bytes))
        .ok_or(ResidentKernelError::InvalidShape)?;
...
fn matrix_multiply_snapshot_mixed(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    let lhs_input = inputs.get(0).ok_or(ResidentKernelError::InvalidInput)?;
    let rhs_input = inputs.get(1).ok_or(ResidentKernelError::InvalidInput)?;
    let [
        snapshot_output,
        lhs_snapshot,
        rhs_snapshot,
    ...
    let snapshot_output = flag(*snapshot_output)?;
    let lhs_snapshot = flag(*lhs_snapshot)?;
    let rhs_snapshot = flag(*rhs_snapshot)?;
    if lhs_snapshot == rhs_snapshot {
        return Err(ResidentKernelError::InvalidInput);
    }
    let snapshot_value = |input, expected| match (input, expected) {
    ...
    let lhs = snapshot_value(lhs_input, lhs_snapshot)?;
    let rhs = snapshot_value(rhs_input, rhs_snapshot)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let lhs_schema = lhs
    ...
    let lhs_dimensions = match (lhs, lhs_schema) {
        (Some(value), Some(schema)) => snapshot_matrix_dimensions(value, schema.body())?,
        (None, None) => bound_dimensions(*bound_lhs_rows, *bound_lhs_columns)?,
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    let rhs_dimensions = match (rhs, rhs_schema) {
        (Some(value), Some(schema)) => snapshot_matrix_dimensions(value, schema.body())?,
        (None, None) => bound_dimensions(*bound_rhs_rows, *bound_rhs_columns)?,
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    let (rows, inner) = lhs_dimensions;
    let (rhs_rows, columns) = rhs_dimensions;
    if inner != rhs_rows {
        return Err(ResidentKernelError::InvalidShape);
    }
    for schema in [lhs_schema, rhs_schema].into_iter().flatten() {
        if comparison_element_body(schema.body())
            != &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
        {
            return Err(ResidentKernelError::InvalidInput);
        }
    ...
    let compute_work = output_len
        ...
        .and_then(|work| work.checked_mul(2))
        .ok_or(ResidentKernelError::InvalidShape)?;
    let input_elements = lhs_len
        .checked_add(rhs_len)
        .ok_or(ResidentKernelError::InvalidShape)?;
    match (lhs, rhs, snapshot_output) {
        (Some(value), None, true) | (None, Some(value), true) => preflight_snapshot_arithmetic(
            kernel,
            schemas,
            &[value],
            &output,
        ...
        (Some(value), None, false) | (None, Some(value), false) => {
            preflight_snapshot_dense_f64_output(
                schemas,
                &[value],
                input_elements,
                output_len,
    ...
    // Snapshot cloning and dense canonicalization both allocate input-sized
    // staging vectors. Keep them behind the complete mixed-operation permit.
    let lhs_values = match lhs {
        Some(value) => snapshot_numeric_elements(value)?,
        None => dense_f64_canonical_drafts(lhs_input, rows, inner)?,
    };
    let rhs_values = match rhs {
        Some(value) => snapshot_numeric_elements(value)?,
        None => dense_f64_canonical_drafts(rhs_input, inner, columns)?,
    };
    if lhs_values.len() != lhs_len || rhs_values.len() != rhs_len {
        return Err(ResidentKernelError::InvalidShape);
    }
    let output_shape = snapshot_output
        .then(|| resolved_snapshot_output_shape(kernel, rows, columns))
        .transpose()?;
    let mut result = Vec::with_capacity(output_len);
    for row in 0..rows {
        for column in 0..columns {
            let mut sum = ValueDataDraft::F64(F64Bits::from_f64(0.0));
            for offset in 0..inner {
                let product = numeric_multiply(
                    lhs_values[row * inner + offset].clone(),
                    rhs_values[offset * columns + column].clone(),
                )?;
    ...
    if let Some(output_shape) = output_shape {
        ...
        Ok(replace_f64(output, |index| {
            let row = index % rows;
            let column = index / rows;
            let ValueDataDraft::F64(value) = result[row * columns + column] else {
                unreachable!("dense mixed matrix product binds only F64 output")
            };
            value.to_f64()
        }))
    }
...
fn matrix_multiply_snapshot_f64(
    ...
    matrix_dot_snapshot_f64_impl(kernel, inputs, output, false)
}
...
fn preflight_snapshot_arithmetic(
    ...
    let output_schema = schemas
        .get(output_metadata.schema)
        .ok_or(ResidentKernelError::InvalidOutput)?;
    // Canonical primitive matrices retain a value, matrix, and packed-sequence
    // node regardless of population. Scalars retain the value and scalar body.
    let (output_element, output_nodes) = match output_schema.body() {
        SchemaBody::Matrix { element, .. } => (element.as_ref(), 3),
        scalar => (scalar, 2),
    };
    let output_encoded_bytes = output_elements
        .checked_mul(
            snapshot_fixed_element_encoded_bytes(output_element)
                .ok_or(ResidentKernelError::InvalidOutput)?,
        )
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_footprint = ValueFootprint {
        // Fixed-width primitive matrices encode as the packed element
        // population; wrapper and packed-sequence visits use node_count.
        encoded_bytes: super::budget::checked_u64(output_encoded_bytes)?,
        retained_bytes: output_bytes,
        node_count: output_nodes,
    };
...
fn preflight_snapshot_dense_f64_output(
    schemas: &mech_core::SchemaTable,
    inputs: &[&mech_core::Value],
    input_elements: usize,
    output_elements: usize,
    ...
    let mut footprint_meter = super::budget::ResidentBudgetMeter::default();
    let (cloned_bytes, retained_nodes) =
        inputs
            .iter()
    ...
    super::budget::PreparedKernel::new(
        ...
        super::budget::resident_cost! {
            comparison_work: measured.comparison_work()
                .checked_add(super::budget::checked_u64(output_elements)?)
                .ok_or(ResidentKernelError::InvalidShape)?,
            compute_work: measured.compute_work()
                .checked_add(super::budget::checked_u64(output_elements)?)
                .and_then(|work| work.checked_add(
                    super::budget::checked_u64(additional_compute_work).ok()?
                ))
                .ok_or(ResidentKernelError::InvalidShape)?,
            output_elements,
            output_bytes: output_elements
...
fn snapshot_numeric_binary(
    ...
    let right_schema = right
        ...
        .transpose()?;
    let left_dimensions = match (left, left_schema) {
        (Some(value), Some(schema)) => snapshot_numeric_dimensions(value, schema.body())?,
        (None, None) => (
            usize::try_from(*bound_left_rows).map_err(|_| ResidentKernelError::InvalidShape)?,
            usize::try_from(*bound_left_columns).map_err(|_| ResidentKernelError::InvalidShape)?,
        ),
    ...
    let left_len = left.map_or_else(
        ...
        snapshot_numeric_element_count,
    )?;
    let right_len = right.map_or_else(
        || {
            right_dimensions
                .0
                .checked_mul(right_dimensions.1)
    ...
    let left = match left {
        Some(value) => snapshot_numeric_elements(value)?,
        None => dense_f64_canonical_drafts(left_input, left_dimensions.0, left_dimensions.1)?,
    };
    let right = match right {
        Some(value) => snapshot_numeric_elements(value)?,
        None => dense_f64_canonical_drafts(right_input, right_dimensions.0, right_dimensions.1)?,
    };
    let mut result = Vec::with_capacity(output_len);
    for row in 0..rows {
        for column in 0..columns {
    ...
    if let Some(output_shape) = output_shape {
        ...
        Ok(replace_f64(output, |index| {
            ...
            let column = index / rows;
            let ValueDataDraft::F64(value) = result[row * columns + column] else {
                unreachable!("dense mixed arithmetic binds only F64 output")
            };
            value.to_f64()
        }))
    }
...
fn snapshot_numeric_negate(
    ...
    let [_bound_rows, _bound_columns] = kernel.parameters() else {
        return Err(ResidentKernelError::InvalidInput);
    };
    let schemas = kernel
        .snapshot_schemas()
...
fn complex32_integer_power(mut base: (f32, f32), exponent: i32) -> (f32, f32) {
    ...
    if negative {
        complex32_divide((1.0, 0.0), result)
    } else {
        result
    }
}
...
fn complex64_integer_power(mut base: (f64, f64), exponent: i32) -> (f64, f64) {
    ...
    if negative {
        complex64_divide((1.0, 0.0), result)
    } else {
        result
    }
}
...
fn write_snapshot_data_for_shape_with_work_budget(
    kernel: &BoundResidentKernel,
    output: ResidentValueMut<'_>,
    shape: &ShapeInstance,
    data: ValueDataDraft,
...
fn finalize_snapshot_data_for_shape_with_work_budget(
    kernel: &BoundResidentKernel,
    shape: &ShapeInstance,
    data: ValueDataDraft,
    canonicalization_work_limit: Option<u64>,
...
fn matrix_dot_snapshot(
    ...
    let right_schema = right
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    let left_dimensions = snapshot_numeric_dimensions(left, left_schema.body())?;
    let right_dimensions = snapshot_numeric_dimensions(right, right_schema.body())?;
...
fn matrix_dot_snapshot_f64(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    matrix_dot_snapshot_f64_impl(kernel, inputs, output, true)
}

fn preflight_snapshot_dot_f64(
    schemas: &mech_core::SchemaTable,
    snapshots: &[&mech_core::Value],
    element_count: usize,
) -> Result<(), ResidentKernelError> {
    let mut meter = super::budget::ResidentBudgetMeter::default();
    let (cloned_bytes, retained_nodes) =
        snapshots
            .iter()
            .try_fold((0_u64, 0_u64), |(bytes, nodes), snapshot| {
                let (next_bytes, next_nodes) = snapshot_clone_cost(&mut meter, snapshot, schemas)?;
                Ok::<_, ResidentKernelError>((
                    bytes
                        .checked_add(next_bytes)
                        .ok_or(ResidentKernelError::InvalidShape)?,
    ...
    let draft_bytes = element_count
        .checked_mul(2)
        .and_then(|elements| elements.checked_mul(core::mem::size_of::<ValueDataDraft>()))
        .ok_or(ResidentKernelError::InvalidShape)?;
    let arithmetic_work = element_count
        .checked_mul(2)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let measured = meter.estimate();
    super::budget::PreparedKernel::new(
        ...
        super::budget::resident_cost! {
            comparison_work: measured.comparison_work()
                ...
                .ok_or(ResidentKernelError::InvalidShape)?,
            compute_work: measured.compute_work()
                .checked_add(super::budget::checked_u64(arithmetic_work)?)
                .ok_or(ResidentKernelError::InvalidShape)?,
            output_elements: 1,
            output_bytes: core::mem::size_of::<f64>(),
            temporary_bytes: cloned_bytes
                .checked_add(super::budget::checked_u64(draft_bytes)?)
                .ok_or(ResidentKernelError::InvalidShape)?,
            cloned_bytes,
...
fn matrix_dot_snapshot_f64_impl(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
    require_same_axes: bool,
) -> Result<bool, ResidentKernelError> {
    let (
        Some(ResidentValueRef::Snapshot([Some(left)])),
        Some(ResidentValueRef::Snapshot([Some(right)])),
    ...
    let left_dimensions = snapshot_numeric_dimensions(left, left_schema.body())?;
    let right_dimensions = snapshot_numeric_dimensions(right, right_schema.body())?;
    if require_same_axes && left_dimensions != right_dimensions {
        return Err(ResidentKernelError::InvalidShape);
    }
    let left_count = snapshot_numeric_element_count(left)?;
    let right_count = snapshot_numeric_element_count(right)?;
    ...
    preflight_snapshot_dot_f64(schemas, &[left, right], left_count)?;
    let left = snapshot_numeric_elements(left)?;
    let right = snapshot_numeric_elements(right)?;
    let mut next = 0.0;
    for (left, right) in left.into_iter().zip(right) {
        let (ValueDataDraft::F64(left), ValueDataDraft::F64(right)) = (left, right) else {
            return Err(ResidentKernelError::InvalidInput);
        };
        next += left.to_f64() * right.to_f64();
    }
    let changed = target.to_bits() != next.to_bits();
    *target = next;
    Ok(changed)
...
fn matrix_dot_snapshot_mixed_f64(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    ...
    let left_input = inputs.get(0).ok_or(ResidentKernelError::InvalidInput)?;
    let right_input = inputs.get(1).ok_or(ResidentKernelError::InvalidInput)?;
    let snapshot = match (left_input, right_input, left_snapshot, right_snapshot) {
        (ResidentValueRef::Snapshot([Some(value)]), ResidentValueRef::F64(_), true, false)
        | (ResidentValueRef::F64(_), ResidentValueRef::Snapshot([Some(value)]), false, true) => {
            value
        }
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    ...
    let snapshot_schema = snapshot
        .validate_against(schemas)
        .map_err(|_| ResidentKernelError::InvalidInput)?;
    if comparison_element_body(snapshot_schema.body())
        != &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
    {
        return Err(ResidentKernelError::InvalidInput);
    }
    ...
    let left_dimensions = if left_snapshot {
        snapshot_numeric_dimensions(snapshot, snapshot_schema.body())?
    } else {
        bound_dimensions(*left_rows, *left_columns)?
    };
    let right_dimensions = if right_snapshot {
        snapshot_numeric_dimensions(snapshot, snapshot_schema.body())?
    } else {
        bound_dimensions(*right_rows, *right_columns)?
    };
    if left_dimensions != right_dimensions {
        return Err(ResidentKernelError::InvalidShape);
    }
    let element_count = left_dimensions
        .0
        .checked_mul(left_dimensions.1)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let dense = if left_snapshot {
        right_input
    } else {
        left_input
    };
    if dense.len() != element_count || snapshot_numeric_element_count(snapshot)? != element_count {
        return Err(ResidentKernelError::InvalidShape);
    }
    let output = f64_output(output)?;
    let [target] = output else {
    ...
    preflight_snapshot_dot_f64(schemas, &[snapshot], element_count)?;
    let snapshot_values = snapshot_numeric_elements(snapshot)?;
    let dense_values = dense_f64_canonical_drafts(dense, left_dimensions.0, left_dimensions.1)?;
    let (left, right) = if left_snapshot {
        (snapshot_values, dense_values)
    } else {
        (dense_values, snapshot_values)
    };
    let mut next = 0.0;
    for (left, right) in left.into_iter().zip(right) {
        let (ValueDataDraft::F64(left), ValueDataDraft::F64(right)) = (left, right) else {
...
fn matrix_solve_snapshot_mixed_f64(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    let [
        ...
        right_snapshot,
        bound_coefficient_rows,
        bound_coefficient_columns,
        bound_right_rows,
        bound_right_columns,
    ] = kernel.parameters()
    else {
    ...
    let snapshot_output = flag(*snapshot_output)?;
    let coefficient_snapshot = flag(*coefficient_snapshot)?;
    let right_snapshot = flag(*right_snapshot)?;
    if coefficient_snapshot == right_snapshot {
        return Err(ResidentKernelError::InvalidInput);
    }
    let coefficient_input = inputs.get(0).ok_or(ResidentKernelError::InvalidInput)?;
    let right_input = inputs.get(1).ok_or(ResidentKernelError::InvalidInput)?;
    let snapshot_value = |input, expected| match (input, expected) {
        (ResidentValueRef::Snapshot([Some(value)]), true) => Ok(Some(value)),
        (ResidentValueRef::Snapshot(_), _) | (_, true) => Err(ResidentKernelError::InvalidInput),
        (ResidentValueRef::F64(_), false) => Ok(None),
        _ => Err(ResidentKernelError::InvalidInput),
    };
    let coefficients = snapshot_value(coefficient_input, coefficient_snapshot)?;
    let right = snapshot_value(right_input, right_snapshot)?;
    let schemas = kernel
        .snapshot_schemas()
        .ok_or(ResidentKernelError::InvalidOutput)?;
    let coefficient_schema = coefficients
        .map(|value| {
            value
                .validate_against(schemas)
                .map_err(|_| ResidentKernelError::InvalidInput)
    ...
    let coefficient_dimensions = match (coefficients, coefficient_schema) {
        (Some(value), Some(schema)) => snapshot_matrix_dimensions(value, schema.body())?,
        (None, None) => bound_dimensions(*bound_coefficient_rows, *bound_coefficient_columns)?,
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    let right_dimensions = match (right, right_schema) {
        (Some(value), Some(schema)) => snapshot_matrix_dimensions(value, schema.body())?,
        (None, None) => bound_dimensions(*bound_right_rows, *bound_right_columns)?,
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    for schema in [coefficient_schema, right_schema].into_iter().flatten() {
        if comparison_element_body(schema.body())
            != &SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)
        {
            return Err(ResidentKernelError::InvalidInput);
    ...
    let coefficient_count = rows
        .checked_mul(columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let right_count = right_rows
        .checked_mul(right_columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_count = rows
        .checked_mul(right_columns)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let input_elements = coefficient_count
        .checked_add(right_count)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let snapshot_input = coefficients
        .or(right)
        .ok_or(ResidentKernelError::InvalidInput)?;
    if snapshot_output {
        preflight_snapshot_arithmetic(
            ...
            input_elements,
            output_count,
            solve_work.saturating_sub(output_count),
        )?;
    } else {
        preflight_snapshot_dense_f64_output(
            schemas,
    ...
    let snapshot_column_major =
        |value: &mech_core::Value, rows: usize, columns: usize| match value.data() {
            ValueData::Matrix(matrix) => match matrix.elements() {
                SequenceView::F64(values) if values.len() == rows.saturating_mul(columns) => Ok((0
                    ..columns)
                    .flat_map(|column| {
                        (0..rows).map(move |row| values[row * columns + column].to_f64())
                    })
                    .collect::<Vec<_>>()),
                _ => Err(ResidentKernelError::InvalidInput),
            },
    ...
    let dense_column_major = |input: ResidentValueRef<'_>, expected: usize| match input {
        ResidentValueRef::F64(values) if values.len() == expected => Ok(values.to_vec()),
        _ => Err(ResidentKernelError::InvalidInput),
    };
    let coefficient_values = match coefficients {
        Some(value) => snapshot_column_major(value, rows, columns)?,
        None => dense_column_major(coefficient_input, coefficient_count)?,
    };
    let right_values = match right {
        Some(value) => snapshot_column_major(value, right_rows, right_columns)?,
        None => dense_column_major(right_input, right_count)?,
    };
    let next = solve_dense(coefficient_values, right_values, rows, right_columns)?;
    if snapshot_output {
        let output_shape = resolved_snapshot_output_shape(kernel, rows, right_columns)?;
        let data = (0..rows)
            .flat_map(|row| {
                let next = &next;
...
fn matrix_solve_snapshot(
    kernel: &BoundResidentKernel,
    inputs: &dyn ResidentKernelInputs,
    output: ResidentValueMut<'_>,
) -> Result<bool, ResidentKernelError> {
    let [snapshot_output] = kernel.parameters() else {
        return Err(ResidentKernelError::InvalidInput);
    };
    let snapshot_output = match *snapshot_output {
        0 => false,
    ...
    ) = (coefficient_schema.body(), right_schema.body())
    else {
        return Err(ResidentKernelError::InvalidInput);
    };
    let output_element = if snapshot_output {
        let metadata = kernel
            .snapshot_output()
            .ok_or(ResidentKernelError::InvalidOutput)?;
        let output_schema = schemas
    ...
    if coefficient_element != right_element || coefficient_element.as_ref() != output_element {
        return Err(ResidentKernelError::InvalidInput);
    }
    let (rows, columns) = snapshot_matrix_dimensions(coefficients, coefficient_schema.body())?;
    ...
    let output_shape = snapshot_output
        .then(|| resolved_snapshot_output_shape(kernel, rows, right_columns))
        .transpose()?;
    let element_bytes = match coefficient_element.as_ref() {
        SchemaBody::FloatingPoint(mech_core::FloatWidth::W32) => core::mem::size_of::<f32>(),
        SchemaBody::FloatingPoint(mech_core::FloatWidth::W64) => core::mem::size_of::<f64>(),
        _ => return Err(ResidentKernelError::InvalidInput),
    };
    let output_count = rows
        .checked_mul(right_columns)
    ...
    let previous_footprint = match &output {
        ...
        ResidentValueMut::F64(values) if !snapshot_output && values.len() == output_count => None,
        _ => return Err(ResidentKernelError::InvalidOutput),
    };
    // Canonical primitive matrices retain one value, one matrix, and one
    // packed sequence regardless of their population. Per-element drafts are
    // temporary storage and are charged separately below.
    let output_nodes = if snapshot_output { 3 } else { 0 };
    let output_payload_bytes = output_count
        .checked_mul(element_bytes)
        .ok_or(ResidentKernelError::InvalidShape)?;
    let output_footprint = ValueFootprint {
        encoded_bytes: super::budget::checked_u64(output_payload_bytes)?,
        retained_bytes: super::budget::checked_u64(output_payload_bytes)?,
        node_count: output_nodes,
    };
    let publication_work = match (&output, previous_footprint, output_shape.as_ref()) {
        (ResidentValueMut::Snapshot(values), Some(previous_footprint), Some(output_shape)) => {
            let previous = values
                .first()
                .and_then(|value| value.as_ref())
                .ok_or(ResidentKernelError::InvalidOutput)?;
    ...
    if let Some(output_shape) = output_shape {
        ...
        Ok(replace_f64(output, |index| {
            let row = index % rows;
            let column = index / rows;
            let ValueDataDraft::F64(value) = values[row * right_columns + column] else {
                unreachable!("dense snapshot solve output is F64")
            };
            value.to_f64()
        }))
    }
...
mod tests {
    ...
    #[test]
    fn projected_fixed_snapshot_payload_uses_canonical_element_widths() {
        use mech_core::{FloatWidth, IntegerWidth};

        for (element, width) in [
            (SchemaBody::Bool, 1),
            (SchemaBody::UnsignedInteger(IntegerWidth::W8), 1),
            (SchemaBody::SignedInteger(IntegerWidth::W16), 2),
            (SchemaBody::FloatingPoint(FloatWidth::W32), 4),
            (SchemaBody::Index, 8),
            (SchemaBody::Complex(FloatWidth::W32), 8),
            (SchemaBody::UnsignedInteger(IntegerWidth::W128), 16),
            (SchemaBody::Rational64, 16),
        ] {
            assert_eq!(snapshot_fixed_element_encoded_bytes(&element), Some(width));
        }
        assert_eq!(
            snapshot_fixed_element_encoded_bytes(&SchemaBody::String),
            None
        );
    }
    ...
    #[test]
    fn snapshot_transpose_executes_index_and_string_matrices() {
        for (element, elements) in [
            (
                SchemaBody::Index,
                vec![ValueDataDraft::Index(1), ValueDataDraft::Index(2)],
            ),
            (
                SchemaBody::String,
                vec![
            ...
            let (schemas, ids) = test_schema_table([matrix(1, 2), matrix(2, 1)]);
            let [input_schema, output_schema] = ids.as_slice() else {
                unreachable!()
            };
            ...
            let kernel = bind_semantic_transpose(&ResidentKernelBindRequest {
                contract: &contract,
                schemas: &schemas,
                inputs: &[test_layout(
                    &schemas,
            ...
            .unwrap();
            let input = [Some(test_value(
                &schemas,
                *input_schema,
                ValueDataDraft::Matrix(elements.clone().into_boxed_slice()),
            ))];
            let mut output = [None];
            assert_eq!(
                kernel.execute(
                    &Inputs(&[ResidentValueRef::Snapshot(&input)]),
            ...
            assert!(
                output[0]
                    .as_ref()
                    .unwrap()
                    .language_eq(&schemas, &expected, &schemas)
                    .unwrap()
            );
        }
    }
    ...
    #[test]
    fn nullary_snapshot_matrix_constructor_publishes_the_declared_empty_shape() {
        ...
        let [matrix_schema] = ids.as_slice() else {
            unreachable!()
        };
        let contract = test_contract(
            &[],
        ...
        let kernel = bind_matrix_comprehension(&ResidentKernelBindRequest {
            contract: &contract,
            schemas: &schemas,
            inputs: &[],
            output: test_layout(
        ...
        let mut output = [None];
        kernel
            .execute(&Inputs(&[]), ResidentValueMut::Snapshot(&mut output))
            .unwrap();
        let output = output[0].as_ref().unwrap();
        assert_eq!(
            snapshot_matrix_dimensions(output, schemas.get(*matrix_schema).unwrap().body()),
            Ok((1, 0))
        );
        assert!(matches!(
            output.data(),
            ValueData::Matrix(matrix) if matrix.elements().is_empty()
        ));
    }
    ...
    #[test]
    fn dense_string_snapshot_materialization_charges_every_comparison_byte() {
        let (schemas, ids) = test_schema_table([SchemaBody::Matrix {
        ...
        let shape = schemas
            ...
            .instantiate_shape(Box::new([]))
            .unwrap();
        let values = ["x".repeat(40_000), "y".repeat(40_000)];
        let string_bytes = values.iter().map(String::len).sum::<usize>() as u64;
        let cost = resident_value_snapshot_materialization_cost(
            &schemas,
            schema,
            &shape,
            ResidentValueRef::String(&values),
        ...
        assert_eq!(cost.output_elements, 2);
        assert_eq!(cost.comparison_work, string_bytes + 2);
        assert_eq!(cost.compute_work, string_bytes + 4);
        assert_eq!(cost.retained_nodes, 5);
        assert!(cost.comparison_work > super::super::budget::MAX_RESIDENT_COMPARISON_WORK);
    }

    #[test]
    fn dense_snapshot_materialization_counts_canonical_sequence_nodes() {
        let (schemas, ids) = test_schema_table([
            SchemaBody::Matrix {
                element: Box::new(SchemaBody::FloatingPoint(mech_core::FloatWidth::W64)),
                dimensions: vec![
                    mech_core::DimensionExpr::Constant(1),
                    mech_core::DimensionExpr::Constant(65_536),
                ]
                .into_boxed_slice(),
            },
            SchemaBody::Matrix {
                ...
                dimensions: vec![
                    mech_core::DimensionExpr::Constant(1),
                    mech_core::DimensionExpr::Constant(65_534),
                ]
                .into_boxed_slice(),
            },
        ]);
        let numeric_shape = schemas
            .get(ids[0])
            .unwrap()
            .instantiate_shape(Box::new([]))
            .unwrap();
        let numeric = vec![0.0; 65_536];
        let numeric_cost = resident_value_snapshot_materialization_cost(
            &schemas,
            ids[0],
            &numeric_shape,
            ResidentValueRef::F64(&numeric),
        )
        .unwrap();
        assert_eq!(numeric_cost.retained_nodes, 3);

        let string_shape = schemas
            .get(ids[1])
            .unwrap()
            .instantiate_shape(Box::new([]))
            .unwrap();
        let strings = vec![String::new(); 65_534];
        let string_cost = resident_value_snapshot_materialization_cost(
            &schemas,
            ids[1],
            &string_shape,
            ResidentValueRef::String(&strings),
        )
        .unwrap();
        assert_eq!(string_cost.retained_nodes, 65_537);
    }
    ...
    #[test]
    fn indexed_assignment_capability_supports_maps_and_nested_matrix_members() {
        fn schema(body: SchemaBody) -> mech_core::Schema {
            mech_core::SchemaDraft {
                dimension_parameters: Box::new([]),
                body,
    ...
    #[test]
    fn snapshot_index_conversion_publishes_runtime_cardinality() {
        use mech_core::{
            DimensionExpr, DimensionLifetime, DimensionParameterDeclaration, DimensionParameterId,
            DimensionParameterOrigin, SchemaDraft,
        };
        ...
        let output_handle = builder
            ...
            .unwrap();
        let built = builder.finish().unwrap();
        let input_schema = built.resolve(input_handle).unwrap();
        let output_schema = built.resolve(output_handle).unwrap();
        let (schemas, _) = built.into_parts();
        let layout = |schema_id| mech_core::ResidentPortLayout {
            schema_id,
            schema_key: schemas.entry(schema_id).unwrap().key(),
            kind: ResidentValueKind::Snapshot,
            shape: ResidentShape::SCALAR,
        ...
        let kernel = bind_scalar_index(&ResidentKernelBindRequest {
            contract: &contract,
            schemas: &schemas,
            inputs: &[layout(input_schema)],
        ...
        let output = output[0].as_ref().unwrap();
        assert_eq!(
            snapshot_matrix_dimensions(output, schemas.get(output_schema).unwrap().body()),
            Ok((3, 1))
        );
        let ValueData::Matrix(matrix) = output.data() else {
            panic!("index conversion must publish a matrix")
        };
        let SequenceView::Index(values) = matrix.elements() else {
            panic!("index conversion must retain packed indices")
        };
    ...
    #[test]
    fn snapshot_access_rejects_compensating_runtime_axis_errors() {
        use mech_core::{
            DimensionExpr, DimensionLifetime, DimensionParameterDeclaration, DimensionParameterId,
            DimensionParameterOrigin, SchemaDraft,
        };
        ...
        let source_handle = builder
            ...
            .unwrap();
        let selector_handle = builder
            .insert(test_schema(matrix(SchemaBody::Bool, 3, 1)))
            .unwrap();
        let output_handle = builder
            .insert(test_schema(matrix(
            ...
            .unwrap();
        let built = builder.finish().unwrap();
        let source_schema = built.resolve(source_handle).unwrap();
        let selector_schema = built.resolve(selector_handle).unwrap();
        let output_schema = built.resolve(output_handle).unwrap();
        let (schemas, _) = built.into_parts();
        let source = ValueDraft {
            schema: source_schema,
            shape_values: vec![3, 2].into_boxed_slice(),
            data: ValueDataDraft::Matrix(
                (1..=6)
                    .map(ValueDataDraft::U64)
                    .collect::<Vec<_>>()
        ...
        let plan = SnapshotAccessPlan {
            ...
            matrix_constructor: None,
            selectors: Box::new([]),
            matrix_mode: Some(ResolvedSelectionMode::Rows),
            source_dimensions: None,
            // The element count agrees with the selected 2x2 result, but the
            // individual axes do not.
            output_dimensions: Some((1, 4)),
            output_dimensions_dynamic: false,
            output_geometry: ResolvedAccessGeometry {
                logical_output_rows: 1,
                logical_output_columns: 4,
            },
            aggregate_ordinal: None,
            output_schema,
        };
        let source_input = Some(source.clone());
        let selector_input = Some(selector);
        let inputs = [
            ResidentValueRef::Snapshot(core::slice::from_ref(&source_input)),
            ResidentValueRef::Snapshot(core::slice::from_ref(&selector_input)),
        ];
        assert!(matches!(
            snapshot_access_output_cost(
                &source,
                schemas.get(source_schema).unwrap().body(),
                &plan,
                &Inputs(&inputs),
                &mut super::super::budget::ResidentBudgetMeter::default(),
            ),
            Err(ResidentKernelError::InvalidShape),
        ));
    }
    ...
    #[test]
    fn snapshot_string_matrix_compares_with_dense_scalar_into_snapshot_output() {
        let matrix = |element| SchemaBody::Matrix {
            element: Box::new(element),
            dimensions: vec![
                mech_core::DimensionExpr::Constant(2),
        ...
        let (schemas, ids) = test_schema_table([
            matrix(SchemaBody::String),
            SchemaBody::String,
            matrix(SchemaBody::Bool),
        ]);
        let [input_schema, scalar_schema, output_schema] = ids.as_slice() else {
            unreachable!()
        };
        let contract = test_contract(
        ...
        let kernel = bind_semantic_equal(&ResidentKernelBindRequest {
            contract: &contract,
            schemas: &schemas,
            inputs: &[
                test_layout(
        ...
        let input = test_value(
            ...
            *input_schema,
            ValueDataDraft::Matrix(
                ["a", "b", "a", "c"]
                    .into_iter()
                    .map(|value| ValueDataDraft::String(value.into()))
                    .collect::<Vec<_>>()
                    .into_boxed_slice(),
            ),
        );
        let input = [Some(input)];
        let scalar = [String::from("a")];
        let inputs = [
            ResidentValueRef::Snapshot(&input),
            ResidentValueRef::String(&scalar),
        ];
        let mut output = [None];
        assert_eq!(
            kernel.execute(&Inputs(&inputs), ResidentValueMut::Snapshot(&mut output)),
        ...
        let ValueData::Matrix(output) = output[0].as_ref().unwrap().data() else {
            panic!("comparison must publish a Bool matrix")
        };
        let SequenceView::Bool(values) = output.elements() else {
            panic!("comparison must use packed Bool storage")
        };
    ...
    fn snapshot_math_and_dot_reject_staging_cost_before_output_mutation() {
        ...
        let snapshot_kernel = |executor, output_schema, parameters| {
        ...
        assert_eq!(
            snapshot_kernel(
                math_copysign_snapshot,
                *f32_matrix,
                vec![1, 1, 1, 65_537, 1, 65_537, 1].into_boxed_slice(),
            )
            .execute(&Inputs(&inputs), ResidentValueMut::Snapshot(&mut output),),
            Err(ResidentKernelError::InvalidShape),
        );
        ...
        let mut output = [None];
        assert_eq!(
            snapshot_kernel(matrix_dot_snapshot, *u64_scalar, Box::new([]))
                .execute(&Inputs(&inputs), ResidentValueMut::Snapshot(&mut output),),
            Err(ResidentKernelError::InvalidShape),
        );
        assert!(output[0].is_none());
    ...
    #[test]
    fn snapshot_dot_rejects_equal_cardinality_with_different_live_axes() {
        use mech_core::{
            DimensionExpr, DimensionLifetime, DimensionParameterDeclaration, DimensionParameterId,
            DimensionParameterOrigin, SchemaDraft,
        };
        ...
        let matrix_handle = builder
            ...
            .unwrap();
        let built = builder.finish().unwrap();
        let matrix_schema = built.resolve(matrix_handle).unwrap();
        let (schemas, _) = built.into_parts();
        let value = |shape_values: [u64; 2]| {
            ValueDraft {
                schema: matrix_schema,
                shape_values: shape_values.into(),
                data: ValueDataDraft::Matrix(
                    [1.0, 2.0, 3.0, 4.0]
                        .into_iter()
                        .map(F64Bits::from_f64)
        ...
        let left = [Some(value([2, 2]))];
        let right = [Some(value([1, 4]))];
        let inputs = [
            ResidentValueRef::Snapshot(&left),
            ResidentValueRef::Snapshot(&right),
        ];
        let kernel = BoundResidentKernel::new(matrix_dot_snapshot_f64, Box::new([]))
            .with_snapshot_schemas(schemas);
        let mut output = [99.0];
        assert_eq!(
            kernel.execute(&Inputs(&inputs), ResidentValueMut::F64(&mut output)),
