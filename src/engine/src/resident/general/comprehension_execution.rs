...
fn binding_schema_id(
    context: &SourceSchemaContext,
    owner_schemas: &SchemaTable,
    schema: SchemaId,
) -> Option<SchemaId> {
    let key = owner_schemas.entry(schema)?.key();
    indexed_schema_id(&context.binding_schema_index, key)
}

fn projected_binding_schema_id(
    context: Option<&SourceSchemaContext>,
    projection_schemas: &SchemaTable,
    schema: SchemaId,
) -> Option<SchemaId> {
    match context {
        Some(context) => binding_schema_id(context, projection_schemas, schema),
        None => Some(schema),
    }
}
...
fn source_schema_contexts(
    ...
    // Preserve plan IDs for annotated binding targets, then append every
    // runtime-owner projection by canonical key. Projection IDs remain local
    // to their owner context; binding IDs always refer to this merged arena.
    let binding_schemas = plan_schemas
        .extend_many_preserving_ids(projected.iter().map(|(_, schemas, _)| schemas))
        .ok()?;
...
impl SourceContextMaterializationBound {
    ...
    fn add_binding_base(&mut self, schemas: &SchemaTable) -> Result<(), ResidentKernelError> {
        let schema_bytes = schemas
            .clone_allocation_bound_bytes()
            .ok_or(ResidentKernelError::InvalidShape)?;
        let schema_nodes = budget::checked_u64(schemas.len())?;
        let index_bytes = schema_nodes
            .checked_mul(core::mem::size_of::<(SchemaKey, SchemaId)>() as u64)
            .ok_or(ResidentKernelError::InvalidShape)?;
        self.retained_bytes = self
        ...
        self.work = self
            .work
            .checked_add(schema_bytes)
            .and_then(|work| work.checked_add(index_bytes))
            .and_then(|work| work.checked_add(schema_nodes))
            .ok_or(ResidentKernelError::InvalidShape)?;
        self.merged_schema_nodes = self
            .merged_schema_nodes
            .checked_add(schema_nodes)
...
fn source_context_materialization_bound(
    ...
    bound.add_binding_base(plan_schemas)?;
        ...
        PatternItem::SourceComponent {
            ...
            if !matches!(body, SchemaBody::Dynamic) {
                if let Some(schema) = value_schema
                    && context.binding_schemas.get(schema).is_none()
                {
            ...
            value_schema = Some(
                binding_schema_id(&context, &context.schemas, schema)
                    .ok_or(ResidentKernelError::InvalidInput)?,
            );
        },
                ...
                .map(|data| {
                    adapt_resolved_pattern_item(
                        ResolvedPatternItem {
                            ...
                            value_schema: projection
                                .as_ref()
                                .map(|(schema, _)| {
                                    projected_binding_schema_id(
        ...
        fn selected(
            ...
            Some(
                if let (Some(context), Some(contexts)) =
                    (source_context.as_ref(), source_contexts)
                {
                    let value_schema = match projection_schema {
                        Some(schema) => Some(projected_binding_schema_id(
                            Some(context.as_ref()),
                            projection_schemas,
                            schema,
                        )?),
...
pub(super) fn resident_pattern_item(
    ...
    if let ResidentValueRef::Snapshot([Some(value)]) = value {
        ...
        let value_schema = binding_schema_id(&context, &context.schemas, source_schema)?;
        let body = context
            .schemas
            .get(source_schema)?
            .closed_body(value.shape())
...
fn pattern_item_tree_copy_count(
    source_backed: bool,
...
pub(super) fn admit_pattern_item_materialization(
    ...
    let finalization_count = dense_finalization_count;
    let _ = binding_count;
    let footprint = match value {
        ResidentValueRef::Snapshot([Some(value)]) => {
    ...
    // equality leaf can clone once more. Source-backed items retain a parallel
    // canonical data tree, so every live descent level owns both trees.
    let copies = pattern_item_tree_copy_count(
...
mod tests {
    ...
    #[test]
    fn source_backed_pattern_descent_accounts_for_draft_and_canonical_trees() {
        assert_eq!(pattern_item_tree_copy_count(false, 3).unwrap(), 4);
        assert_eq!(pattern_item_tree_copy_count(true, 3).unwrap(), 8);
        assert!(pattern_item_tree_copy_count(true, u64::MAX).is_err());
    }
    ...
    #[test]
    fn plan_only_structural_binding_targets_rebind_foreign_projected_children() {
        let mut foreign = SchemaTableBuilder::new();
        foreign
            .insert(
                SchemaDraft {
        ...
        let plan_matrix = plan
            ...
            .unwrap();
        let plan = plan.finish().unwrap();
        let plan_target = plan.resolve(plan_target).unwrap();
        let plan_matrix = plan.resolve(plan_matrix).unwrap();
        let (plan, projections) = structural_projection_schema_context(&plan.table).unwrap();
        let plan = Arc::new(plan);

        assert!(
            foreign
                .find_by_key(plan.entry(plan_target).unwrap().key())
                .is_none(),
            "the annotated target exists only in the plan arena",
        );
                ...
                vec![ValueDataDraft::Dynamic(Some(Box::new(ValueDraft {
                    ...
                    data: ValueDataDraft::Tuple(
                        vec![
                            ...
                            ValueDataDraft::F64(F64Bits::from_f64(5.0)),
                        ]
                        .into_boxed_slice(),
                    ),
                })))]
        ...
        .expect("foreign source materializes against the plan");
        let tuple = item
            .child(0, &plan, &projections)
            .expect("foreign Dynamic unwraps to its tuple owner");
        let binding = tuple
            .into_binding(plan_target, &[], &plan, &projections)
            .unwrap()
            .expect("plan-only annotated tuple accepts the foreign tuple");
        let value = finalize_pattern_binding(
            plan_target,
            &binding.shape_values,
            binding.data,
            binding.schemas,
        ...
        .expect("adapted tuple finalizes in the plan-rooted merged arena");
        assert_eq!(value.schema(), plan_target);
        let ValueData::Tuple(values) = value.data() else {
            panic!("plan target remains a tuple")
        };
        assert_eq!(values.len(), 2);
        for (value, expected) in values.iter().zip([3.0, 5.0]) {
            let ValueData::Dynamic(dynamic) = value else {
                panic!("annotated tuple child remains Dynamic")
            };
            let value = dynamic.value().expect("Dynamic child remains present");
            assert!(matches!(
                value.data(),
                ValueData::F64(actual) if actual.to_f64() == expected
            ));
            assert!(value.schemas().is_some());
        }
    }
