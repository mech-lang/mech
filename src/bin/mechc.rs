...
fn get_final_artifact_path() -> Option<String> {
    if let Some(data) = BUILD_DATA.get() {
        let data = data.lock().unwrap();
        data.final_artifact.as_ref().map(|p| {
            p.canonicalize()
...
impl BuildProcess {
    pub fn new(id: u64, name: String) -> Self {
        ...
        let status_style =
            ProgressStyle::with_template("{spinner:.yellow} [{elapsed_precise}] {msg}")
                .unwrap()
                .tick_strings(SAND);
    ...
    pub fn finish(&mut self) {
        ...
        let completed_style = ProgressStyle::with_template("{prefix:.yellow} {msg} {spinner}")
            .unwrap()
        ...
        final_state.finish_with_message(format!(
            "Artifact available at: {}",
            style(path).color256(75)
        ));
    }
    ...
    pub fn fail(&mut self) {
        ...
        self.build_progress.finish_and_clear();
        let failed_style = ProgressStyle::with_template("{prefix:.yellow} {msg} {spinner}")
            .unwrap()
    ...
    pub fn add_build_stage(&mut self, mut stage: BuildStage, steps: usize) {
        ...
        let header_style =
            ProgressStyle::with_template("{prefix:.yellow.dim} {msg:.dim} {spinner:.dim}")
                .unwrap()
                .tick_strings(&EMPTY);
        let header = self
            .indicators
            .insert_before(&self.build_progress, ProgressBar::new_spinner());
...
impl BuildStage {
    ...
    pub fn fail(&mut self) {
        ...
        self.header
            .finish_with_message(format!("{} {}", self.name, style("✗").red()));
    }

    pub fn cancel(&mut self) {
        self.end_time = Some(Instant::now());
        self.status = StepStatus::Failed;
        let cancel_style = ProgressStyle::with_template("{prefix:.yellow.dim} {msg:.dim}")
            .unwrap()
            .tick_chars(FAILEDSQUARESPINNER);
        self.header.set_style(cancel_style);
        self.header
            .finish_with_message(format!("{} {}", self.name, "✗"));
    }
}
...
pub fn main() -> anyhow::Result<()> {
  ...
  └─┘     └─┘ └──────┘ └──────┘ └─┘  └─┘"#
    ...
    if let Some(matches) = matches.subcommand_matches("clean") {
        let clean_paths: Vec<String> = matches
            .get_many::<String>("mech_clean_paths")
    ...
    let mut mech_paths: Vec<String> = matches
        .get_many::<String>("mech_paths")
        .map_or(vec![], |files| files.map(|file| file.to_string()).collect());
    let mut output_name = matches
        .get_one::<String>("output_name")
        .map(|s| s.to_string())
        .unwrap_or(
            mech_paths
                .get(0)
    ...
    if let Some(matches) = matches.subcommand_matches("build") {
        ...
        output_name = matches
            .get_one::<String>("build_output_name")
            .map(|s| s.to_string())
            .unwrap_or(
                mech_paths
                    .get(0)
    ...
    if mech_paths.is_empty() {
        ...
        println!(
            "{} No source files provided, using current directory: {}",
            style("✗").red(),
            current_dir.display()
        );
    } else {
    ...
    println!(
        r#"{} Building: {}"#,
        style(format!("[mech v{}]", VERSION).yellow()),
        start_message
    );
    {
...
fn prepare_build(stage: &mut BuildStage, tx: mpsc::Sender<Vec<PathBuf>>) {
    ...
    let exts = [
        "mec", "mpkg", "mecb", "mdoc", "mdb", "dll", "rlib", "m", "md", "🤖",
    ];
...
fn package_artifacts(stage: &mut BuildStage, release: bool) {
    ...
    match BUILD_DATA.get() {
        Some(data) => {
            let pairs = {
                let mut data = data.lock().unwrap();
                vec![("program".to_string(), data.bytecode.clone())]
            };
            ...
            let built_exe = match find_built_exe(&build_dir, &"mech_shim", None, release) {
                Ok(p) => {
                    pb.finish_with_message(format!(
                        "Read built shim executable ({} bytes)",
                ...
                Err(e) => {
                    pb.set_style(fail_style());
                    pb.finish_with_message(format!(
                        "Failed to find built shim executable: {:?} {}",
                        e,
                        style("✗").red()
                    ));
            ...
            let out_path = Path::new(BUILD_DIR)
                .join(mode)
            ...
            match write_final_exe(&built_exe, &zip_bytes, &out_path) {
                ...
                Err(e) => {
                    pb.set_style(fail_style());
                    pb.finish_with_message(format!(
                        "Failed to write final executable to {}: {} {}",
                        out_path
                            .canonicalize()
