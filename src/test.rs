...
use crate::cli::module_execution::{
    execute_source_module_roots_with_report, module_runtime_config,
};
...
fn is_bytecode_test_path(path: &Path) -> bool {
    matches!(
        mech_runtime::SourceKind::from_path(path),
        mech_runtime::SourceKind::MechBytecode
    )
}
...
fn integrity_reason(reason: Option<&IntegrityConstraintFailureReason>) -> String {
    match reason {
        None => "evaluated to true".to_string(),
        Some(IntegrityConstraintFailureReason::EvaluatedFalse) => "evaluated to false".to_string(),
...
fn integrity_evaluation_case(evaluation: &IntegrityConstraintEvaluation) -> CaseDetail {
    CaseDetail {
...
fn integrity_violation_case(violation: &IntegrityConstraintViolation) -> CaseDetail {
    CaseDetail {
...
fn case_to_mech(c: &CaseDetail) -> String {
    format!(
        "{{\n  name: {}\n  expression: {}\n  reason: {}\n  evaluated-kind: {}\n  actual: {}\n  expected: {}\n}}",
        mech_str(&c.name),
        mech_str(&c.expression),
        mech_str(&c.reason),
        mech_kind(&c.evaluated_kind),
        mech_str(&c.actual),
        mech_str(&c.expected)
    )
}
fn file_to_mech(file: &FileReport, verbose: bool) -> String {
    let failed_items = file
        .failed
        .iter()
        .map(case_to_mech)
        .collect::<Vec<_>>()
    ...
    format!(
        ...
        if failed_items.is_empty() {
            ...
            indent_block(&failed_items, 4)
        },
        if passed_items.is_empty() {
            "".to_string()
        } else {
            indent_block(&passed_items, 4)
        },
        run_error
    )
}
...
pub(crate) fn run_mech_tests_without_tree(
    ...
    for target in &expanded_targets {
        ...
        let execution =
            match execute_source_module_roots_with_report(config, &[target.path.clone()]) {
                Ok(execution) => execution,
                Err(err) => {
                    if let Some(failures) = err.kind_as::<IntegrityConstraintViolationSet>() {
                        let mut passed_cases = Vec::new();
                        let mut failed_cases = Vec::new();
                        if failures.evaluations.is_empty() {
                            failed_cases
                                .extend(failures.violations.iter().map(integrity_violation_case));
                        } else {
    ...
    if let Some(output_path) = output_path {
        ...
        match extension {
            ...
            _ => {
                eprintln!(
                    "{} Unsupported --out extension `.{}`. Use .json or .mec.",
                    "[Error]".truecolor(246, 98, 78),
                    extension
                );
                return Ok(1);
...
mod tests {
    ...
    fn contains_address_like_diagnostic(text: &str) -> bool {
        text.split("0x")
            .skip(1)
            .any(|suffix| suffix.chars().take_while(char::is_ascii_hexdigit).count() >= 6)
    }
    ...
    fn mech_tests_preserve_report_formats_for_imported_modules() {
        ...
        assert!(
            std::fs::read_to_string(json_output)
                .unwrap()
                .contains("files-total")
        );
        assert!(
            std::fs::read_to_string(mech_output)
                .unwrap()
                .contains("files-total")
        );
        std::fs::remove_dir_all(root).unwrap();
    ...
    fn test_explicit_mecb_input_is_still_collected_for_rejection() {
        let root = std::env::temp_dir().join(format!(
            "mech-test-bytecode-explicit-collect-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn test_directory_with_source_and_output_mecb_passes_collection() {
        let root = std::env::temp_dir().join(format!(
            "mech-test-bytecode-source-plus-output-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
