...
fn fragment_case(fragment: &str, kind: FragmentKind) {
    ...
    let source = TextSnapshot::new(DocumentId(77), Revision(9), complete.as_str()).unwrap();
    let start = prefix.len() as u32;
    let range = TextRange::new(TextSize(start), TextSize(start + fragment.len() as u32));
    ...
    assert!(
        snapshot.consumed_complete,
        "{kind:?} did not consume its range"
    );
...
fn fragment_diagnostic_uses_heading_right_context() {
    ...
    let full_diagnostics =
        normalize_diagnostics_in_range(&full.diagnostics, full.revision, &full.nodes, range);
...
fn fragment_nesting_limit_uses_actual_enclosing_depth() {
    ...
    let top_text = "x := (1)\n";
    let top_source = TextSnapshot::new(DocumentId(8), Revision(3), top_text).unwrap();
    ...
    validate_lossless_range(&top_fragment.root, &top_fragment.source, top_range).unwrap();
    assert_eq!(
    ...
    let nested_text = "x := ((1))\n";
    let nested_source = TextSnapshot::new(DocumentId(8), Revision(4), nested_text).unwrap();
    ...
    validate_lossless_range(&inner_fragment.root, &inner_fragment.source, inner_range).unwrap();
    let fragment_diagnostics = normalize_diagnostics_in_range(
