...
fn legacy_fixed_terminal_contracts() -> BTreeMap<String, LegacyFixedTerminalContract> {
    let source = fs::read_to_string(PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/base.rs"))
        .expect("read legacy base terminal declarations");
    let mut contracts = BTreeMap::new();
    for line in fixed_terminal_declarations(&source) {
        let line = line.trim();
        ...
        let fields =
            split_macro_arguments(invocation[open + 1..close].trim_end().trim_end_matches(','));
        assert_eq!(
            fields.len(),
            3,
            "malformed fixed-terminal declaration: {line}"
        );
...
fn assert_fixed_terminal(spec: &FixedTerminalSpec) {
    let (input, literal_start) = match spec.spacing {
        ...
        TerminalSpacing::Whitespace0Both => (format!(" \n{}\t\r", spec.literal), 2),
    };
    ...
    if spec.spacing == TerminalSpacing::Exact {
        assert_eq!(
            prefix.consumed,
            TextRange::new(TextSize::ZERO, TextSize(spec.literal.len() as u32),)
        );
    }
}
...
fn all_41_non_fixed_lexical_contracts_match_legacy_boundaries() {
    ...
    assert_legacy_boundary_parity(rules::ALPHA, mech_syntax::alpha, &["e\u{301}", "Δ", "1"]);
    assert_legacy_boundary_parity(rules::DIGIT, mech_syntax::digit, &["1\u{20e3}", "٣", "a"]);
    assert_legacy_boundary_parity(rules::ANY, mech_syntax::any, &["\r\n", "λ", ""]);
    assert_legacy_boundary_parity(rules::ANY_TOKEN, mech_syntax::any_token, &["\r\n", "λ", ""]);
    assert_legacy_boundary_parity(
        rules::FORBIDDEN_EMOJI,
    ...
    assert_legacy_boundary_parity(rules::RAW_TEXT, mech_syntax::raw_text, &["\\n", "(", "\n"]);
    assert_legacy_boundary_parity(rules::NEW_LINE, mech_syntax::new_line, &["\r\n", "\r", "x"]);
    assert_legacy_boundary_parity(
        rules::WHITESPACE,
    ...
    assert_legacy_boundary_parity(rules::LEFT_ANGLE, mech_syntax::left_angle, &["<", "⟨", "("]);
    assert_legacy_boundary_parity(
        rules::RIGHT_ANGLE,
    ...
    for input in ["λ", "λx", "x", "λ\u{301}"] {
        let canonical = parse_canonical_tag_for_test(source(input), "λ", ParseConfig::default());
        let graphemes = mech_syntax::graphemes::init_tag(input);
...
fn whitespace_classes_preserve_the_frozen_distinctions() {
    ...
    assert_eq!(
        parse("\r\n  x", rules::NEWLINE_INDENT).consumed.end,
        TextSize(4)
    );
}
...
fn grapheme_primitives_and_emoji_exclusions_are_exact() {
    ...
    let alpha =
        parse_canonical_base_rule_for_test(alpha, rules::ALPHA, ParseConfig::default()).unwrap();
    assert!(alpha.matched);
    assert_eq!(alpha.consumed.end, TextSize(3));

    let digit = source("1").append("\u{20e3}").unwrap();
    let digit =
        parse_canonical_base_rule_for_test(digit, rules::DIGIT, ParseConfig::default()).unwrap();
    ...
    for forbidden in ["┌", "\u{00a0}", "\u{2009}", "⸢", "⸥", "⟨", "⟩"] {
