...
#[test]
fn every_fixed_terminal_executes_its_canonical_contract() {
    assert_eq!(FIXED_TERMINAL_COUNT, 108);
    assert_eq!(FIXED_TERMINALS.len(), FIXED_TERMINAL_COUNT);

    for spec in FIXED_TERMINALS {
        assert!(
            canonical_rule_name(spec.rule).is_some(),
            "fixed terminal {:?} is not registered",
            spec.rule
        );
...
#[test]
fn identifiers_and_path_segments_keep_canonical_boundaries() {
