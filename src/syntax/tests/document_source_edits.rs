...
fn rejects_invalid_ranges_order_overlap_and_utf8_boundaries() {
    ...
    assert!(matches!(
        ascii.apply_edits(&[TextEdit::delete(range(5, 7)), TextEdit::delete(range(0, 1)),]),
        Err(SourceError::UnsortedEdits)
    ));
    assert!(matches!(
        ascii.apply_edits(&[TextEdit::delete(range(0, 5)), TextEdit::delete(range(4, 7)),]),
...
fn deterministic_random_edit_sequences_match_string_model() {
    ...
    for case in 0..128 {
        ...
        for _ in 0..32 {
            ...
            actual = actual
                .apply_edits(&[TextEdit::replace(range(start as u32, end as u32), insert)])
                .unwrap();
