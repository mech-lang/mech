...
pub fn null<'a, F, O>(mut parser: F) -> impl FnMut(ParseString<'a>) -> ParseResult<()>
where
    ...
    F: FnMut(ParseString<'a>) -> ParseResult<O>,
{
    move |input: ParseString| {
        ...
        match parser(input) {
            Err(Err::Error(mut e)) => {
                e.cause_range = SourceRange {
                    start,
                    end: e.cause_range.end,
                };
...
pub fn is<'a, F, O>(mut parser: F) -> impl FnMut(ParseString<'a>) -> ParseResult<O>
where
    F: FnMut(ParseString<'a>) -> ParseResult<O>,
{
    move |input: ParseString| {
...
/// For parser p, return the `!p` peek parsing expression.
pub fn is_not<'a, F, E>(mut parser: F) -> impl FnMut(ParseString<'a>) -> ParseResult<()>
where
    F: FnMut(ParseString<'a>) -> ParseResult<E>,
{
    move |input: ParseString| {
        let input_clone = input.clone();
        match parser(input_clone) {
            Err(Err::Failure(_)) | Err(Err::Error(_)) => Ok((input, ())),
            _ => Err(Err::Error(ParseError::new(input, "Unexpected character"))),
        }
    }
}
...
pub fn skip_till_eol(input: ParseString) -> ParseResult<Token> {
    let (input, matched) = many0(nom_tuple((is_not(new_line), any_token)))(input)?;
...
pub fn skip_till_end_of_statement(input: ParseString) -> ParseResult<Token> {
    ...
    let (input, matched) = many0(nom_tuple((
        ...
        is_not(alt((new_line, semicolon, mika_section_close))),
        any_token,
    )))(input)?;
...
pub fn skip_till_section_element(input: ParseString) -> ParseResult<Token> {
    ...
    let (input, matched2) = many0(nom_tuple((is_not(section_element), skip_past_eol)))(input)?;
...
pub fn skip_till_paragraph_element(input: ParseString) -> ParseResult<Token> {
    ...
    let (input, matched) = many0(nom_tuple((is_not(paragraph_element), any_token)))(input)?;
    let mut matched: Vec<Token> = matched.into_iter().map(|(_, t)| t).collect();
    let tkn = Token::merge_tokens(&mut matched).unwrap_or(Token::default());
    Ok((input, tkn))
}
    ...
    F: Fn(ParseString) -> ParseResult<Token>,
{
    ...
    Ok((
        input,
        T::error_placeholder(matched, SourceRange { start, end }),
    ))
}
...
pub fn mech_code_alt(input: ParseString) -> ParseResult<MechCode> {
    ...
    let parsers: Vec<(&str, Box<dyn Fn(ParseString) -> ParseResult<MechCode>>)> = vec![
        (
            "activation_scope",
            Box::new(|i| activation_scope(i).map(|(i, v)| (i, MechCode::ActivationScope(v)))),
        ),
        (
            "fsm_specification",
            Box::new(|i| fsm_specification(i).map(|(i, v)| (i, MechCode::FsmSpecification(v)))),
        ),
        (
            "fsm_implementation",
            Box::new(|i| fsm_implementation(i).map(|(i, v)| (i, MechCode::FsmImplementation(v)))),
        ),
        (
            "function_define",
            Box::new(|i| function_define(i).map(|(i, v)| (i, MechCode::FunctionDefine(v)))),
        ),
        (
            "module_import",
            Box::new(|i| module_import(i).map(|(i, v)| (i, MechCode::Import(v)))),
        ),
        (
            "statement",
            Box::new(|i| statement(i).map(|(i, v)| (i, MechCode::Statement(v)))),
        ),
        (
            "expression",
            Box::new(|i| expression(i).map(|(i, v)| (i, MechCode::Expression(v)))),
        ),
        (
            "comment",
            Box::new(|i| comment(i).map(|(i, v)| (i, MechCode::Comment(v)))),
        ),
    ];
...
pub fn code_terminal(input: ParseString) -> ParseResult<Option<Comment>> {
    ...
    let (input, _) = alt((
        null(new_line),
        null(semicolon),
        null(peek(right_brace)),
        null(eof),
        null(peek(mika_section_close)),
    ))(input)?;
    let (input, _) = whitespace0(input)?;
...
pub fn parse_grammar(text: &str) -> MResult<Grammar> {
    ...
    if error_log.is_empty() {
        ...
        let report: Vec<ParserErrorContext> = error_log
            .into_iter()
            .map(|e| ParserErrorContext {
                cause_rng: e.0,
...
pub fn parse(text: &str) -> MResult<Program> {
    ...
    if error_log.is_empty() {
        ...
        let report: Vec<ParserErrorContext> = error_log
            ...
            .collect();
        Err(MechError::new(ParserErrorReport(text.to_string(), report), None).with_compiler_loc())
    }
}
