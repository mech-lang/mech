...
define_syntax_kinds! {
  ...
  AlignmentSeparator => node,
  CenterAlignment => node,
  CheckList => node,
  CheckListItem => node,
  CheckedItem => node,
  Citation => node,
  ...
  EnumDefine => node,
  EnumVariant => node,
  EnumVariantInlineKind => node,
  EnumVariantKind => node,
  ErrorBlock => node,
  EvalInlineMechCode => node,
  FigureItem => node,
  Figures => node,
  FiguresRow => node,
  Float => node,
  ...
  Fsm => node,
  FsmArm => node,
  FsmBlockTransition => node,
  FsmCommentArm => node,
  FsmDeclare => node,
  FsmGuard => node,
  FsmGuardArm => node,
  FsmImplementation => node,
  FsmSpecification => node,
  FsmStateDefinition => node,
  FsmStateDefinitionVariables => node,
  FsmStatementTransition => node,
  FsmTransition => node,
  FunctionArg => node,
  FunctionDefine => node,
  FunctionDefineMatchArms => node,
  FunctionDefineStatements => node,
  FunctionMatchArm => node,
  FunctionOutArg => node,
  FunctionOutArgs => node,
  Highlight => node,
  Hyperlink => node,
  IdeaBlock => node,
  Img => node,
  ...
  MechdownList => node,
  MechdownTable => node,
  MechdownTableHeader => node,
  MechdownTableNoHeader => node,
  MechdownTableRow => node,
  MechdownTableWithHeader => node,
  MicroMika => node,
  Mika => node,
  MikaArmLeft => node,
  MikaArmRight => node,
  MikaExpressionInner => node,
  MikaEyeLeft => node,
  MikaEyeRight => node,
  MikaNose => node,
  MikaSection => node,
...
mod tests {
    ...
    fn phase_2a_kinds_are_append_only() {
        ...
        assert!(SyntaxKind::GrammarDocument as u16 > SyntaxKind::WarningSigil as u16);
