...
pub enum TreeInvariantError {
    RootLength { tree: TextSize, source: TextSize },
    SyntheticTokenHasWidth { token: TokenId, len: TextSize },
    TokenOutsideSource { token: TokenId, range: TextRange },
    TokenTextHash { token: TokenId, range: TextRange },
    NonTokenKind { token: TokenId, kind: SyntaxKind },
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::RootLength { tree, source } => {
                write!(
                    f,
                    "tree length {} differs from source length {}",
                    tree.0, source.0
                )
            }
