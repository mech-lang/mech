...
use crate::document::{
    ...
    IdGenerator, NodeFlags, NodeId, ParseConfig, ParseStats, RecoveryAction, Revision, SourceError,
    SyntaxKind, SyntaxSnapshot, TextEdit, TextRange, TextSize, TextSnapshot,
    normalize_diagnostics_in_range,
};

use super::change_map::{Affinity, ChangeMap};
use super::restart::{ReparseRoot, parent_supported_root, select_reparse_root};
...
impl ParserWork {
    fn add_stats(&mut self, stats: ParseStats) {
        self.parser_steps = self.parser_steps.saturating_add(stats.parser_steps);
        self.events_emitted = self.events_emitted.saturating_add(stats.events_emitted);
        self.recovery_bytes = self.recovery_bytes.saturating_add(stats.recovery_bytes);
    }

    fn add_work(&mut self, work: Self) {
        self.parser_steps = self.parser_steps.saturating_add(work.parser_steps);
        self.events_emitted = self.events_emitted.saturating_add(work.events_emitted);
        self.recovery_bytes = self.recovery_bytes.saturating_add(work.recovery_bytes);
    }
}
...
pub(crate) fn reparse(
    ...
    loop {
        ...
        let attempt = try_reparse_root(old, &source, &changes, root, config, ids);
...
impl RootAttempt {
    fn rejected(fragment_stats: ParseStats, validation_stats: ParseStats) -> Self {
        Self {
            result: None,
            fragment_stats,
            validation_stats,
...
fn try_reparse_root(
    ...
    if root.kind == SyntaxKind::Section && root.range.len().0 > 0 && replacement.text_len.0 == 0 {
        return RootAttempt::rejected(fragment_stats, validation_stats);
    }
    if root.kind == SyntaxKind::Section {
        ...
        if root.range.end < old.source.byte_len()
            && !Cursor::for_range(source, TextRange::new(mapped.end, source.byte_len()))
                .is_line_start()
        {
            return RootAttempt::rejected(fragment_stats, validation_stats);
        }
    ...
    let Some((new_root, _)) = splice_node(&old.root, root.node, replacement, ids) else {
        return RootAttempt::rejected(fragment_stats, validation_stats);
    };
    ...
    snapshot.diagnostics = merge_diagnostics(old, &fragment, &snapshot, changes, root.range);
    if snapshot.diagnostics.len() > config.limits.max_diagnostics as usize {
        return RootAttempt::rejected(fragment_stats, validation_stats);
    }
    if root.kind != SyntaxKind::Section {
        let validation =
            validate_section_envelope(old, source, changes, root, &snapshot, config, ids);
...
fn fragment_context(
    ...
    let enclosing_fence = (kind == FragmentKind::CodeBlock)
        .then(|| {
            ...
            crate::document::parser::mechdown::fence_delimiter(&cursor).map(|start| start.delimiter)
        })
        .flatten();
    ParseContext {
        mode,
...
fn finish_result(
    ...
    let reused = old_ids.intersection(&new_ids).copied().collect::<Vec<_>>();
    ...
    result.snapshot.stats.source_bytes = u64::from(result.snapshot.source.byte_len().0);
    result.snapshot.stats.diagnostics_emitted = result.snapshot.diagnostics.len() as u64;
...
fn finish_full_fallback(
    ...
    let mut finished = finish_result(old, result, root, changed, attempted, work);
...
fn find_node_by_id(node: &Arc<GreenNode>, id: NodeId) -> Option<&Arc<GreenNode>> {
    if node.id == id {
        return Some(node);
    }
...
fn has_direct_child(node: &GreenNode, kind: SyntaxKind) -> bool {
    node.children
        .iter()
        .any(|child| matches!(child, GreenElement::Node(child) if child.kind == kind))
}
...
fn validate_section_envelope(
    ...
    let Some(section) = ancestor_record(old, root.node, SyntaxKind::Section) else {
    ...
    let expected = parse_fragment(source, mapped, FragmentKind::Section, context, config, ids);
...
fn find_node_at_range(
    ...
    for child in node.children.iter() {
        if let GreenElement::Node(child) = child {
            if let Some(found) = find_node_at_range(child, kind, target, offset) {
                return Some(found);
            }
        }
...
fn merge_diagnostics(
    ...
    for diagnostic in old.diagnostics.iter().cloned() {
        if absolute_primary_belongs_to_replaced(&diagnostic, old, replaced_old_range) {
            continue;
        }
        let diagnostic = map_old_diagnostic(diagnostic, changes, new_snapshot.revision);
        if diagnostic
            .primary
...
fn map_old_anchor(anchor: &mut DiagnosticAnchor, changes: &ChangeMap, revision: Revision) {
