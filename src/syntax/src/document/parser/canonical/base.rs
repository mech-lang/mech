...
pub(crate) fn parse_rule(parser: &mut Parser<'_>, rule: RuleId) -> bool {
    ...
    let matched = parser.with_canonical_rule(rule, |parser| parse_rule_inner(parser, rule));
    if !matched {
...
fn parse_rule_inner(parser: &mut Parser<'_>, rule: RuleId) -> bool {
    ...
    match rule {
        ...
        rules::EMOJI_GRAPHEME => consume_classified_grapheme(parser, SyntaxKind::Emoji, is_emoji),
        rules::ALPHA => consume_classified_grapheme(parser, SyntaxKind::Alpha, char::is_alphabetic),
        rules::DIGIT => consume_classified_grapheme(parser, SyntaxKind::Digit, char::is_numeric),
        rules::ANY => consume_any_grapheme(parser),
        rules::ANY_TOKEN => parse_rule(parser, rules::ANY),
        rules::FORBIDDEN_EMOJI => parse_choice(parser, FORBIDDEN_EMOJI_RULES),
        ...
        rules::ALPHANUMERIC => parse_choice(parser, &[rules::ALPHA_TOKEN, rules::DIGIT_TOKEN]),
        rules::UNDERSCORE_DIGIT => {
            parse_rule(parser, rules::UNDERSCORE) && parse_rule(parser, rules::DIGIT_TOKEN)
        }
        rules::DIGIT_SEQUENCE => parse_digit_sequence(parser),
        rules::GROUPING_SYMBOL => parse_choice(parser, GROUPING_SYMBOL_RULES),
        ...
        rules::IDENTIFIER_SYMBOL => parse_choice(parser, IDENTIFIER_SYMBOL_RULES),
        ...
        rules::WHITESPACE => parse_choice(parser, &[rules::SPACE, rules::TAB, rules::NEW_LINE]),
        rules::WHITESPACE0 => {
            parse_zero_or_more(parser, rules::WHITESPACE);
            true
        }
        rules::WHITESPACE1 => parse_one_or_more(parser, rules::WHITESPACE),
        rules::NEWLINE_INDENT => {
        ...
        rules::WS1E | rules::SPACE_TAB1 => parse_one_or_more(parser, rules::SPACE_TAB),
        ...
        rules::IDENTIFIER_PATH_SEGMENT_EMOJI => parse_identifier_path_segment_emoji(parser),
        rules::IDENTIFIER_PATH_SEGMENT => parse_identifier_path_segment(parser),
        rules::LEFT_ANGLE => parse_choice(parser, &[rules::LEFT_ANGLE1, rules::LEFT_ANGLE2]),
        rules::RIGHT_ANGLE => parse_choice(parser, &[rules::RIGHT_ANGLE1, rules::RIGHT_ANGLE2]),
        rules::BOX_DRAWING_CHAR => parse_choice(parser, BOX_DRAWING_CHAR_RULES),
        rules::BOX_DRAWING_EMOJI => parse_choice(parser, BOX_DRAWING_EMOJI_RULES),
...
fn parse_fixed_terminal(parser: &mut Parser<'_>, spec: &FixedTerminalSpec) -> bool {
    if spec.spacing == TerminalSpacing::Whitespace0Both && !parse_rule(parser, rules::WHITESPACE0) {
        return false;
    }
    if !consume_exact_token(parser, spec.literal, spec.kind) {
        return false;
    }
    spec.spacing != TerminalSpacing::Whitespace0Both || parse_rule(parser, rules::WHITESPACE0)
}

fn consume_exact_token(parser: &mut Parser<'_>, literal: &str, kind: SyntaxKind) -> bool {
...
fn matches_without_consuming(parser: &mut Parser<'_>, rule: RuleId) -> bool {
    let checkpoint = parser.checkpoint();
...
fn parse_digit_sequence(parser: &mut Parser<'_>) -> bool {
    ...
    loop {
        let before = parser.offset();
        if !parse_choice(parser, &[rules::UNDERSCORE_DIGIT, rules::DIGIT_TOKEN]) {
            break;
        }
        if parser.offset() == before || parser.is_halted() {
            break;
...
fn parse_escaped_char(parser: &mut Parser<'_>) -> bool {
    let marker = parser.start();
    if !parse_rule(parser, rules::BACKSLASH) || !consume_escaped_value(parser) {
        return false;
    }
...
fn consume_escaped_value(parser: &mut Parser<'_>) -> bool {
    // Integration accepts zero and Unicode scalar escapes before the original
    // one-grapheme escape. Failed Unicode candidates fall back to `\\u`.
    if parser.cursor().grapheme_literal_end("0").is_some() {
        return parser
            .bump_bytes_token(1, SyntaxKind::EscapedChar)
            .is_some();
    }
    if parser.cursor().grapheme_literal_end("u{").is_some() {
        let checkpoint = parser.checkpoint();
        let start = parser.offset();
        if parser.bump_grapheme_raw().is_none() || parser.bump_grapheme_raw().is_none() {
            return false;
        }
        let digits_start = parser.offset();
        while parser
            .cursor()
            .peek_char()
            .is_some_and(|ch| ch.is_ascii_hexdigit())
        {
            if parser.bump_grapheme_raw().is_none() {
                return false;
            }
        }
        let digits = crate::document::TextRange::new(digits_start, parser.offset());
        let scalar = parser
            .source()
            .text(digits)
            .ok()
        ...
        if scalar.is_some() && parser.cursor().grapheme_literal_end("}").is_some() {
            if parser.bump_grapheme_raw().is_none() {
                return false;
            }
            parser.token(
                SyntaxKind::EscapedChar,
                crate::document::TextRange::new(start, parser.offset()),
            );
            return true;
        }
        parser.rewind(checkpoint);
...
fn parse_identifier_path_segment_emoji(parser: &mut Parser<'_>) -> bool {
