...
use super::recovery::{Attempt, RecoveryClass, abandon_error, insert_missing, nesting_limit};
use super::terminal::{
    is_horizontal_space, is_identifier_continue, is_identifier_start, is_newline_start,
};
use super::{Cursor, Parser, canonical_rule_id, parser_context_id};

pub(crate) fn parse_mech_item(parser: &mut Parser<'_>) -> Attempt<()> {
    if is_variable_definition_start(parser.cursor()) {
        return parser.with_rule(parser_context_id("prototype-mech-item"), None, |parser| {
...
fn parse_comment_item(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-comment"), None, |parser| {
        let item = parser.start();
        let _ = parser.consume_horizontal_space();
...
pub(crate) fn parse_expression(parser: &mut Parser<'_>) -> Attempt<()> {
    ...
    parser.with_rule(parser_context_id("prototype-expression"), None, |parser| {
...
fn parse_additive(parser: &mut Parser<'_>) -> Attempt<()> {
    let checkpoint = parser.checkpoint();
    parser.with_rule(parser_context_id("prototype-l3"), None, |parser| {
...
fn parse_factor(parser: &mut Parser<'_>) -> Attempt<()> {
    ...
    if parser.cursor().peek_char().is_some_and(is_identifier_start) {
        parse_identifier(parser);
        return Attempt::Match(());
    }
    Attempt::NoMatch
...
fn parse_integer(parser: &mut Parser<'_>) {
    parser.with_rule(
        ...
        |parser| {
            ...
            for suffix in [
                "u128", "i128", "u64", "i64", "u32", "i32", "f64", "f32", "u16", "i16", "u8", "i8",
            ] {
...
fn parse_identifier(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-identifier"), None, |parser| {
...
fn parse_kind_annotation(parser: &mut Parser<'_>) {
    ...
    parser.with_rule(
        ...
        |parser| {
            ...
            let mut depth = 1_u32;
            while !parser.is_eof() && !parser.is_halted() && !is_newline_start(parser.cursor()) {
                if parser.cursor().starts_with("<") {
...
fn missing_expression(parser: &mut Parser<'_>, operator: Option<TextRange>) {
    if operator.is_none() {
        parser.with_rule(parser_context_id("prototype-expression"), None, |parser| {
            emit_missing_expression(parser, None)
        });
    } else {
