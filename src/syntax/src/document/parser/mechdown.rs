...
use super::terminal::{is_horizontal_space, is_newline_start, token_kind_for_char};
...
pub(crate) fn fence_delimiter_context(view: ContextView<'_>) -> Option<FenceStart> {
...
pub(crate) fn parse_ul_subtitle(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-ul-subtitle"), None, |parser| {
        let heading = parser.start();
        consume_line_tokens(parser);
        consume_line_tokens(parser);
        heading.complete_with_flags(parser, SyntaxKind::UlSubtitle, NodeFlags::REPARSE_ROOT);
    });
}

pub(crate) fn parse_subtitle(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-subtitle"), None, |parser| {
        let heading = parser.start();
        consume_line_tokens(parser);
        heading.complete_with_flags(parser, SyntaxKind::Subtitle, NodeFlags::REPARSE_ROOT);
    });
}

pub(crate) fn parse_generic_fence(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-code-block"), None, |parser| {
        ...
        fence.complete_with_flags(parser, SyntaxKind::GenericFence, NodeFlags::REPARSE_ROOT);
    });
}

pub(crate) fn parse_paragraph(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-paragraph"), None, |parser| {
        ...
        while !parser.is_eof() && !is_newline_start(parser.cursor()) && !parser.is_halted() {
            ...
            if parser.cursor().peek_char().is_some_and(is_horizontal_space) {
                let _ = parser.consume_horizontal_space();
                continue;
            }
            if parser.cursor().peek_char().is_some_and(|character| {
                matches!(character, '`' | '[' | ']' | '{' | '}' | '*' | '_' | '~')
            }) {
                let _ = skip_error(
                    parser,
...
fn consume_subtitle_segment(cursor: &mut Cursor<'_>) -> bool {
    ...
    while cursor.peek_char().is_some_and(|character| {
        (alphabetic && character.is_alphabetic()) || (numeric && character.is_numeric())
    }) {
        consumed = true;
        let _ = cursor.bump_char();
    }
...
fn consume_horizontal_context(view: ContextView<'_>, mut relative: u32) -> u32 {
    while let Some(character) = view
        .at_relative(relative)
        .and_then(ContextView::peek_char)
...
fn consume_until_newline_context(view: ContextView<'_>, mut relative: u32) -> u32 {
    while let Some(character) = view.at_relative(relative).and_then(ContextView::peek_char) {
...
fn consume_newline_context(view: ContextView<'_>, relative: u32) -> Option<u32> {
    let lookahead = view.at_relative(relative)?;
    if lookahead.starts_with("\r\n") {
