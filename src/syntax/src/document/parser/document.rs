...
pub(crate) fn parse_document_root(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("document"), None, |parser| {
...
pub(crate) fn parse_section(parser: &mut Parser<'_>) {
    parser.with_rule(parser_context_id("prototype-section"), None, |parser| {
        ...
        section.complete_with_flags(parser, SyntaxKind::Section, NodeFlags::REPARSE_ROOT);
    });
}
