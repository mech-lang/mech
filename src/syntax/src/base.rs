...
leaf!(info_sigil, "(i)>", TokenKind::InfoSigil);
leaf!(idea_sigil, "(*)>", TokenKind::IdeaSigil);
leaf!(warning_sigil, "(!)>", TokenKind::WarningSigil);
leaf!(error_sigil, "(x)>", TokenKind::ErrorSigil);
leaf!(error_alt_sigil, "(✗)>", TokenKind::ErrorSigil);
leaf!(success_check_sigil, "(✓)>", TokenKind::SuccessSigil);
leaf!(success_sigil, "(+)>", TokenKind::SuccessSigil);
...
leaf!(import_sigil, "+>", TokenKind::ModuleImportSigil);
...
ws0_leaf!(
    ...
    "~>",
    TokenKind::AsyncTransitionOperator
);
...
pub fn emoji(input: ParseString) -> ParseResult<Token> {
    ...
    Ok((
        ...
        Token {
            ...
            chars: g.chars().collect::<Vec<char>>(),
            src_range,
        },
    ))
}
...
pub fn symbol(input: ParseString) -> ParseResult<Token> {
    let (input, symbol) = alt((
        ampersand, grave, dollar, bar, percent, at, slash, hashtag, equal, backslash, tilde, plus,
        dash, asterisk, caret, underscore,
    ))(input)?;
    Ok((input, symbol))
}
...
pub fn identifier_symbol(input: ParseString) -> ParseResult<Token> {
    let (input, symbol) = alt((
        ampersand, dollar, percent, slash, hashtag, backslash, tilde, plus, dash, asterisk, caret,
    ))(input)?;
    Ok((input, symbol))
}
...
pub fn identifier(input: ParseString) -> ParseResult<Identifier> {
    let (input, (first, mut rest)) = nom_tuple((
        alt((alpha_token, emoji)),
        many0(alt((alpha_token, digit_token, identifier_symbol, emoji))),
    ))(input)?;
