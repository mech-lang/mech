        ...
        Ok((remaining, paragraph)) => {
            ...
            if new_line(remaining.clone()).is_ok() {
                ...
                Ok((
                    recovered_input,
                    Comment {
                        paragraph: Paragraph::from_tokens(vec![line_token]),
                    },
                ))
            }
        }
...
pub fn context_send(input: ParseString) -> ParseResult<ContextSend> {
    ...
    if target.context.is_none() {
        return Err(nom::Err::Error(ParseError::new(
            input,
            "send target must be an addressed context path",
        )));
    }
    if target.kind.is_some() {
        return Err(nom::Err::Error(ParseError::new(
            input,
            "send targets cannot have kind annotations",
        )));
    }
...
pub fn variable_define(input: ParseString) -> ParseResult<VariableDefine> {
    ...
    if var.context.is_some() {
        return Err(nom::Err::Error(ParseError::new(
            input,
            "addressed context paths cannot be defined with :=",
        )));
    }
...
fn source_import_tail(input: ParseString) -> ParseResult<Token> {
    let start = input.loc();
    let (input, matched) =
        many1(nom_tuple((is_not(alt((new_line, semicolon))), any_token)))(input)?;
...
fn source_mec_path(input: ParseString) -> ParseResult<Vec<Token>> {
    ...
    let text = Token::merge_tokens(&mut tokens.clone())
...
fn source_mec_path_wildcard_suffix(input: ParseString) -> ParseResult<Vec<Token>> {
    ...
    Ok((
        input,
        suffix
            .map(|(slash, asterisk)| vec![slash, asterisk])
            .unwrap_or_default(),
    ))
}
...
fn relative_source_import_specifier(input: ParseString) -> ParseResult<MechString> {
    ...
    let (input, prefix) = alt((
        nom_map(nom_tuple((period, period, slash)), |(a, b, c)| {
            vec![a, b, c]
        }),
...
fn uri_source_import_specifier(input: ParseString) -> ParseResult<MechString> {
    ...
    tokens.push(Token::new(
        TokenKind::Any,
        SourceRange {
            start: uri_marker_start,
            end: uri_marker_end,
        },
        uri_marker.chars().collect(),
    ));
...
pub fn import_declaration(input: ParseString) -> ParseResult<ImportDeclaration> {
    ...
    if !source_wildcard_specifier_is_valid(&specifier.to_string()) {
        return Err(nom::Err::Failure(ParseError::new(
            input,
            "Invalid wildcard placement in import specifier",
        )));
    }
...
pub fn statement(input: ParseString) -> ParseResult<Statement> {
    let parsers: Vec<(
        &'static str,
        Box<dyn Fn(ParseString) -> ParseResult<Statement>>,
    )> = vec![
        (
            "import_declaration",
            Box::new(|i| import_declaration(i).map(|(i, v)| (i, Statement::ImportDeclaration(v)))),
        ),
        (
            "export_declaration",
            Box::new(|i| export_declaration(i).map(|(i, v)| (i, Statement::ExportDeclaration(v)))),
        ),
        (
            "context_declaration",
            Box::new(|i| {
                context_declaration(i).map(|(i, v)| (i, Statement::ContextDeclaration(v)))
            }),
        ),
        (
            "fsm_declare",
            Box::new(|i| fsm_declare(i).map(|(i, v)| (i, Statement::FsmDeclare(v)))),
        ),
        #[cfg(feature = "invariant_define")]
        (
            "invariant_define",
            Box::new(|i| invariant_define(i).map(|(i, v)| (i, Statement::InvariantDefine(v)))),
        ),
        (
            "context_send",
            Box::new(|i| context_send(i).map(|(i, v)| (i, Statement::ContextSend(v)))),
        ),
        (
            "variable_define",
            Box::new(|i| variable_define(i).map(|(i, v)| (i, Statement::VariableDefine(v)))),
        ),
        (
            "variable_assign",
            Box::new(|i| variable_assign(i).map(|(i, v)| (i, Statement::VariableAssign(v)))),
        ),
        (
            "op_assign",
            Box::new(|i| op_assign(i).map(|(i, v)| (i, Statement::OpAssign(v)))),
        ),
        (
            "enum_define",
            Box::new(|i| enum_define(i).map(|(i, v)| (i, Statement::EnumDefine(v)))),
        ),
        (
            "tuple_destructure",
            Box::new(|i| tuple_destructure(i).map(|(i, v)| (i, Statement::TupleDestructure(v)))),
        ),
        (
            "kind_define",
            Box::new(|i| kind_define(i).map(|(i, v)| (i, Statement::KindDefine(v)))),
        ),
    ];
