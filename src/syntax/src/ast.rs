...
impl Ast {
  ...
  pub fn build_syntax_tree(&mut self, node: &ParserNode) -> Vec<AstNode> {
    ...
    match node {
      ...
      ParserNode::Section{children, title} => {
        let result = self.compile_nodes(children);
        let (title, level): (Option<Vec<char>>, usize) = match title {
          Some(title) => {
            let result = self.compile_nodes(title);
            if result.len() == 1 {
              match &result[0] {
                AstNode::Title{text, level, ..} => (Some(text.clone()),*level), 
                _ => (None, 1),
              } 
            }else {
              (None,1)
        ...
        compiled.push(AstNode::Section{title, level, children: result.clone()});
      },
      ...
      ParserNode::Title{children} => {
        ...
        let node = match &result[0] {
          AstNode::String{text, src_range} => AstNode::Title{text: text.clone(), src_range: *src_range, level: 0},
          _ => AstNode::Null,
        };
        compiled.push(node);
      },
      ParserNode::Subtitle{children, level} => {
        let result = self.compile_nodes(children);
        let node = match &result[0] {
          AstNode::String{text, src_range} => AstNode::Title{text: text.clone(), src_range: *src_range, level: *level},
          _ => AstNode::Null,
        };
        compiled.push(node);
      },
