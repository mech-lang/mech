...
impl Compiler {
  ...
  pub fn compile_sections(&mut self, nodes: &Vec<AstNode>) -> Result<Vec<Vec<SectionElement>>,MechError> {
    ...
    if sections.len() > 0 {
      ...
      Err(MechError{tokens: vec![], msg: "".to_string(), id: 3749, kind: MechErrorKind::None})
    }
  }
  ...
  pub fn compile_node(&mut self, node: &AstNode) -> Result<Vec<Transformation>,MechError> {
    ...
    match node {
      ...
      AstNode::FollowedBy{children} => {
        let mut out = self.compile_node(&children[0])?;
        let mut initial = self.compile_node(&children[1])?;
        let mut subsequent = self.compile_node(&children[2])?;
        match (&out[0],&initial[0],&subsequent[0]) {
          (Transformation::NewTable{table_id: out, ..},
           Transformation::NewTable{table_id: init, ..},
           Transformation::NewTable{table_id: sub, ..},
          ) => {
            tfms.push(Transformation::FollowedBy{
              table_id: *out,
              initial: *init,
              subsequent: *sub,
            });
        ...
        tfms.append(&mut out);
        tfms.append(&mut initial);
        tfms.append(&mut subsequent);
      }
...
pub fn compile_text(node: &ParserNode) -> Result<String,MechError> {
  ...
  match &ast.build_syntax_tree(node)[0] {
    ...
    x => Err(MechError{tokens: vec![], msg: "".to_string(), id: 7392, kind: MechErrorKind::GenericError(format!("Unhandled Node: {:?}", x))})
  }
}