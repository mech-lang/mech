...
pub fn complex_number(input: ParseString) -> ParseResult<C64Node> {
    ...
    if let Ok((input, (sign, imaginary_num, _))) = nom_tuple((
        alt((plus, dash)),
        untyped_real_number,
        alt((tag("i"), tag("j"))),
    ))(input.clone())
...
pub fn scientific_literal(input: ParseString) -> ParseResult<RealNumber> {
    let (input, base) = match float_literal(input.clone()) {
        ...
        _ => match integer_literal(input.clone()) {
            Ok((input, RealNumber::TypedInteger((base, _)))) => (input, (base, Token::default())),
            Ok((input, RealNumber::Integer(base))) => (input, (base, Token::default())),
            Err(err) => {
                return Err(err);
            }
            _ => unreachable!(),
    ...
    let (input, (ex_whole, ex_part)) = match float_literal(input.clone()) {
        Ok((input, RealNumber::Float(exponent))) => (input, exponent),
        _ => match integer_literal(input.clone()) {
            Ok((input, RealNumber::Integer(exponent))) => (input, (exponent, Token::default())),
            Err(err) => {
                return Err(err);
            }
            _ => unreachable!(),
    ...
    Ok((
        input,
        RealNumber::Scientific((base, (ex_sign, ex_whole, ex_part))),
    ))
}
...
pub fn hexadecimal_literal(input: ParseString) -> ParseResult<RealNumber> {
    ...
    let (input, mut tokens) =
        label!(many1(alt((digit_token, underscore, alpha_token))), msg)(input)?;
    let mut merged = Token::merge_tokens(&mut tokens).unwrap();
    merged.kind = TokenKind::Number;
...
pub fn kind_set(input: ParseString) -> ParseResult<Kind> {
    ...
    let ltrl = match opt_lit {
        ...
        None => match notation {
            Some(_) => Some(Box::new(Literal::Empty(Token::new(
                TokenKind::Empty,
                SourceRange::default(),
                vec!['N'],
            )))),
...
mod tests {
    ...
    fn kind_annotation_parses_multiline_record_fields() {
        ...
        let (_, annotation) =
            kind_annotation(input).expect("expected multiline record kind annotation to parse");
    ...
    fn kind_annotation_parses_multiline_record_with_nested_kinds() {
        ...
        let (_, annotation) = kind_annotation(input)
            .expect("expected nested multiline record kind annotation to parse");
