    ...
    if let Ok((input, _)) = left_brace(input.clone()) {
        ...
        if let Ok((input, _)) = right_brace(input.clone()) {
            Ok((
                ...
                ActivationArm {
                    pattern,
                    guard,
                    body: ActivationArmBody::Block(Vec::new()),
                },
            ))
        } else {
            let (input, parsed) = mech_code(input)?;
            ...
            Ok((
                ...
                ActivationArm {
                    pattern,
                    guard,
                    body: ActivationArmBody::Block(parsed.code),
                },
            ))
        }
    } else {
        ...
        Ok((
            ...
            ActivationArm {
                pattern,
                guard,
                body: ActivationArmBody::Expression(expression),
            },
        ))
    }
}
...
mod tests {
    use super::*;
    #[test]
    fn activation_scope_parses_fixed_block_body() {
        assert!(parse("~> tick {\n output := x + 1\n}").is_ok());
    }
    #[test]
    fn activation_scope_parses_pattern_block_and_expression_arms() {
        assert!(parse("~> event\n | :pressed(x) => {\n output := x\n }\n | * => 0.").is_ok());
    }
    #[test]
    fn activation_scope_parses_pattern_guard() {
        assert!(
            parse("~> event\n | (x, y), x > y => { output := x }\n | * => { output := 0 }").is_ok()
        );
    }
    #[test]
    fn activation_scope_parses_empty_fixed_and_pattern_blocks() {
    ...
    #[test]
    fn activation_scope_pattern_body_does_not_parse_as_match_expression() {
        let p = parse("~> event | * => 0.").unwrap();
        let SectionElement::MechCode(code) = &p.body.sections[0].elements[0] else {
            panic!("expected Mech code")
        };
        assert!(matches!(
            &code[0].0,
            MechCode::ActivationScope(ActivationScope {
                body: ActivationBody::PatternArms(_),
                ..
            })
        ));
    }
