...
pub fn l1(input: ParseString) -> ParseResult<Factor> {
    ...
    let factor = if rhs.is_empty() {
        lhs
    } else {
        Factor::Term(Box::new(Term { lhs, rhs }))
    };
    Ok((input, factor))
}
...
pub fn factor(input: ParseString) -> ParseResult<Factor> {
    let (input, fctr) = if let Ok((input, fctr)) = parenthetical_term(input.clone()) {
        ...
        (
            input,
            Factor::Expression(Box::new(Expression::MatrixComprehension(Box::new(m)))),
        )
    } else if let Ok((input, s)) = structure(input.clone()) {
        (
            input,
            Factor::Expression(Box::new(Expression::Structure(s))),
        )
    } else if let Ok((input, f)) = function_call(input.clone()) {
        (
            input,
            Factor::Expression(Box::new(Expression::FunctionCall(f))),
        )
...
pub fn range_expression(input: ParseString) -> ParseResult<RangeExpression> {
    ...
    let range = match y {
        Some((op2, terminal)) => RangeExpression {
            start,
            increment: Some((op, x)),
            operator: op2,
            terminal,
        },
        None => RangeExpression {
            start,
            increment: None,
            operator: op,
            terminal: x,
...
pub fn matrix_comprehension(input: ParseString) -> ParseResult<MatrixComprehension> {
    ...
    if !quals.iter().any(|q| {
        matches!(
            q,
            ComprehensionQualifier::Generator(_) | ComprehensionQualifier::Let(_)
        )
    }) {
...
pub fn bracket_subscript(input: ParseString) -> ParseResult<Subscript> {
    ...
    let (input, subscripts) = separated_list1(
        list_separator,
        alt((select_all, range_subscript, formula_subscript)),
    )(input)?;
    let (input, _) = right_bracket(input)?;
