...
pub enum AstNode {
  ...
  SelectData              { name: Vec<char>, id: TableId, children: Vec<AstNode> },
  UpdateData              { name: Vec<char>, children: Vec<AstNode> },
  TableColumn             { children: Vec<AstNode> },
  Function                { name: Vec<char>, children: Vec<AstNode> },
  ...
  Statement               { children: Vec<AstNode> },
...
pub fn print_ast_node(node: &AstNode, level: usize, f: &mut fmt::Formatter) {
  ...
  let children: Option<&Vec<AstNode>> = match node {
    ...
    AstNode::Block{children, ..}            => { write!(f, "Block\n").ok(); Some(children) },
    AstNode::Statement{children}            => { write!(f, "Statement\n").ok(); Some(children) },
    AstNode::SetData{children}              => { write!(f, "SetData\n").ok(); Some(children) },
    AstNode::UpdateData{name, children}     => { write!(f, "UpdateData({:?})\n", name).ok(); Some(children) },
    ...
    AstNode::SelectData{name, id, children} => { write!(f, "SelectData({:?} {:?}))\n", name, id).ok(); Some(children) },
    ...
    AstNode::Function{name, children}       => { write!(f, "Function({:?})\n", name).ok(); Some(children) },
    ...
    AstNode::Identifier{name, id}           => { write!(f, "Identifier({:?}({}))\n", name, humanize(id)).ok(); None },
    AstNode::String{text}                   => { write!(f, "String({:?})\n", text).ok(); None },
    AstNode::RationalNumber{children}       => { write!(f, "RationalNumber\n").ok(); Some(children) },
    AstNode::NumberLiteral{kind, bytes}     => { write!(f, "NumberLiteral({:?})\n", bytes).ok(); None },
    AstNode::Quantity{children, ..}         => { write!(f, "Quantity\n").ok(); Some(children) },
    AstNode::Table{name,id}                 => { write!(f, "Table(#{:?}({:#x}))\n", name, id).ok(); None },
    AstNode::Define{name,id}                => { write!(f, "Define #{:?}({:?})\n", name, id).ok(); None },
    AstNode::Token{token, chars}            => { write!(f, "Token({:?})\n", token).ok(); None },
    ...
    // Markdown Nodes                                      
    AstNode::Title{text}                    => { write!(f, "Title({:?})\n", text).ok(); None },
    AstNode::ParagraphText{text}            => { write!(f, "ParagraphText({:?})\n", text).ok(); None },
...
pub enum ParserNode {
  ...
  Block                   { children: Vec<ParserNode> },
  Statement               { children: Vec<ParserNode> },
...
fn print_parser_node(node: &ParserNode, level: usize, f: &mut fmt::Formatter) {
  ...
  let children: Option<&Vec<ParserNode>> = match node {
    ...
    ParserNode::Block{children}                   => { write!(f, "Block\n"); Some(children) },
    ...
    ParserNode::Statement{children}               => { write!(f, "Statement\n"); Some(children) },
    ...
    ParserNode::Token{token, chars}               => { write!(f, "Token({:?} ({:?}))\n", token, chars); None },
    ...
    ParserNode::FloatLiteral{chars}               => { write!(f, "FloatLiteral({:?})\n", chars); None },
    ParserNode::DecimalLiteral{chars}             => { write!(f, "DecimalLiteral({:?})\n", chars); None },
    ParserNode::HexadecimalLiteral{chars}         => { write!(f, "HexadecimalLiteral({:?})\n", chars); None },
    ParserNode::OctalLiteral{chars}               => { write!(f, "OctalLiteral({:?})\n", chars); None },
    ParserNode::BinaryLiteral{chars}              => { write!(f, "BinaryLiteral({:?})\n", chars); None },
    ...
    ParserNode::False                             => { write!(f, "True\n",); None },
    ParserNode::True                              => { write!(f, "False\n",); None },
