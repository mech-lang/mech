...
fn impl_assign_scalar_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(
      UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "matrix/assign-scalar".to_string() },
      None
    ).with_compiler_loc()
  )
}
...
fn impl_assign_range_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignRange".to_string() }, None).with_compiler_loc())
}
    ...
    match impl_assign_range_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(x) => {
        match (sink.clone(),&ixes,source.clone()) {
          ...
          (sink, ixes, source) => Err(MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignRange".to_string() }, None).with_compiler_loc()),
        }
      }
    }
  }
...
fn impl_assign_all_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignAll".to_string() }, None).with_compiler_loc())
}
    ...
    match impl_assign_all_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(_) => {
        match sink {
          ...
          _ => Err(MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignAll".to_string() }, None).with_compiler_loc()),
        }
      }
    }
  }
...
fn impl_assign_scalar_scalar_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignScalarScalar".to_string() }, None).with_compiler_loc())
}
    ...
    match impl_assign_scalar_scalar_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(_) => {
        match sink {
          ...
          _ => Err(MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignScalarScalar".to_string() }, None).with_compiler_loc()),
        }
      }
    }
  }
...
fn impl_assign_all_scalar_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignRangeScalar".to_string() }, None).with_compiler_loc())
}
    ...
    match impl_assign_all_scalar_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(x) => {
        match sink {
          ...
          sink => Err(MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignRangeScalar".to_string() }, None).with_compiler_loc()),
        }
      }
    }
  }
    ...
    match impl_assign_scalar_all_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(_) => {
        match sink {
          ...
          _ => Err(MechError2::new(UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "MatrixAssignRangeScalar".to_string() }, None).with_compiler_loc()),
        }
      }
    }
  }
...
fn impl_assign_scalar_range_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .map_err(|_| MechError2::new(
    UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.to_vec(), source.clone()), fxn_name: "MatrixAssignScalarRange".to_string()  }, None).with_compiler_loc()
  )
}
...
macro_rules! impl_assign_range_range_fxn_s {
  ($struct_name:ident, $op:tt, $ix1:ty, $ix2:ty) => {
    ...
    impl<T, R, C, S: 'static, IxVec1: 'static, IxVec2: 'static> MechFunctionFactory for $struct_name<T, na::Matrix<T, R, C, S>, IxVec1, IxVec2>
    where
      Ref<naMatrix<T, R, C, S>>: ToValue,
      T: Scalar + Clone + Debug + Sync + Send + 'static + CompileConst + ConstElem + AsValueKind,
      IxVec1: CompileConst + ConstElem + Debug + AsRef<[$ix1]> + AsNaKind,
      IxVec2: CompileConst + ConstElem + Debug + AsRef<[$ix2]> + AsNaKind,
      R: Dim, C: Dim, S: StorageMut<T, R, C> + Clone + Debug,
      naMatrix<T, R, C, S>: CompileConst + ConstElem + Debug + AsNaKind,
    {
      fn new(args: FunctionArgs) -> MResult<Box<dyn MechFunction>> {
    ...
    impl<T, R, C, S, IxVec1, IxVec2> MechFunctionImpl for $struct_name<T, na::Matrix<T, R, C, S>, IxVec1, IxVec2>
    where
      Ref<naMatrix<T, R, C, S>>: ToValue,
      T: Scalar + Clone + Debug + Sync + Send + 'static,
    ...
    #[cfg(feature = "compiler")]
    impl<T, R, C, S, IxVec1, IxVec2> MechFunctionCompiler for $struct_name<T, na::Matrix<T, R, C, S>, IxVec1, IxVec2> 
    where
      T: CompileConst + ConstElem + AsValueKind,
...
fn impl_assign_range_range_fxn(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
  ...
  .or_else(|_| impl_assign_fxn!(impl_assign_range_range_arms, Assign2DRR, arg, u128, "u128"))
  .or_else(|_| impl_assign_fxn!(impl_assign_range_range_arms, Assign2DRR, arg, i8, "i8"))
  .or_else(|_| impl_assign_fxn!(impl_assign_range_range_arms, Assign2DRR, arg, i16, "i16"))
  ...
  .map_err(|_| MechError2::new(
      UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.to_vec(), source.clone()), fxn_name: "matrix/assign-range".to_string() },
      None
    ).with_compiler_loc()
  )
}
    ...
    match impl_assign_range_range_fxn(sink.clone(),source.clone(),ixes.clone()) {
      ...
      Err(_) => {
        match sink {
          ...
          _ => Err(MechError2::new(
              UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "matrix/assign-range".to_string() },
              None
            ).with_compiler_loc()
          ),
        }
  ...
  ($op_fxn_name:tt, $fxn_name:ident) => {
    paste::paste! {
      fn $op_fxn_name(sink: Value, source: Value, ixes: Vec<Value>) -> MResult<Box<dyn MechFunction>> {
        ...
        .map_err(|_| MechError2::new(
            UnhandledFunctionArgumentIxes { arg: (sink.clone(), ixes.clone(), source.clone()), fxn_name: "matrix/assign-range-all".to_string() },
            None
          ).with_compiler_loc()
        )
      }
