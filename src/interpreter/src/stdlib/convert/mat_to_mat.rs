    ...
    TTo: Debug + Scalar + Default + ConstElem + CompileConst + AsValueKind,
{
    ...
    match (v, shape[0], shape[1]) {
        ...
        (Matrix::Matrix3(v), 1, 9) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(RowDVector::from_element(9, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "matrix3", feature = "vectord"))]
        (Matrix::Matrix3(v), 9, 1) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(DVector::from_element(9, zero)),
                _marker: PhantomData,
            }));
        }

        #[cfg(all(feature = "matrix4", feature = "row_vectord"))]
        (Matrix::Matrix4(v), 1, 16) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(RowDVector::from_element(16, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "matrix4", feature = "vectord"))]
        (Matrix::Matrix4(v), 16, 1) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(DVector::from_element(16, zero)),
                _marker: PhantomData,
            }));
        }

        #[cfg(all(feature = "matrix3x2", feature = "row_vectord"))]
        (Matrix::Matrix3x2(v), 1, 6) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(RowDVector::from_element(6, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "matrix3x2", feature = "vectord"))]
        (Matrix::Matrix3x2(v), 6, 1) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(DVector::from_element(6, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "matrix3x2", feature = "matrix2x3"))]
        (Matrix::Matrix3x2(v), 2, 3) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(Matrix2x3::from_element(zero)),
                _marker: PhantomData,
        ...
        (Matrix::RowDVector(v), n, 1) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(DVector::from_element(n, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "row_vectord", feature = "matrixd"))]
        (Matrix::RowDVector(v), n, m) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
        ...
        (Matrix::DVector(v), 1, n) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(RowDVector::from_element(n, zero)),
                _marker: PhantomData,
            }));
        }
        #[cfg(all(feature = "vectord", feature = "matrixd"))]
        (Matrix::DVector(v), n, m) => {
            return Ok(Box::new(ConvertMatToMat2 {
                arg: v,
                out: Ref::new(DMatrix::from_element(n, m, zero)),
                _marker: PhantomData,
    ...
    pub fn impl_conversion_mat_to_mat_fxn(
      ...
      if let (Value::MatrixValue(source_matrix), ValueKind::Matrix(target_element_kind, target_dims)) = (&source_value, &target_kind) {
        ...
        let output_shape = if target_dims.is_empty() {
          source_shape.clone()
        } else if source_shape == *target_dims {
          target_dims.clone()
        } else if source_shape[0] * source_shape[1] == target_dims[0] * target_dims[1] {
          target_dims.clone()
        } else {
          return Err(MechError::new(
            ReshapeError { original: (source_shape[0], source_shape[1]), requested: (target_dims[0], target_dims[1]) },
            None,
          ).with_compiler_loc());
        };
        ...
        let converted = source_matrix
          ...
          .into_iter()
          .map(|value| {
            if matches!(target_element_kind.as_ref(), ValueKind::Any) {
            ...
            match value {
              ...
              other => other
                .convert_to(target_element_kind.as_ref())
                .ok_or_else(|| MechError::new(
                  UnsupportedConversionError { from: other.kind(), to: target_element_kind.as_ref().clone() },
                  None,
                ).with_compiler_loc()),
            }
          })
...
impl MechErrorKind for ReshapeError {
    ...
    fn message(&self) -> String {
        format!(
            "Cannot reshape matrix of shape {:?} into {:?}",
