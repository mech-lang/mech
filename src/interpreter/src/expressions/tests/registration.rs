...
use crate::{
  MechFunction, MechFunctionImpl, MResult, Plan, ReactiveCellId, ReactiveDependencyKind, Ref,
  Value,
};
...
fn binary_term_batch_registration_preserves_order_and_edges() {
  ...
  register_expression_function_batch(&plan, vec![(f1, vec![a, b]), (f2, vec![mid, c])])
    .unwrap();
  let plan = plan.borrow();
