...
impl MechFunctionCompiler for MatrixMul {
  ...
  fn compile(&self, block: &mut Block, arguments: &Vec<Argument>, out: &(TableId, TableIndex, TableIndex)) -> std::result::Result<(),MechError> {
    ...
    match (&arg_shapes[0],&arg_shapes[1]) {
      ...
      (TableShape::Row(columns), TableShape::Column(rows)) => {
        ...
        match (lhs_kind, rhs_kind) {
          ...
          (ValueKind::Compound(_),_) => {
            return Err(MechError{id: 3042, kind: MechErrorKind::GenericError("matrix/multiply doesn't support compound table kinds.".to_string())});
          }
          (k,j) => {
            if (k == j) {
              ...
              return Err(MechError{id: 3042, kind: MechErrorKind::GenericError("matrix/multiply doesn't support disparate table kinds.".to_string())});
            }
          }
        }
      ...
      x => {return Err(MechError{id: 9404, kind: MechErrorKind::GenericError(format!("{:?}", x))});},
    }
    Ok(())
  }
}
