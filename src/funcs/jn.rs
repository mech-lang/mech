  ...
  ($arg1:expr, $arg2:expr, $out:expr) => {
    unsafe {
      ...
      for i in 0..arg1_deref.len() {
        (out_deref[i]).0 = jn(arg1_deref[i].0,arg2_deref[i].0);
      }}};}

macro_rules! jnf_op {
  ($arg1:expr, $arg2:expr, $out:expr) => {
    unsafe{(*$out).0 = jnf((*$arg1).0,(*$arg2).0);}
  };}
...
macro_rules! jnf_vec_op {
  ($arg1:expr, $arg2:expr, $out:expr) => {
    unsafe {
      ...
      let mut out_deref = (&mut *$out);
      for i in 0..arg1_deref.len() {
        (out_deref[i]).0 = jnf(arg1_deref[i].0,arg2_deref[i].0);
      }}};}
