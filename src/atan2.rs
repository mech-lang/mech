...
impl MechFunctionCompiler for MathAtan2 {
  ...
  ) -> std::result::Result<(),MechError>
  {
    ...
    if (arg_name1 == *Y && arg_name2 == *X) {
        ...
        (TableShape::Scalar,TableShape::Scalar) => {
          ...
          if let Column::F32(out_col) = out_brrw.get_column_unchecked(0) {
            match (arg1,arg2) {
              ...
              ((_,col,_),_) => { return Err(MechError{id: 1348, kind: MechErrorKind::UnhandledFunctionArgumentKind(col.kind())}); }
            }
          }
        }
      ...
      return Err(MechError{id: 1351, kind: MechErrorKind::UnknownFunctionArgument(arg_name1)});
    }
    Ok(())
  }
}
