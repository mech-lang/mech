...
def main() -> None:
    ...
    rust_ceiling = values["Rust packed SIMD unchecked"]
    ...
    lines = [
        ...
        '<text x="52" y="42" font-size="26" font-weight="700">Parallel EKF steady-state throughput</text>',
        '<text x="52" y="68" class="muted" font-size="15">Apple M1 | 10,000 filters x 20 turns | median of three isolated process samples | million EKF turns per second</text>',
    ]
    ...
    lines.append(f'<text x="{ceiling_x + 8:.1f}" y="{top - 29}" fill="#f3a847" font-size="13">Rust packed-SIMD ceiling: {rust_ceiling:.2f}</text>')
    ...
    lines.append(f'<text x="52" y="{height - 55}" class="muted" font-size="12">GPU rows are Mech context measurements, not CPU comparisons. Parse, compilation, setup, and readback are excluded.</text>')
    lines.append('</svg>')
