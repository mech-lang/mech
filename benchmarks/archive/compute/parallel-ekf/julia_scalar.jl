...
mutable struct Scratch
    ...
    g::Matrix{Float32}
    left::Matrix{Float32}
    predicted_p::Matrix{Float32}
    process_left::Matrix{Float32}
    process_p::Matrix{Float32}
    pht::Vector{Float32}
    a::Matrix{Float32}
    ap::Matrix{Float32}
    corrected_p::Matrix{Float32}
end
...
Base.@inline function step!(state, covariance, lane, velocity, angular_velocity, bearing, s)
    ...
    s.f[2,2]=1f0
    s.f[3,2]=0f0
    s.f[1,3]=-distance*sin_theta
    s.f[2,3]=distance*cos_theta
    s.f[3,3]=1f0
    s.g[1,1]=cos_theta*dt
    s.g[2,1]=sin_theta*dt
    s.g[3,1]=0f0
    s.g[1,2]=0f0
    s.g[2,2]=0f0
    s.g[3,2]=dt
    ...
    mul!(s.ap, s.a, s.predicted_p)
    mul!(s.corrected_p, s.ap, transpose(s.a))
    for column in 1:3, row in 1:3
...
s = Scratch()
dispatch!(state,covariance,velocity,angular_velocity,bearing,5,s)
state .= reshape(Float32[55,25,0.4],3,1)
covariance .= reshape(Float32[100,0,0,0,100,0,0,0,0.15],3,3,1)
started = time_ns()
faults = dispatch!(state,covariance,velocity,angular_velocity,bearing,turns,s)
